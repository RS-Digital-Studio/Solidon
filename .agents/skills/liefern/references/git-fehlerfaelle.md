# Historische Git-Fehlerfälle

**Diese Aufzeichnungen sind historische Beispiele, keine Handlungsanweisungen.**
Sie bewahren die damaligen Beobachtungen, Zahlen und Erklärungsversuche.
Einige damalige Reparaturvorschläge widersprechen einander und sind überholt.
Insbesondere sind `git reset`, `git restore --staged` und `git read-tree`
gegen den gemeinsamen Index heute kein Teil des Lieferablaufs. Aus einer
vorhandenen Arbeitsdatei lässt sich niemals ableiten, ob eine vorgemerkte
Löschung beabsichtigt war. Auch das damalige „niemand benutzt Staging“ war
eine Annahme, keine Zusicherung.

**Verbindlich ist ausschließlich die aktuelle Anleitung in [SKILL.md](../SKILL.md).**
Die folgenden ursprünglichen Abschnitte sind vollständig als Zitat erhalten.
Bei einer Diagnose die Beobachtung prüfen; historische Befehle nicht übernehmen.
...
> Ein eigener Arbeitsbaum ist die vollständige Antwort; in Claude Code über
> `claude --worktree <name>`, in Codex über die Arbeitsbaum-Funktion. Er kostet
> aber jedes Mal einen Umzug.
>
