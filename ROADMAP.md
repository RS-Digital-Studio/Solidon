...
- [x] **Ein gescheiterter Merge ist ein Eingriff, kein Nichts.** Erledigt am
      31.08.2026: Die Regel steht jetzt im Verfahren — eigener Abschnitt im
      `liefern`-Skill („Wer einen Merge abbricht, prüft danach `git status`
      und `git stash list`"), samt Rückspiel-Auflage für einen
      zurückgebliebenen Autostash. Gefunden am
...
- [x] **Geschlossen mit Negativbefund und Messung statt Gefühl (15,
      31.08.2026):** 3882 Oberflächentexte per AST erhoben, 497 mehrfach —
      und Mehrfachnutzung ist bei Parameterbeschriftungen das **gewünschte**
      Verhalten (eine Übersetzung, überall dieselbe; „Höhe" beim Quader und
      beim Zylinder zu trennen hieße, sie auseinanderlaufen zu lassen).
      Gefährlich wäre die Gegenrichtung — ein Wort, zwei Bedeutungen, eine
      Übersetzung: 45 kurze Wörter in drei oder mehr Gebieten gezielt
      nachgesehen (Art, Spiel, Steigung, Größe …), alle kontextfrei
      tragfähig, **null Funde**. Wer die Zeile später wieder aufmacht, soll
      wissen: Hier wurde gemessen, nicht bloß nicht geschaut. Ursprünglich:
...
- [x] **Erledigt in zwei Stufen, und das Register hinkte zum dritten Mal an
      einem Tag:** Das Paket war mit `a8c87e97` längst gebaut (groups,
      synchrone Kappung, `Release.grouped()`, Dialog über `groups_html`);
      `b1ed3263` liefert die Alt-Client-Messung nach (v0.2.1/v0.2.2 aus
      echten Worktrees mit echter Unterschrift: beide lesen vollständig,
      unbekanntes Feld bricht nichts — `signed_payload` unterschreibt alles
      außer der Unterschrift, eine Feldliste dort hätte jede ausgelieferte
      Fassung gebrochen) **und schließt den eigentlichen Fund: Den Inhalt
      der ausgehenden version.json prüfte niemand** — der eine Nachbartest
      misst die Quelle, der andere zählt Bytes, dazwischen lag die Kappung
      ungeprüft; zwei neue Tests, Mutationsprobe drei von drei rot.
      Ursprünglich: **d3 baut das Paket** (beauftragt 30.08.2026): `groups` je Sprache
...
| P6 | Nach dem Drehen aufs Bett setzen (Haken, Vorgabe an, ein Undo) — **zu (`eb53d8ce`, reviewt)**: Haken „aufs Bett" in der Drehen-Rolle, vorgewählt und trotzdem einer (wer eine Baugruppe in ihrer Lage dreht, braucht ihn nicht); beide Drafts in **einer** Transaktion „Drehen und aufs Bett setzen" — zwei Undos für eine Handlung wären der Fehler. Drei Tests messen Geometrie (Unterkante null), das eine Undo und die Gegenprobe (Verschieben setzt nie auf); Leiste unverändert 867/887. Blob-Commit wegen 15s paralleler D6-Arbeit, beidseitig nachgemessen und beidseitig gehalten | S | **zu — d3, reviewt** |
...
| P9 | Aufeinanderfolgende Züge werden ein Verlaufsschritt — **entschieden (Freigabe, 30.08. spät), mit angesagter Bauplan-Änderung**: Bauplan §15.5 („Manuelle Operationen sind Einzeltransaktionen") bekommt einen Bündel-Satz — d3 hat die Kollision gemeldet statt still zu bauen, genau richtig. Der Schnitt ist eng: Verschmolzen wird nur **dieselbe Operation auf denselben Eingängen mit demselben Anker**, und **Bündeln ist opt-in je Operation** — nur wer eine Kumulationsregel definiert, bündelt (Verschieben: Vektorsumme; Drehen: nur gleiche Achse, Winkelsumme; **Skalieren bündelt vorerst nicht** — multiplikativ wäre rechenbar, aber der Kundenfall ist „dreimal nachgeschoben", und die gefährliche Kante bleibt in Ruhe). Ort: in der `History`, sofort landen, der nächste gleichartige Zug **ersetzt** mit kumuliertem Wert (Texteditor-Muster). Ende des Bündels: jede andere Handlung dazwischen — Auswahlwechsel, Werkzeugwechsel, andere Operation, Schließen der Leiste; **keine Zeitpause** (geratene Zahl). Ein Undo nimmt das ganze Bündel. Tests messen an der Geometrie, Gegenprobe: zwei Achsen ergeben zwei Schritte. **Dazu Roberts Erweiterung (30.08., 22:3x, wörtlich „oberpunkte im verlauf auch noch einklappen, damit es nicht zu voll wird, baum logik"):** Der Verlauf zeigt Mehr-Teile-Schritte (P6-Paare, P9-Bündel, Mehr-Op-Transaktionen) als **einklappbare Oberpunkte**, Vorgabe eingeklappt — Titel und Ergebnis am Oberpunkt, Teilschritte darunter. Auflagen: setExpanded lügt vor dem Anzeigen (Test am gezeigten Baum), und der V6-Anschluss hält — ein Befund-Klick auf eine Teil-Op klappt den Oberpunkt auf statt ins Leere zu zeigen. Bei d3, mit P9 zusammen. §15.5-Satz eng formulieren; die Änderung ist Robert im Bericht ausdrücklich angesagt und mit einem Satz rücknehmbar. **Zu (`f9059280`, reviewt) — P9 und Roberts Verlaufsbaum in einer Landung:** `bundling.py` hält die Kumulationsregeln (Vektorsumme, Winkelsumme nur gleiche Achse, Skalieren ausdrücklich nicht — „was hier fehlt, kann jederzeit dazukommen; umgekehrt wäre es ein Rückbau"), §15.5 trägt den Satz, der Anker vergleicht mit Toleranz statt `==`. Der Verlauf klappt ein: Dreieck ▸/▾ als zweite Kodierung neben der Einrückung (ein Bildschirmleser liest keine Leerzeichen), aufgeklappter Zustand überlebt den Neuaufbau, V6-Anschluss mit eigenem Test (Befund-Klick klappt auf). Der stille Bruch saß woanders als erwartet: `setHidden` an einem Element **ohne Liste** tut nichts und meldet nichts — Familie „Qt lügt vor dem Anzeigen", von d3s eigenen Tests gefangen. Zwölf Tests, sechs Mutationen; ein Test wurde begründet gelockert (Gleichheit → `in`, die Zusage unverändert). Freigabe-Stichprobe test_bundling 8/8. **Ein Nachzug bleibt: `_ANKER_TOLERANz`-Bezeichner ist deutsch** — umbenennen auf `_ANCHOR_TOLERANCE` und den Stamm `anker` in GERMAN_STEMS eintragen; die kuratierte Liste kannte ihn nicht, der Test blieb grün. **Dazu d3s Verfahrens-Fund am neuen Fortschrittsleser: Ein Segfault nach 15 von 60 verdeckte in drei Wiederholungen je 1 failed** — ohne Zeichenzählung wäre der Riss als Abbau-Riss durchgegangen und der echte Fehlschlag nie gesehen worden; der G18-Bau zahlt sich am ersten Abend aus. **Und eine Betriebs-Erkenntnis der Nacht (50/d3): Der pre-commit-Hook prüft den Arbeitsbaum, nicht den Index** — legen zwei Sitzungen gleichzeitig Katalogtexte an, erbt jede die fehlenden Schlüssel der anderen, und alle hängen an der unfertigsten Datei; das Mittel ist, Kataloge und Bezeichner **sofort** nachzuziehen statt am Ende. **Und die zweite Seite derselben Sache (50, an meinem P9-Registertext vorgeführt): Der Hook hat eine Laufzeit, und sie liegt NACH dem Index** — `read-tree HEAD` unmittelbar vor `git commit` schützt nicht, wenn der pre-commit Sekunden bis Minuten fährt und ein fremder Commit hineinfällt: 50s Commit nahm meine P9-Fortschreibung zurück (sieben Dateien im Index, acht im Commit), sie fand es an der Numstat-Kontrolle und stellte in `090f07db` wieder her. Konsequenz für jede Kette: **nach** dem Commit die eigene Dateizahl gegen den Numstat halten — eine Zahl über der eigenen ist das Rennen, nicht der eigene Index. **Präzisiert nach 3as Fehlalarm (Kontrollvariante der Index-Alterung):** Geprüft wird der **eigene Hash**, nie `HEAD` — im geteilten Baum ist HEAD schon beim Lesen ein anderer (3as Sollprobe zeigte 58 fremde Zeilen, weil 15s Commit dazwischenfiel; der eigene war sauber). Den Hash direkt aus der Commit-Ausgabe nehmen und `git show --numstat <eigener-hash>` fahren. **Und die Memory-Index-Regel aus dem Takt ist gestrichen — sie ist jetzt ein Wächter** (`1f71b74a` + `a874a863`, 15, reviewt): `test_directory_docs` prüft beide Richtungen (Erinnerung ohne Zeiger, Zeiger ohne Erinnerung), Gegenprobe aus git gegen den Stand vor der Reparatur (2+1 Funde, beide Richtungen rot), drei Index-Löcher des Tages geschlossen. Dazu 15s Selbstbefund: sechs der 76 schalen Stempel-Verweise gehören ihrer make_legal-Landung (frisch erzeugt heißt stempellos — wer den Erzeuger fährt, zieht den Stempellauf nach oder sagt an, dass er es nicht kann); Behebung im gesammelten Lauf nach WD3 | M | **zu — d3, reviewt; ein Bezeichner-Nachzug** |

- [x] **Alle Pakete stehen — am Code belegt, nicht am Register abgelesen**
      (d3, 31.08.2026, vierter Register-hinkte-Fall des Tages): P9 liegt in
      `app/core/scene/bundling.py` mit den Kumulationsregeln, P6 als Block
      „Nach dem Drehen aufs Bett — in derselben Transaktion" in
      `main_window.py:9197`, und auch der Bezeichner-Nachzug ist längst da
      (`_ANCHOR_TOLERANCE`, `anker` in GERMAN_STEMS — `f20bc7c2`).
      Ursprünglich: Die neun Pakete abarbeiten — je Paket Zahlen, Gegenproben
      und Review vor dem Commit; achsweises Skalieren bleibt bewusst
...
| D3 | Fehlerdialog und ComfyUI-Dialog haben keinen/den falschen Hauptknopf | S | **15, nach WD2** |
...
| D5 | Leere Pflichtliste lässt den Hauptknopf aktiv — sperren und begründen, Grund aus `_needs_phrase` | M | **15, nach WD2** |
| D6 | Die Kopfzeile verschwindet unter 2330 px Fensterbreite ins unbeschriftete Überlaufmenü — kürzen statt verschwinden | M | **15, nach WD2** |
...
| D8 | **Zweiundzwanzig Schlüssel sprachen Deutsch, und sieben standen neben ihrem Zwilling** (`54d2db34`, 15) — der Vormittagsbefund war fünfmal zu klein: 22 deutsche Platzhalternamen in 38 Quelltexten, 49 Vorkommen. Zwei Kaper-Fallen geprüft statt gewählt (stufe→severity, nicht step — das ist der Verlaufsschritt; ort→drive, nicht place — das ist die Zeichenebene: „zwei Bedeutungen teilen sich ein Wort, also darf keine es bekommen"). Der Doppelsatz „Knochen" fiel durch die Umbenennung zusammen, und die Werkzeug-Zusicherung ließ genau diesen einen Zusammenfall zu und prüfte die Übersetzungsgleichheit — jeder andere hätte einen Text spurlos verschluckt. Wächter mitgebaut: kuratierter Grundstock plus Vollauflistung bei Verstoß plus Mindestfundzahl (kein Verbotstest über leerer Menge). Nebenbefund fürs Register: `pose_bar` schreibt „1 Knochen", wo `op_dialog` „Ein Knochen" sagt — Singular-Regel nur an einer der zwei Stellen | M | **fertig** |
...
| D10 | **Der Chat rechnete in Token ab, und niemand hatte danach gefragt** (`9b189966`, 15) — statt „24512 → 1830 Token" und „Tokenbudget erreicht" liest der Kunde jetzt, was geschah und was hilft: „Diese Anfrage war umfangreich und endet hier … Übernehmen und weiterfragen führt fort; jede neue Anfrage beginnt von vorn" — und dass genau das hilft, ist am Kern gemessen (`spent` ist je Anfrage lokal), nicht ausgedacht. Die Zahlen liegen dreifach am Element (Tooltip, Statuszeile, Bildschirmleser), wo „Token" als Abrechnungsname stehen darf — dieselbe Begründung wie Skirt und Brim. Zwei Tests schrieben den alten Zustand wörtlich fest und prüfen jetzt die Zusage über die Menge der vier Ausgänge; eine Mutation blieb grün, weil sie selbst danebenband (Pythons Präzedenz) — „eine Mutation, die den Fall nicht trifft, sagt über den Test nichts" | S | **fertig** |
...
| Z6 | Der Knopf heißt „Grundform", ist das Rechteckwerkzeug und versteckt das beste Anfänger-Angebot (sechs vollbemaßte Formen) hinter einem Pfeil | S | zu (`84589c99` + `6e1170b5`, beide reviewt) — der Rest war echt: Die Leerzustands-Zeile nannte „Grundform einfügen", ein Ziel, das es unter dem Namen nicht gab; jetzt ist sie der Weg, den sie behauptet (anklickbarer Verweis „Rechteck-Menü", Klick/Enter öffnet mit Fokus im Menü; Muster-Grenze in Code und Test: Leerzustand lädt ein, laufende Meldungen bleiben PlainText). **Z6b als präzisierter Rest offen:** Der Knopf zeigt am Bildschirm nur Symbol+Pfeil, `text()` sagt „Rechteck", der Nutzer sieht es nicht — wer den Satz liest statt klickt, sucht weiter; nicht die Formen fehlen, der Knopfname fehlt |
| Z7 | Der Kreis misst Radius, der Kunde denkt in Durchmesser (M3 → Ø3,2 getippt = Ø6,4 gedruckt): sofort „Radius" statt „Abstand" beschriften und „R" ans Maßfeld; die echte Ø-Bedingung im Kern ist der eigene Punkt darunter | S | zu (`2f5d0920` + `6f252023`, reviewt) — die „Radius"-Beschriftung steht im Editor, von 72 am Quelltext belegt |
...
| Z9 | **Dieselbe Operation, zwei Umgebungen — je nachdem, ob man sie anlegt oder ändert.** Über das Menü angelegt: Zeichenmodus mit Ziehgriff, Maßeingabe im Bild, Ebenenkarten, Zeichnung am Körper. Aus dem Verlauf geändert: weißes Blatt im Dialog. Gemessen sechs verlorene Fähigkeiten (`set_sketch_pull`, `set_sketch_entry`, `set_sketch_edit`, Ebenenkarten, räumlicher Bezug, Auswahl-Kapsel), fünf betroffene Operationen, **ein** Aufrufer des Dialogs im ganzen Programm. Der schwerste Verlust war der Ziehgriff: Eine Tasche ließ sich dort nicht ziehen, obwohl genau diese Geste sie erzeugt hat. Jetzt ein Knopf „Im Raum zeichnen“, der den Dialog **endgültig** schließt (kein modales Fenster, das wiederkommt — die übrigen Werte stehen am Schritt) — und **„Fertig“ ändert denselben Schritt**, wie beim Skeletteditor: gemessen 2 Schritte vorher, 2 nachher, Dialog zurück als „Operation 2“. Der Knopf steht nur, wo es etwas zu ändern gibt. Zwei Tests, vier Gegenproben, alle rot | M | **fertig** (`7a71154b`, 50) |

- [x] **Z6b ist erledigt — von 72 gemessen und zu eigenen Ungunsten
      zurückgenommen** (31.08.): Der Icon-only-Knopf ist regelkonform
      (`oberflaeche.md`: ein Rechteck ist ein geeinigtes Bild, und Wort
      steht an QAction, Tooltip und statusTip weiter — plus
      accessibleDescription); die Zwillingssuche über alle 3 924
      Katalogtexte fand **keinen** Anleitungstext, der einen der vier
      verborgenen Knöpfe als Weg nennt, außer dem heute gebauten Verweis —
      und der löst es auf, denn wer klickt, braucht den Namen nicht.
      Erste Meldung war ein Foto-Eindruck, keine Messung. Die ehrliche
      Restfrage („sieht jemand die Statuszeile?") ist eine
      Mensch-vor-Bildschirm-Frage für eine spätere Live-Durchsicht.
      Ursprünglich: Die neun Z-Pakete abarbeiten — Z1 zuerst; je Paket Zahlen,
      Gegenproben und Review vor dem Commit.
- [x] **Ø-Bedingung im Kern** (aus Z7, Variante b) — **erledigt seit
      `2f5d0920`, die Zeile wusste es zwei Tage nicht** (nachgemessen 15,
      31.08.2026, alle vier Ebenen: Löser, Serialisierung, Editor, Format 19
      mit umdeutender Migration). Was wirklich fehlte, war der Wächter über
      „bis in die Projektdatei": `c8c16a16` fährt lösen → schreiben → lesen →
      wieder lösen und prüft, dass die Art wörtlich in der Datei steht und
      der Wert als Ausdruck reist — Gegenprobe rot. Ursprünglicher Text: eine
      echte Durchmesser-Bedingungsart in `sketch/solver.py` samt Feld,
...
| V6 | **Zu** (`9da6d4fa` — der Befund lag schwerer als notiert: nicht „fliegt ohne Marke", sondern **58 von 58 Befunden lösten beim Klick gar nichts aus**. Zwei Ursachen: Ein Operationsfehler trägt weder Ort noch Merkmale (`evaluate.py:1644` gibt ihm nur `object_id` und `op_id`), und der Ort eines Kartenbefunds steht erst fest, wenn die Karte gerechnet ist — beim ersten Klick nie, beim zweiten schon, was wie ein Bedienfehler des Kunden aussah. Die Zusage heißt jetzt „ein Klick auf einen Befund bleibt nie folgenlos" und antwortet gestuft: Ort → Flug samt vergänglicher Marke, Körper → Auswahl (57 von 58, vorher 0), `op_id` → der Verlauf zeigt den Schritt. Nebenbei behoben: `fly_to` nahm seinen Punkt roh und flog bei Platte 2 eine Bettbreite daneben. **Offen an Stufe 1:** in den Beispielprojekten nicht auslösbar, weil deren Karten nichts hervorheben — einzeln belegt und getestet; der Kundenweg-Nachweis braucht ein Projekt mit echter Warnung und folgt als kleiner Anschluss | M | **fertig** (3a); Warnprojekt-Nachweis als Anschluss |
...
| G14 | **Roberts Order (30.08.2026): die Statusleiste unten komplett prüfen und sauber, modern, innovativ gestalten.** Dreistufig: (1) Bestandsaufnahme — jedes Element, seine Herkunft und seine Zustände (Statusmeldungen/announce, Fortschritt, Lizenzstand, Verbrauch; was erscheint wann), mit Belegbildern; (2) Gestaltungsentwurf im Ton der G6-Notiz — Hierarchie und Bereiche, Anzeige mit Weg (klickbare Auskünfte führen dorthin, wo die Sache lebt: Verbrauch zu den Druckeinstellungen, Lizenz zur Freischaltung), Symbole als zweite Kodierung, Akzent-Haushalt beachten; Review vor dem Bau; (3) Bau mit vorher/nachher. B24 war die Vorarbeit. **Der Entwurf liegt vor und ist gebilligt**: Heute zuckt bei 23 von 28 gemessenen Rechnungen ein Balken, der nichts sagt (elf davon unter 0,2 s — die häufigsten Gesten überhaupt); und die Entwurfs-Selbstkorrektur (72) machte den Bau kleiner: **Die 10-s-Schätzung existiert längst** (`remaining_time` samt „gleich fertig"-Fassung — die erste Suche hatte auf drei Signalnamen gegrept, an denen die Schätzung nicht hängt; die Suche antwortete sauber auf die falsche Frage). Wirklich fehlen zwei Stufen: unter 200 ms erscheint heute sofort etwas, und die Bis-2-s-Stufe (BusyCursor plus Statuszeile, ohne Balken) gibt es nicht als eigenen Zustand. **Entschieden: Die 2-s-Schwelle wird wörtlich nach §2.8 gebaut** — auch wenn eine 2,9-s-Rechnung ihren Balken dann nur 0,9 s zeigt: Das Abbrechen-Fenster ab zwei Sekunden gehört dem Kunden, und ein Balken, der in eine laufende Wartesituation eintritt (der BusyCursor steht ja schon), ist kein Zucken aus dem Nichts. Null zusätzliche Höhe, alle Widgets stehen. Der Stufen-Bau ist zu (`2400bfae` — zwei Zeitgeber, BusyCursor, die 2-s-Schwelle wörtlich; drei Gegenproben fallen an der jeweils gemeinten Zusage). Mitgeheilt: **eine G6-Regression, die seit `04fc6968` auf main stand** — der Symbol-Lesbarkeits-Wächter prüfte das Verhalten der entfernten `_recolour`-Methode, ohne ihren Namen zu tragen (die brauchbare Geschwisterfrage ist nicht „wer ruft das", sondern „wer würde rot, wenn das Gegenteil gälte"); der Test war zudem nur zufällig grün gewesen, weil er ohne Thema zweimal Windows-Systemfarben maß — der dritte Betriebslage-Fund des Tages, nachgezogen in `tests.md`. Der Test misst jetzt „das Zeichen ist auf seiner Fläche lesbar" gegen `active_fill` Der Anzeige-mit-Weg-Teil war vergessen, nicht verschoben — ehrlich gemeldet und nachgebaut (`f1ddb4ca`): Die Lizenzzeile **nannte** ihren Weg und ließ den Kunden ihn zu Fuß gehen — jetzt ist sie der Weg (Klick öffnet die Freischaltung), und der Verbrauch führt zu den Druckeinstellungen; beide sagen vorher, dass sie ein Weg sind (Zeiger, Tooltip, Bildschirmleser). Die Uhrenfrage ist gemessen entschieden: **zwei Uhren, eine Zahlenquelle** — verschieden sind die Anzeigen, nicht die Schwellen, also stehen `DELAY_MS` und `BAR_AFTER_MS` nebeneinander in `loading.py` und das Fenster importiert sie. Dazu die Doku-Nachzüge (`96f21ef6`). **Nachschlag auf Roberts Nachfrage (`5c9cebc2`):** G14 hatte acht Bestandstests seit zwei Stunden auf origin rot — `restoreOverrideCursor` nimmt blind das **oberste** Stapelelement, nicht das eigene, und räumte den fremden Wartezeiger ab, während der eigene darunter stehen blieb; der Zeiger ist jetzt eine Eigenschaft des Fensters statt eines Overrides der Anwendung (kein Stapel, ehrlichere Aussage, `release()` räumt mit). Der Grund, warum es der eigene Torlauf nicht fand, ist die dritte Gestalt der Abriss-Falle: **Ein Lauf, der abbricht, hat nicht bestanden, sondern aufgehört** — „0 F" bei 124 von 372 gelaufenen Tests sagt nichts über die 248 danach | M | **fertig** (72) |
| G15 | Das Fensterchrom: **Stufe 1 ist gemessen gegenstandslos** — Qt 6 setzt das Dunkel-Flag der Titelleiste selbst, sie folgt dem Thema bereits (Belegbilder beider Themen liegen vor; vier vermeintliche Befunde unterwegs waren alle der Prüfstand, der nicht den Weg der Oberfläche ging). Die Empfehlung gegen eine **eigene** Titelleiste steht: Windows zeichnet sie in der Akzentfarbe, die der Kunde eingestellt hat, und neunzehn Dialoge verhielten sich sonst anders als alles andere auf seinem Schirm. Roberts Auskunft kam direkt: **„moderner, innovativer"** — keine Fehler-, eine Gestaltungsfrage. Stufe-2-Entwurf (d3, mit 72 abgestimmt): die vier DWM-Attribute je Fenster — Mica als Hintergrundmaterial, Titelleiste und Titeltext auf die `window`-Tokens, der Rahmen auf denselben `line`-Token wie die Statuszeilen-Trennungen; damit werden die beiden Fensterkanten buchstäblich dieselbe Farbe, und Verschieben, Andocken, Systemmenü, Mehrschirm-DPI und Barrierefreiheit bleiben, wie Windows sie macht. Vor der Empfehlung wird gemessen, ob die Attribute auf dieser Windows-Fassung greifen (ein stiller DWM-Fehlschlag ist „gesetzt heißt nicht gezeigt" in Reinform; Mica braucht transparente Flächen und bleibt an opakem Qt wirkungslos) — dazu der Fall „Akzentfarbe auf Titelleisten" in den Windows-Einstellungen: ob `CAPTION_COLOR` die Kundenwahl überschreibt, wird gemessen und als bewusste Entscheidung dokumentiert, nicht still hingenommen. **Stufe 2 ist zu (`83f82b66`, reviewt)**: `window_chrome.py` sagt Windows die Farbe von Titelleiste und Titeltext (DWM-Attribute 35/36, ab Build 22000 — vorher gefragt statt hinterher geprüft), gelesen aus der **Palette**, nicht aus einem Themennamen; ein Wächter am Ereignisstrom (`Show` + `ApplicationPaletteChange`) statt neunzehn Aufrufen, `action_theme` braucht keine Zeile. Mica und `DWMWA_BORDER_COLOR` gemessen wirkungslos und deshalb bewusst **nicht** gesetzt — ein Aufruf, der nichts bewirkt, sieht in einem Jahr aus wie einer, der etwas bewirkt. Messbeleg: beide Themen exakt auf dem `window`-Token (11292 von 11377 Stichproben verändert); die Frage „Akzentfarbe auf Titelleisten" ist damit beantwortet — wir setzen die Anwendungsfarbe bewusst. Freigabe: Diff vollständig gelesen, 4/4 Tests in eigener Stichprobe grün; eigene Sonde fand eine Redundanz, kein Hindernis — der globale Filter sieht den Palettenwechsel je Empfänger (6 Feuerungen bei 2 Fenstern + 20 Kindern), `paint_every_window` läuft also mehrfach je Themenwechsel; idempotent, selten, an d3 gemeldet — **und von d3 behoben (`d99f12bc`)**: Der Filter malt nur noch am Anwendungsobjekt, `dwmapi` lädt einmal statt je Fenster, der Test zählt die Feuerungen mit. Neu für die Prüfstand-Sammlung: `DwmGetWindowAttribute` kann die Farbe **nicht lesen** (`E_INVALIDARG`) — sie ist schreibbar, nicht lesbar | M | **zu — d3, reviewt** |
| G18 | **Die Einheit des geteilten Suite-Laufs ist zu grob** (72, gemessen am 30.08.): `suite-getrennt.sh` fährt eine Fensterdatei als einen Prozess — `test_ui.py` reißt aber **innerhalb**, bei rund 124 von 372, und das Skript kann für diese Datei kein vollständiges Ergebnis liefern. Die Portionsgröße ist gemessen: 60 Tests je Prozess kommen durch (sieben Portionen, siebenmal Exit 0, 372 bestanden), 372 am Stück nicht. Das Skript lernt Portionierung über `--collect-only -q` für Dateien ab einer Schwelle — und der Gesamtlauf-Riss ist damit zugleich als Abbau-Riss belegt statt erzählt. **Zu (`5518b0bc`, reviewt):** Portionen zu 60 (`SUITE_PORTION`, „kein Naturgesetz, sondern die Zahl, die gemessen durchkommt"), Testnamen als Feld statt Zeichenkette (eckige Klammern parametrisierter Namen sind der Shell ein Dateimuster — zwei Portionen Exit 4, gemessen und behoben), CRLF vom collect-only abgestreift, und der **Fortschrittszeichen-Leser** als zweite Hälfte: Fehlt die Schlusszeile, entscheiden die Zeichen — aber nur vollzählig gegen die Soll-Größe (die Freigabe-Frage „Riss nach Test 30 von 60?" fand die Lücke, das achte gefälschte Protokoll fand den Kommentar, der das Gegenteil des Codes behauptete; zwölf Zweige, alle geprüft; `s`/`x` gelaufen und grün, `F`/`E`/`X` rot). Kern-Beleg: `test_ui.py` vorher Exit 139 ohne ein Wort, jetzt 7 Portionen, 372/372, ein Abbau-Riss (Teil 5, Exit 127 bei 60 grünen Punkten) korrekt grün. Sammelgruppe beide Läufe vollständig (5256/5259). Die sieben Fehlläufe des zweiten Laufs gehören belegt fremder laufender Arbeit; einer davon war echt und ist jetzt ein eigener Punkt: test_wording (siehe W1-Zeile). **Nachtrag aus 72s Basisrate:** Die 60 ist dateispezifisch — `test_print_settings_ui.py` reißt schon bei 40 (139 mitten im Lauf), sicher waren dort 24–34. Anschluss bei d3, Empfehlung der Freigabe: selbsttätige Halbierung nur bei einem NICHT-GELAUFEN-Riss statt einer Konstantenliste. **Der Anschluss ist zu (`7658e2d3`, 72, reviewt):** Eine Warteschlange statt fester Schrittweite — reißt eine Portion so, dass Tests nie liefen (`nicht_gelaufen` trennt den Riss vom roten Test an Schlusszeile, roten Zeichen und Zeichenzahl), kommen ihre Hälften vorn wieder hinein, bis zum Boden `SUITE_MIN_PORTION` (vier — darunter ist ein Riss ein Befund, den jemand ansieht). Ein Abbau-Riss teilt nicht (jeder Test lief), ein roter Test teilt nicht (er bliebe in jeder Hälfte rot). Bleibender Prüfstand `tests/test_suite_script.py` mit gefälschten Protokollen und einem gefälschten pytest, das über einer Grenze reißt; `SUITE_NUR_FUNKTIONEN` macht die Funktionen ohne Suite aufrufbar. Vier Gegenproben — die vierte fand die eigentliche Lücke: Der Boden gegen die Endlos-Schleife war ungeprüft, und ein Testfall, der sein Urteil doppelt trug, ließ die Zeilenprüfung ungeprüft. Freigabe: Diff gelesen, Prüfstand in eigenem Lauf 4/4 grün. Für die Runde: Kleinere Portionen als sechzig sind ab jetzt Automatik, kein Fehler | S | **zu — d3+72, reviewt** |
| W1 | **Roberts Order (30.08.2026): Die ganze Website bekommt ein moderneres, innovativeres, optimiertes Design** — Ziel wörtlich: Kunden sollen das „das will ich haben"-Gefühl bekommen und begeistert sein. Dreistufig wie die App-Durchsicht: (1) **Recherche läuft** (Referenz-Muster bei Bambu/Prusa/Orca/Elegoo, den Begehrlichkeits-Vorbildern Linear/Raycast/Arc/Figma und den 3D-Tools Shapr3D/Plasticity — 8–12 übertragbare Muster mit Wirkbegründung) plus Bestandsaufnahme der eigenen Seiten gegen diese Muster (W-Befunde); (2) Design-Entwurf mit Review — verträglich mit der Werkzeugkette (erzeugte Bilder, stamp_assets, sechs Sprachen); (3) Umsetzung in Paketen. **Die T1-Galerie folgt dem neuen Stil** — 50s Tauschbörsen-Notiz und der W1-Entwurf stimmen sich ab, damit die Börse nicht im alten Kleid gebaut wird | L | **Befundlage steht** (`konzepte/durchsicht-website-2026-08.md`): zwölf Referenz-Muster, zwölf Befunde (vier hoch: überladener Hero mit Spende vor der Begehrlichkeit, kein bewegtes Produkt, null fremde Stimmen, keine Ergebnis-Fotos der Kernwege) — und die Stärken bleiben benannt (die Headline schlägt die Referenzen, die Schmerzpunkt-Sektion hat kein Vorbild, die Preisklarheit erfüllt M3 schon). **Der Dreiklang als Paketschnitt:** WD1 das Produkt in Bewegung (Hero-Loop plus je Weg einer; `make_video.py` liefert schon, Vorarbeit: stamp_assets lernt mp4/webm, Loops 720p/5–15 s wegen des 1,8-MB/s-Uploads), WD2 den Hero auf eine Botschaft entrümpeln (Spende in eigenen Abschnitt, ein CTA-Ziel überall), WD3 Beweis durch Ergebnisse und Stimmen (Fotos gedruckter Teile, Vorher/Nachher für Weg 1, Presse-Rückläufe). Sekundär Schrift und verlinkte Zahlen. **Eine Geschmacksfrage an Robert: hell bleibt Default oder dunkle Kino-Bühne für die Produktzonen (WB10)** | L | **UPLOAD-SPERRE (Robert, 30.08.2026, via 15): „seiten dann noch nicht hochladen" — gebaut und committet wird weiter, aber `upload_website.py` läuft nicht, bis Robert ausdrücklich freigibt; solidon3d.de bleibt auf dem Stand von heute Mittag. Der Upload liegt bei der Freigabe-Sitzung, die Sperre ist an alle gestreut.** Verteilt (Roberts „wer macht das? sauber abarbeiten"): WD1 (Produkt in Bewegung — Hero-Loop plus je Weg einer, Vorarbeit stamp_assets lernt mp4/webm) bei **3a**, der Aufnahme-Hand des Tages — sie liefert mit den ersten Loops auch die dunkle Hero-Bildvariante als Entscheidungsvorlage für WB10, damit Robert am Bild entscheidet statt am Begriff; WD2 (Hero auf eine Botschaft entrümpeln, ein CTA-Ziel, ×6 Sprachen) bei **15**, der Text- und Strukturhand; WD3 (Beweis-Sektion: Rahmen für Ergebnis-Galerie, Vorher/Nachher Weg 1, Stimmen-Band aus count.php und Presse-Rückläufen) bei **50**, im Verbund mit ihrer Börsen-Galerie — **mit einer Auflage an Robert: 2–4 Fotos real gedruckter Teile**, die kann nur er liefern. Je Paket Review, Reihenfolge der Landungen frei. **WD2 ist zu (`5a358858`, reviewt):** Der Hero trägt eine Botschaft — Kicker, H1, Lead, ein Knopf, die sechs Zusagen als eine Zeile; Download als eigener Abschnitt darunter, Countdown zur Preis-Wahrheit, alle CTAs auf `#download`, der Hero-Knopf an der Release-Mechanik (`data-release-href`/`-download` gegen site.js geprüft). **Robert korrigierte die Spende-Position live** („sollte die spende aber nicht relativ weit oben sein" — gemeint: nicht zu weit *unten*): von 75 % Seitenhöhe hinter dem Preis auf 41 % unmittelbar nach den vier Wegen, in allen sechs Sprachen. Freigabe-Nachmessung: alle sechs Fassungen tragen dieselbe Abschnittsfolge, 48/48 `div`, 273 `test_website` in eigenem Lauf grün. Zwei Tests prüfen jetzt die Zusage statt der Bauart (Handlung vor Bild, Geld zuletzt), dazu der Zonen-Wächter: kein Spendenblock im Aufmacher. 15s Massenumbau-Fund: ein fehlendes `</div>` sah im Browser völlig heil aus (still repariert) — gefunden nur durch Tag-Zählung an *einer* Sprache, bevor die anderen vier liefen; ein Tag-Balance-Wächter in `test_website` ist als kleiner Punkt sinnvoll (15). Ein Rest an 15 gemeldet: der CSS-Kommentar zu `#unterstuetzen` sagt noch „nach dem Preis". **WD3, erstes Stück gelandet (`1f052e4a`, reviewt):** das Vorher/Nachher-Paar (M10) als SVG über `drawing.project` — ohne Qt, ohne Fenster, auf jedem Rechner dasselbe; entsteht einmal statt sechsmal, weil das Bild keinen Text trägt. Stoff ist `weg3-generiert-aufbereiten` (einziges Beispiel mit echt offenem Netz, 3372 Dreiecke; das Weg-1-Beispiel ist beim Einlesen schon geschlossen, die Korpus-Krüppel überzeugen niemanden). **Frage an Robert dazu:** Das Paar zeigt Weg 3 statt Weg 1 — bleibt das so, oder steuert er ein wirklich kaputtes Fremdmodell bei? **Zwei Baustein-Landungen von 50 (reviewt): der Lochwandhalter-Befund (`3cf05010`)** — `keeps_up` richtet nur entlang einer gewählten Fläche auf; wer Koordinaten tippt, behält Achse Z, und um Z kommt ein -Y nie nach oben (hinter der Wand standen 0,8 mm statt 3,7, jede Prüfung war grün). Nicht repariert, sondern **gesagt**: Warnbefund `parts.up_points_nowhere` mit beiden Wegen im Satz (Regel 21 — ein Haken, der nichts hält, ist die stillste aller Antworten); Roberts Bild-Fund war der Anlass. **Und drei Galerieteile (`73e3060d`)**: Klappbox, Lochwandhalter repariert, Schraubdose auf ⌀75 mit Deckel daneben — dazu reisen die drei `.p3d` nach `website/teile/` (Rezepte, unter 1 KB). **Dazu ein Baustein-Befund (3a beim Nachmessen der Galerie, 30.08. spät): `screw_lid` hat eine stille Untergrenze um 66,8 mm** — bei ⌀50 und ⌀60 ist der Deckel konstant 66,8, erst ⌀75 wächst mit (80,2). **Geklärt (d3, auf die Kommastelle reproduziert): `screw_lid` ist unschuldig, die 66,8 ist eine gealterte abgelesene Zahl im Galerie-Rezept** — `apply_texture` trägt `wrap_diameter=65.3` fest eingetragen (einmal bei ⌀60 abgelesen; `width=205.1` ist π×65,3, der Beweis), und bei jeder kleineren Dose wickelt die Rändelung um einen zu großen Zylinder und ragt bis 21,5 mm hinaus. Der Baustein wächst an drei unabhängigen Wegen konstant mit (+5,3, ⌀30–⌀120), `neck` war nie gesetzt; Korrektur zur Freigabe-Angabe: die Datei trug immer ⌀75 — **3a hat beim Messen den Parameter verstellt und damit den Kunden simuliert**, und genau das ist der Wert der Messung: Ein öffentliches Rezept verspricht Parametrik und liefert beim Verstellen stillen Ausschuss. **Beschlossen: Weg 3** — `apply_texture` meldet künftig, mit 50s gemessenem Verstärker: Der ⌀75-Deckel bestand aus **553 Komponenten** (die Rändelung als lose Stücke, auf dem Bild unsichtbar — „was fehlte, sah aus wie eine Gestaltungsentscheidung"), und **die Teilezahl lügt nicht** — der `_hanging_loose`-Prüfsatz fehlt bei den Texturen; d3 baut mit Teilezahl als primärem Kriterium. **Die Rezept-Reparatur ist zu (`e563c6b7`, 50, reviewt — 300/300):** beide Zahlen als Formel aus der **Bauart** (`@durchmesser + 2*@wand + 0,5`, width mal π), geprüft ⌀50–120 durchgehend ein Körper, Bild auf der Seite angesehen, Stempel ×6. `screw_lid` ist **vollständig** entlastet (fünf Größen, konstant +5,30) — die 66,8 bei zwei Größen war der Hüllquader der festen Wicklung, und 3as Lesart war der Normalfall: **Wer einen Baustein über ein fertiges Rezept misst, misst das Rezept mit.** **Die Familie heißt jetzt richtig (50): nicht „drei Bausteine mit stillen Grenzen", sondern *eine feste Zahl oder Vorgabe, die nur in der Lage stimmte, in der sie entstand*** — Haken, Klappbox, Rändelung; keiner an einer Kennzahl zu sehen, alle drei am Bild; **Die Familie hat jetzt ihren Wächter (`90f093c6`, 50, reviewt — 305/305):** Ein Test dreht an den Parametern der Galerieteile und prüft die Komponentenzahl — und fand sofort zwei Fälle: 50s eigene Reparatur von vorhin war zur Hälfte Materialprofil (die 0,5 war `2*clearance` bei PETG; der Standard fährt PLA, dort zerfiel der Deckel in 542 Stücke — **eine Zahl aus dem Profil in einer Formel ist derselbe Fehler wie eine aus der Geometrie, nur besser getarnt**); jetzt steht das Spiel gar nicht mehr in der Formel, die Rändelung wickelt bewusst zu eng (die sichere Richtung: verschmilzt statt abzufallen), ein Körper über ⌀50–120 und beide Materialien. Und **das Gehäuse zerfällt unter 110 mm Breite** (feste Buchsen-Positionen fallen aus der schmaler werdenden Wand — 5 Stücke bei 90) — der Test deckt ehrlich nur ab 110 und nennt die Messung, der Fix liegt bei 3a. Dazu die Umlaut-Erinnerung fortgeschrieben (`a699528b`): Drei Commit-Meldungen des Abends trugen ae/ue — alle drei über `printf`, die guten über eine Write-Datei; die Notiz hieß „Heredoc kann Umlaute" und wurde als Heredoc-Aussage gelesen statt als Text-Anweisung; der Prüfbericht meldet „erzeugt, beide mit derselben Steigung", die Auswertung ist vollständig, und der Kunde druckt Ausschuss. **Der Fehler ist das Schweigen, nicht zwingend die Grenze**: Entweder die Grenze ist echt, dann gehört sie ins Parameterschema (der Dialog verhindert ⌀50), oder sie ist falsch verdrahtet, dann skaliert der Deckel mit — 50 misst am Baustein und entscheidet; parts_version steigt **nur bei Maßänderung** (3as Präzisierung — ist die Grenze fachlich echt, ist der Fix ein Befund und kein Versions-Fall). Fix bei der **nächsten freien Hand nach den Börsen-Strecken** — 50 baut ihn ausdrücklich nicht selbst, alle Hände sind an der Börse, und die Galerie steht mit der ⌀75er-Dose nicht mehr darauf **Zwei Befunde aus 3as Schaustück-Prüfung (30.08. spät, Landung folgt):** (1) `judge().support_volume` — **die Deutung trug nicht, die Zahl ist absolut richtig** (d3s Pilz-Gegenmessung: handrechenbarer Sollwert 30 000 mm³, gemessen 29 986,7, Abweichung 0,044 % = der benannte Restterm; Quader→0 und ohne-Achsen-mehr sind die **Definition** des gestützten Raums — gefüllter Raum braucht keine Stütze). 3as Messung war richtig gemessen (Quader 100×100×50: 0,0; der Halter **ohne** Achsen meldet mehr als mit): Die Zahl verhält sich wie „Raum zwischen Körper und Hüllform", nur ihre Lesart nicht. **Der Einschränkungs-Satz der Freigabe ist gestrichen — er wäre falsch gewesen, und der Nächste hätte um eine korrekte Zahl herumgerechnet.** Der echte Kern ist kleiner: Der Name legt Stütz*material* nahe, die Zahl ist der gestützte *Raum*, und der Code schichtet sauber (`estimate.support_material()` multipliziert mit der Dichte); d3 trägt den Zwei-Zeilen-Satz an `Candidate.support_volume` ein, der Name bleibt (Slicer sagen es genauso, vor der Dichte) — **beides ist zu (`b81e9ee0` und `d64977b2`, reviewt, 19/19)**: Der apply_texture-Befund läuft mit **Teilezahl als primärem Kriterium**, und die Tabelle belegt die Richtungs-Blindheit (bei ⌀75 ist der Wickel *kleiner* als der Körper — die Durchmesserprüfung schweigt zu Recht, und trotzdem liegen 553 Stücke daneben; „eine Prüfung, die nur eine Richtung kennt, ist genau dann still, wenn der Fehler von der anderen kommt"). Zwei Prüfstand-Lehren dabei: Ein grob aufgelöster Zylinder ist ein **Vielflach** (Flächenmitten innerhalb des Nenndurchmessers — ein exakt passender Wickel schwebt; der Test erzeugte seinen eigenen Zerfall), und die Musterbreite ist der **Umfang**, keine freie Zahl. Die Mutationsprobe fand einen Test, der nichts prüfte (die Lage „Durchbruch" ist über die Operation nicht herstellbar) — er prüft jetzt die Funktion direkt. Damit: drei echte Fälle der Familie plus eine **widerlegte Deutung** — auch das gehört zur Ehrlichkeit der Liste. (2) **`advise` hat recht behalten und das Schaustück verbessert:** Die eingewachsenen Achsen waren eine 68-mm-Brücke (306 mm² Überhang auf einer Schicht); jetzt **gesteckt** in Lageraugen — Überhang 978,8→179,4 mm², drei Körper zweifarbig, und das Teil zeigt damit eine **Passung**, was ein Einzelkörper nie kann. Die eigene Analyse hat das eigene Schaustück druckbar gemacht — der beste Beleg der Nacht dafür, dass die Beratung trägt. Dazu `hide_axis_marker` neben `hide_orientation_widget` (das Achsenkreuz hängt am Renderer, `plotter.axes_widget` gibt es in pyvista 0.48 nicht und liefert still None). **WD1 ist zu** — der Aufmacher steht (`42e6303d` Rollenhalter als zweites Schaustück samt `website/teile/rollenhalter.p3d`; `9306ed26` die Drehbühne: Sprite-Sheet, Drehung über die CSS-Zeitachse ohne eine Zeile JavaScript beim Scrollen — `scroll()` statt `view()`, damit die Drehung nicht vorbei ist, bevor jemand scrollt; `steps(35, jump-none)` für 36 Aufnahmen; `touch-action: pan-y`, damit ein Telefon dreht statt wegzuwischen; Regler und Ziehgeste in site.js). **Roberts Zoom-Befund ist erledigt**: Das Teil wächst am Regler von 93 auf 128 mm, statt dass die Kamera zurückweicht. Reviewt, ein Kommentar-Nachzug bei 3a (AXLE_GAP=32 gegen „vierundsechzig" im Docstring — die Zahl und ihr Satz müssen dasselbe sagen). Der Hero-Block für index.html liegt bei 50 zum Einsetzen. **Der Stempellauf gehört der Freigabe** (bestätigt): 76 Verweise sind schal, mehrere Urheber, und `stamp_assets` fasst jede HTML an — er läuft als letzter Schritt, wenn 50 und 15 die Seiten freigeben, der Upload ist ohnehin gesperrt. **WD1, großer Schritt (`82dd26e5`, reviewt): Robert hat die Richtung selbst geschärft** („was spannenderes aufwendigeres als video", „wo man auch mehr sieht", „ein aufwendigeres modell") — daraus wurden `tools/make_showpiece.py` (ein Gehäuse mit 79 134 Dreiecken in zwölf Schritten über die Operations-API, Deckel **danebengelegt**, weil ein Schaustück, das seine Arbeit versteckt, eine Kiste zeigt) und `shoot_turntable` (36 Bilder je Umdrehung als **ziehbare Reihe** statt Video — ein Video läuft, eine Reihe gehorcht; 0,20 MB, kein Fremdskript). Dazu `encode_loop` komplett (stumm wegen Autoplay, faststart, poster aus Bild 0, CRF-Konstanten mit Messbegründung) und die Szenen-Wache in `shoot_loop` (der erste echte Loop filmte den leeren Startbildschirm bei grünen Maschinerie-Kontrollen — die Wache sitzt jetzt in der Funktion). Freigabe-Stichprobe 27 Tests grün. **Urheberschafts-Korrektur, von 15 nachgemessen:** `test_every_page_closes_the_tags_it_opens` (der Tag-Balance-Wächter) ist **15s Arbeit** — er lag ungestaged im Baum und ritt in 3as Commit mit. Das ist die Kehrseite der bekannten Falle und ein eigenes Muster: **Die eigene Arbeit landet im fremden Commit, und die Urheberschaft verschiebt sich still** — auffällig nur am Docstring. Der Wächter selbst ist geprüft (15s Gegenprobe: entferntes `</div>` macht ihn rot), der Punkt ist damit zu. **Der Aufmacher ist entschieden (3a, an der Quelle bestätigt): ein Rollenbock für Filamentrollen** — 18 Operationen, Regler zieht die Rollenbreite 55–90 mm, Roberts Zoom-Befund erledigt (Kamera einmal eingepasst, das Teil wächst 93→128 mm statt Perspektivwechsel). Die Klappbox fiel an zwei gemessenen Argumenten: **„Der Betrachter hat das Gegenstück schon"** (15 — wer die Seite ansieht, hat eine Rolle neben sich; bei der Box muss er sich erst etwas überlegen) und dem Turntable-Winkel (50 — ein Filmscharnier ist von schräg oben ein Strich, ein Beweisstück, das in zwei Dritteln der Drehung verschwindet, beweist zwei Drittel der Zeit nichts). Dazu 3as Regel aus dem Vergleich mit 15s Kerndurchmesser-Plädoyer, verbindlich für alle Regler-Motive: **Im Aufmacher gewinnt das sichtbare Maß gegen das zwingendere** — ein Regler, dessen Wirkung man nicht sieht, beweist nichts. **Das Turntable-Web-Stück ist zu (`7c7f3d95`, reviewt):** 112 Zeilen in `site.js`, ohne fremden Code — ohne Skript bleibt das erste Bild stehen, bei `prefers-reduced-motion` wird nichts vorgeladen und nichts getauscht (auch nicht auf Geste), vorgeladen wird erst beim ersten Anfassen, Zeiger-Capture hält die Drehung über den Rand hinaus, Pfeiltasten für die Tastatur; im echten Browser belegt (240-Punkte-Zug dreht auf Bild 20, Konsole leer). Das HTML-Fragment samt sechs Sprachen liegt bei 50 — **Auflage an deren Landung: die fokussierbare Fläche braucht Rolle und Beschriftung für den Bildschirmleser**, das Skript liefert nur die Bewegung. Offener Stempel-Hinweis: 36 `style.css`-Stempel altern im Baum, solange 15 daran arbeitet — `stamp_assets` läuft ohnehin als Letztes vor dem (gesperrten) Upload. **Dazu ein echtes Rot, gemessen von der Freigabe (30.08. spät): `test_wording` 6× rot** — die erzeugten Handbuchseiten (`website/<sprache>/manual.html`) sind gegenüber den Katalog- und Register-Änderungen des Abends (Z7a-Bedingungsarten, D3/D5-Texte, Katalog-Umbau `54d2db34`) veraltet; kein Codefehler, ein Erzeugnis-Rückstand. **Behoben:** Der make_manual-Lauf ist gefahren (30.08., 23:14, sechs Sprachen + PDFs), `test_wording` ist **15/15 grün** — der Stempellauf liegt bei 15 und läuft jetzt, alle Hände sind aus website/ raus. **WD3 ist zu (`8835a6bd`, 50, reviewt):** Beweis-Sektion in sechs Sprachen (drei Teile mit je einem Merkmal, das kein Würfel-Editor hergibt; ehrliche Bildunterschriften), Vorher/Nachher, 3as Drehbühne eingesetzt (role/aria-label/tabindex, Regler als natives `input[type=range]` mit label). Die Klappbox flog aus der Galerie — **senkrecht durchgesteckter Deckel, dasselbe Koordinatenproblem wie der Lochwandhalter, diesmal an `insert_living_hinge`**: Das ist das zweite Geschwister der keeps_up-Familie und wandert zum screw_lid-Sammelpunkt (stille Lagen und Grenzen der Bausteine, drei Fälle, nächste freie Hand). **Das Stimmen-Band ist offen mit dem Vermerk „Material fehlt", nicht ungebaut:** Es gibt null echte Stimmen (Presse-Rückläufe existieren nicht, count.php liefert Zahlen), und 50 erfindet keine — auch nicht als Platzhalter; ein Zitat, das niemand gesagt hat, wäre eine Fälschung in sechs Sprachen. **Entscheid der Freigabe: Weg 2 jetzt** (ehrliches Zahlen-Band aus count.php — Downloads, Sprachen, Systeme, ehrlich beschriftet), **Weg 1 als Frage an Robert** (gibt er ein Dental-Kundenzitat frei? ein echter Satz schlägt fünf gebastelte und ersetzt dann das Band als Antwort auf WB3). **50s Bild-Regel übernommen:** Ein Bild, das auf eine Seite kommt, wird auf der Seite angesehen — nicht in der Vorschau, nicht einzeln; vier Fehler waren an keiner Zahl zu sehen (Gizmo im Bild, reset_camera nach Gizmo-Abschaltung neu kalibriert, 288-Punkte-Kachel). **Dazu 72s Anschluss-Dreier (`bdc302de`/`ba4061c0`/`d29ea578`, reviewt):** `shared.for_upload(recipe)` erzeugt und prüft untrennbar (inspect hatte gemessen null Aufrufer — die Kette endete ein Glied vor der Tür), Gegenproben blind gepatcht 5/5 und 4/4; das main-Rot aus `c36627e9` (deutscher values-Schlüssel `grund`) ist behoben samt neuem Wächter am Ort der Schlüsselerhebung — `test_language_rules` konnte ihn nie sehen, ein Wörterbuchschlüssel ist kein Bezeichner; shared-rules.json trägt jetzt `licenses` aus d3s Kern-Konstante. **Ein main-Rot bleibt, entschieden als Fehlalarm:** `test_no_number_reaches_the_user_past_the_localisation[panels.py]` trifft zwei f-Strings in einer **Breitenmessung** (`horizontalAdvance` — die Zahl erreicht nie den Kunden, `localised()` wäre sachlich falsch); der Wächter bekommt eine **eng gefasste Metrik-Ausnahme** (nur Zeilen, deren Ergebnis in eine Metrik fließt) samt Gegenprobe, dass echter Nutzer-Text verboten bleibt — bei **15**, zusammen mit dem Stempellauf und dem §4.2-Nachzug (`license` ist Feld- und Dateischlüssel, `licence` nur noch Parameter/Bestand) |
| T1 | **Roberts Idee (30.08.2026): Bausteine exportieren und auf der Website tauschen** — Kunden laden eigene Bausteine hoch, andere importieren sie. Die Grundlage existiert: Ein eigener Baustein ist ein **Rezept** (Namen registrierter Operationen plus Werte, führt nichts aus — Sicherheitslage identisch mit der Projektdatei, Roberts Entscheidung vom 24.08.), Rezepte reisen heute schon in Projektdateien mit, und „lokal schlägt mitgereist" ist gebaute Regel. **Neu wären zwei Teile:** (a) Rezept als eigenständige Datei exportieren/importieren (App, klein — der Ladeweg existiert in `adopt_payload`); (b) die Tauschbörse auf der Website — **und Robert hat nachgelegt (30.08.2026): „auch die webseite dann dafür gestalten"** — der Bereich wird ein gestaltetes Stück der Website, kein Formular: Galerie im Stil der bestehenden Seiten, Kacheln mit Vorschaubild je Baustein (die Rezept-Datei kann das gerenderte Vorschaubild mitführen — der Katalog rendert es ohnehin), Kategorien und Suche wie im Katalog der App, damit beide Orte dieselbe Sprache sprechen; Upload/Download als PHP nach dem support.php-Muster. **Und Robert hat erweitert (30.08.2026): mit Liken und Kommentieren** — das hebt die Community-Fragen der Notiz von wichtig auf zwingend: Likes ohne jede Identität sind beliebig manipulierbar (die leichteste tragfähige Hürde ist die Frage), Kommentare sind nutzergeschriebener Text und brauchen Melden-Weg und Moderation vom ersten Tag, nicht als Nachrüstung. **Konzeptnotiz vor jedem Bau**, mit den vier harten Fragen: serverseitige Format-Validierung (nur Rezepte, nie Code — die Prüfung muss auf dem Server stehen, nicht nur im Client), Moderation und Missbrauch (Sichtung vor Veröffentlichung? Melden-Weg?), Lizenz der Uploads (der Hochladende wählt ausdrücklich, sonst ist jeder Download eine Rechtsfrage), und die Konto-Frage (Upload ohne Hürde ist ein Spam-Tor — die App selbst bleibt konto- und netzfrei, getauscht wird im Browser, importiert per Datei). Abgrenzung: kein Plugin-System, keine Cloud-Ablage von Projekten — genau deshalb Rezepte | L | **BAUAUFTRAG (Robert, 30.08. 22:1x, „gründlich nach allen anforderungen"): Die Börse wird komplett gebaut, beide Hälften, Ziel 0.2.3.** Alle fünf Fragen sind beantwortet (§7 des Konzepts) — die Formatprüfung auf dem Server ist die erste Instanz, wer baut, fängt bei ihr an. Verteilung: **50: Die Server-Formatprüfung steht (`453c3369`/`effe8597`/`9b522aa6`, mit 72, reviewt)** — `shared_common.php` spiegelt `parts/shared.py` Befund für Befund, beide lesen shared-rules.json, und weil eine gemeinsame Regelliste zwei Prüfer noch nicht gleich macht, läuft der **Urteils-Vergleich**: 29 Nutzlasten durch beide Prüfungen plus 72s 21 Grenzfall-Dateien (eingecheckt, weil drei Maschinen arbeiten; der 26-MB-Fall wird gebaut statt gelagert; Titel exakt auf der 120er-Grenze und Umlaute fangen Byte-statt-Zeichen-Zähler), dreifach verglichen (Kern↔PHP wörtlich, beide gegen das eingecheckte Urteil — nur Entscheidung und Gründezahl, denn Texte dürfen sich ändern, die Zusage nicht; zwei Fehler nennen beide Gründe auf einmal). Freigabe-Stichprobe: 88 Tests grün, mit echtem lokalem PHP. **Die vier Kern-Endpunkte stehen (`7d2f011a`, 50, reviewt):** upload/confirm/list/download in `shared.php` über `?do=` (eine statische SQLite-Verbindung statt drei), Ablage in `shared_store.php` **außerhalb httpdocs** (nichts ist vor der Bestätigung über eine geratene URL abrufbar), Adressen nur als `sha256(seed|adresse)`, und ohne gesetzten `SOLIDON_SHARED_SEED` verweigert die Börse den Dienst statt mit wechselndem Startwert zu hashen. Freigabe-Stichprobe 5/5 mit echtem PHP. **Dabei zwei Funde von 50/72:** Der zugesagte **Rückzieh-Link existierte nirgends** (beide Seiten für sich stimmig — die Sorte Lücke, die keine Prüfung findet); `withdraw_key` zieht in beide Tabellen ein, `?do=withdraw` wird ein Endpunkt für beide, 72 verschickt den Schlüssel in der Mail. Und die Sieben-Tage-Frist läuft als **Konstante plus Abfrage** statt als Feld — die Freigabe-Auflage ist zurückgenommen, ihre Begründung war schwächer: ein Feld, das aus zwei vorhandenen ableitbar ist, kann als Einziges falsch werden. **Eine Betreiber-Frage liegt bei Robert, die Nacht baut derweil auf Weg 3+1:** `datenschutz.html` sagt zu, den Kunden „bei einer Beschwerde zu erreichen" — gespeichert wird aber nur der Hash, **aus einem Hash erreicht man niemanden** (der Code ist datensparsamer als der Text; falsch ist der Satz, nicht die Speicherung). **Eingelöst (`e66b8a81`, 15, reviewt — 300/300):** Es waren vier falsche Sätze, nicht einer — der Text versprach zu viel *Speicherung* und machte die Verarbeitung schlechter, als sie ist; jetzt sagt er die datensparsame Wahrheit („aus dem Prüfwert lässt sie sich nicht zurückrechnen, und niemand kann Sie darüber anschreiben — auch wir nicht"), der Rückzieh-Link trägt Gewicht samt Aufbewahrungs-Bitte, und der Sieben-Tage-Satz löscht ehrlich einen **Vorgang** statt einer Adresse. **Dabei gefunden (15) und als Muss-vor-Upload bei 72: Der Sieben-Tage-Verfall existiert im Code nicht** — Spalten ja, kein DELETE, keine Frist; eine öffentliche Löschzusage ist eine Zusage, keine Beschreibung; Bauart-Vorschlag Aufräumen-beim-Zugriff. **Roberts Alternative, falls er bei Beschwerden nachfragen können will statt nur zu löschen:** Adresse zusätzlich verschlüsselt ablegen (Schlüssel neben dem Startwert außerhalb httpdocs) — eine halbe Stunde Umbau, jederzeit nachziehbar; die Frage gehört zu seiner Datenschutz-Leseauflage. **Like, Kommentar und der Verfall sind zu (`2a650885`, 72, reviewt — 12/12):** ein Like je Kennung und Teil (Eindeutigkeit im Index, zurück kommt die Zahl), Kommentar mit Mailbestätigung und `withdraw_key` in derselben Mail, Spalten aufgezählt statt `SELECT *` (ein Stern hätte fremde Rückziehschlüssel an jeden Leser gereicht), Verfall als Aufräumen-beim-Zugriff (48 h Bestätigungslink — „abgelaufen" statt „unbekannt" —, sieben Tage Datensatz, die Frist als Konstante mit Verweis auf datenschutz.html). **Der Fund dabei rettet jede künftige Betriebsdatenbank: `CREATE TABLE IF NOT EXISTS` ändert eine vorhandene Tabelle nicht** — der Prüfstand erzeugt seine DB je Lauf neu und konnte es nie sehen, im Betrieb ist sie per Definition alt; `shared_add_column` migriert protokollierend, und der Test läuft gegen ein **altes Schema als festen Text** statt aus der heutigen Funktion abgeleitet. **Das Zahlen-Band ist gestrichen — die dritte Freigabe-Entscheidung, die an einer Messung fiel (50):** Downloadzahlen sind bewusst nicht öffentlich (stats.php hinter Anmeldung, begründet — die Umkehr wäre Roberts Entscheidung), alles Übrige steht schon in der Zusagenzeile, **eine Zahl über uns selbst ist keine fremde Stimme**, und eine kleine Zahl schadet („312 Downloads" ist schlechter als kein Band). Stattdessen: Der Platz bleibt leer bis zur ersten echten Stimme (Dental-Zitat-Frage bei Robert), und unter die Galerie kommt 50s wahre Zeile — „Alle drei Rezepte liegen offen: Operation für Operation nachvollziehbar, jeder Schritt rücknehmbar" — die auf die Börse zeigt und nicht behauptet, eine Stimme zu sein. **Die Börsen-Seiten stehen in sechs Sprachen (`d2e47fb9`/`4135d0e0`/`51d8d7b7`/`0ee1b490`, 50, reviewt — 346 grün, nur die Stempel-Stelle wartet auf 15s Lauf):** boerse.html/exchange.html mit eigener JS, Gestaltung von 3a freigegeben, sitemap zieht mit, und **die Klappbox ist zurück in der Galerie** (d3s Eine-Zeile-Messung, Bild neu, auf der Seite angesehen). **Dritter Zeilen-Klau der Nacht, diesmal in beide Richtungen binnen zwanzig Minuten** (3a↔50 an test_website.py): Die Kettenregel zählt Dateien, und hier war die Datei richtig und die Zeile fremd — **Regel: Bei einer geteilten Datei wird vor dem Commit der eigene Block gegriffen, nicht nur die Dateizahl gezählt** (drei Fälle in einer Nacht sind kein Zufall). Dazu 50s Diff-Hygiene-Fund: Drei einzelne CR ohne LF blähten den Diff auf 1003/996 für eine Kachel — ein Diff, den niemand mehr liest, verdeckt beim nächsten Mal das Lesenswerte (repariert in 0ee1b490). **Der Server spricht die Kundensprache (`4c51a81a`, 15, reviewt):** 39 Börsentexte in sechs Sprachen, Mails eingeschlossen — und derselbe Commit war der **Rücknahme-Unfall der Nacht**: Er löschte d3s Textur-Arbeit (152 Zeilen + fünf Katalogschlüssel), obwohl 15 den Index **zweimal** frisch gebaut und den ersten Wettlauf sogar gefangen hatte; zwischen zweiter Prüfung und Commit lag nur das Schreiben der Commit-Meldung, und das genügte. Gefangen von der Nachmessung (Guard sechs, Numstat acht), repariert in `d7c83690` (byteweise Gegenprobe gegen den Stand vor dem Fehler; in eigener Stichprobe 163 Tests grün). **Die Regel wird schärfer: Bau, Prüfung und Commit in EINEM Zug, nichts dazwischen — die Commit-Meldung wird vorher geschrieben, nicht zwischen Guard und Commit.** Ihr KEIN_TOR war zweifach angesagt und begründet (72s make_shared_texts.py trug drei deutsche Bezeichner und machte test_language_rules am Baum rot — Sofort-Fix gelandet: `0bbb3748`, es waren **neun** Bezeichner, gefangen vom pre-commit an 72s eigenem Commit; Ursache ehrlich benannt — der Wechsel zwischen tests/ (deutsch erlaubt) und tools/ (englisch Pflicht) ist der Moment, in dem es passiert. Im selben Commit die PHP-Ladeseite (`shared_text()` nach lang-Parameter, Gestaltprüfung, Fallback **en**, Sprachliste aus der erzeugten Datei statt eigener Whitelist). **Der Zwischenstand ist zu (`db91493c`, 72, reviewt — 226 grün, der eine Rote ist ihr eigener Frische-Wächter über der laufenden Schnitt-Arbeit):** Alle 39 Sätze laufen über `shared_text()`, der Wächter prüft beide Richtungen ohne skip, übrig sind SQL und Header. Drei Lehren daraus: Ein Muster über einzelne Literale fand 18 von 39 (PHP verkettet — dieselbe Falle wie 15s Halbsatz und 72s Zählung, dritte Gestalt: **erst der zusammengesetzte Ausdruck zählt**); `sprintf`-Positionsplatzhalter binden die Zahlenreihenfolge an den deutschen Satzbau — benannte `{size}`/`{limit}` machen die Übersetzung umstellbar, ohne dass zwei plausible Zahlen still tauschen; und ihr Wächter erwischte sich selbst (suchte `shared_text(` und übersah den Umweg über `shared_mail_body` — eine Suche antwortet auf ihre eigene Frage). 50s zwei Bestandstests fordern jetzt ausdrücklich Deutsch an, mit Grund im Docstring. **Dazu der von 3a angestoßene Kennung-Wächter (`03942678`, reviewt):** Ein deutscher Oberflächentext ist zugleich die Message-ID — ein `ss` statt `ß` wird zur Kennung, und wer ihn später berichtigt, macht fünf Übersetzungen tot; vier ASCII-Fälle an einem Abend, alle aus Bash-Heredocs; die Probe an bekannten Fällen fing `grosses`, das die Liste (`gross`) durchgelassen hätte. **Der findings-Schnitt läuft** (72+50 stimmen ab, bevor geschrieben wird; von 15 nichts Neues nötig — die Befundtexte gehen in dieselbe Quelle, eine Übersetzungsrunde statt zwei). **Die Operationen-Messung ist abgeschlossen (`37a4ac02`, d3, reviewt):** Genau eine Lücke im ganzen Katalog — `label_text` zerfiel still, jetzt `label.fell_apart` mit parallelem Satz; alle anderen Anfüge-Ops gedeckt (Tabelle im Commit), `union_objects` bewusst befundfrei, denn zwei getrennte Teile zu einem Objekt zu vereinen **ist** der Zweck — ein Befund wäre die Warnung vor der eigenen Absicht. **Die Stempel-Null steht: test_website 347/347 (`e4bd17cf`, 15)** — 78 Verweise gestempelt, 42/42 Dateien ohne Inhaltsänderung nachgemessen, ein Board-Widerspruch per Messung statt per Auskunft aufgelöst; der Ein-Zug-Ablauf hielt zum zweiten Mal und **steht jetzt im liefern-Skill** (drei HEAD-Rennen einer Nacht als Beleg, Block-Grep für geteilte Dateien dabei). **Ein rotes Tor sucht seinen Urheber:** `tools/check_message.py` ungetrackt, kein Board, zweimal `ersatz` — Anfrage läuft bei 15/50. **Der Ausgabeweg der App ist zu (`99bb2463`, 3a, reviewt — 16/16):** „Veröffentlichen …" im Katalog, gesperrt mit **zwei Gründen statt einem** (eingebaut: keine Datei zum Weitergeben; mitgereist: die Lizenz kennt nur sein Autor — ein Satz für beide Lagen wäre in einer unwahr), Grund an drei Stellen, geprüft **vor** dem Ordner-Dialog, geschrieben nur über for_upload, beide Fehlerwege mit Handgriff. Der Importweg ist die zweite Hälfte und läuft. **Dazu 3as commit-msg-Wächter (`d337797f`):** Fünf Umlaut-Rückfälle an einem Abend sind keine Formulierungs-, sondern eine Prüfzeitpunkt-Frage — der Wächter schlug sofort zweimal gegen die eigene Urheberin (die Meldung zitierte die Falschschreibungen; das Werkzeug trug selbst zwei deutsche Bezeichner — **der Wächter gegen einen Sprachverstoß hielt die zweite Sprachregel nicht ein**, von ihr selbst behoben, bevor die Urheberfrage sie erreichte). Und der Regler schrieb sein Komma fest (`99758027`) — im englischen Fenster stand „55.0 mm" neben „90,0 mm", gefunden nur in der fremden Sprachfassung; das Trennzeichen kommt jetzt aus dem lang-Attribut. **Der Baum trägt ein frisches Hook-Rot aus 72s laufender Schnitt-Arbeit** (satz/wert in shared.py:114f, Zuruf läuft). **`app/images/` ist neu erzeugt (`50ee767b`, d3, reviewt — 54 Dateien, 6 Sprachen):** Veraltung belegt statt angenommen (Bilder 11:46, Frontlicht 19:41, Schatten 19:50 — acht Stunden; ein Paketbau hätte eine Anwendung ausgeliefert, die es nicht mehr gibt), der es-Lauf starb im eigenen Prozess nach 3 von 9 Bildern und wurde von der Zeitstempelprüfung gefangen, und die einzige Prüfung, die die Offscreen-Falle fängt, ist das **angesehene Bild** (Datum und Größe sähen dort gleich aus). **Dazu die Skill-Selbstkorrektur (`9f11d933`):** „Gemessen läuft eine einzelne Sprache sauber durch" stand direkt unter der Warnung vor Zahlen aus früheren Läufen — **und war selbst so eine Zahl, nur in Worten**; ersetzt durch Wiederholung-mit-Prüfung, gedoppelt in app/images/CLAUDE.md. **Die App-Hälfte der Börse ist komplett (`75936c82` + `ae68d125`, 3a, reviewt):** Beide Richtungen Ende zu Ende (for_upload 1040 Bytes → adopt 0 Befunde, Herkunft `published` statt `travelled`, Steckbrief nennt die Börse — die **dritte Herkunftszeile**, weil „kam mit einer Projektdatei" für eine Börsendatei die falsche Wahrheit an der §32-Stelle wäre; 15s Anstoß). Ein veröffentlichter Baustein lässt sich nicht weiterveröffentlichen. **Der Ende-zu-Ende-Lauf fand drei inspect-Lücken** (Name/Gruppe/Merkmal — for_upload gab saubere Bytes, adopt lehnte ab; 72 sofort gebaut, rules() leitet jetzt aus dem PartRegistry ab statt abzuschreiben): „Was durch inspect kommt, steht in der Galerie" — und gefunden hat es ein handgebautes Recipe, der Fall außerhalb des Vorstellungsraums, aus dem der Grenzfall-Korpus stammt. **Damit ist die Börse VOLLSTÄNDIG — der findings-Schnitt ist zu (`c59dbf03` 50 + `18e6ae5b` 72, reviewt):** Befunde tragen `code` und `values` statt fertiger Sätze, beide Seiten lesen dieselbe Textquelle, und 50s Mutationsprobe belegt die Bauart (`mb_strlen`→`strlen` bleibt rot, weil die **Zahlen** verglichen werden — zwei Seiten, die denselben Satz aus derselben JSON holen, stimmen immer überein; der Umbau hätte sonst die schärfste Zusicherung entfernt, die er verbessern sollte). **Der Nachschlag traf alle drei: Fünf Referenzrezepte trugen `group: "Befestigung"` statt des Schlüssels — formal sauber, beim Empfänger unbrauchbar, jahrelang grün.** 15s Satz dazu ist Regel: **Ein Test, der seine Voraussetzung im Namen trägt statt sie herzustellen, ist gegen den Fehler blind, den er zu decken vorgibt.** Und 50s Sammelform: **Anwesenheit, Form, Wortlaut — drei Arten, an der Sache vorbeizuprüfen** (drei Gebiete, eine Wurzel, eine Nacht). Betriebslehre dazu: Wer umbaut, sagt vorher, ob überhaupt noch eine Regel kommt — drei verbrannte Messrunden. Die Grenzfall-Sammlung steht bei 24 (19 abgewiesen, 5 gut), shared-texts bei 64 Sätzen ×6. **Die Bilder-Kette ist ebenfalls durch:** d3s Belegbilder (`16863125`, 30 Bilder + 12 img-Maße chirurgisch — das Bausteinband wuchs auf 955), meine Handbuch-Runde (`08063f5a`, 60 Dateien), 15s Stempellauf angestoßen. **Entscheid zur Warnung im Belegbild (d3s Fund — der Prüfbericht zeigt `parts.up_points_nowhere` auf der Verkaufsseite ×6): Sie bleibt.** Der Prüfbericht, der mitdenkt, IST das Produktversprechen — ein leerer wäre Deko; die ganze Seite lebt von Ehrlichkeit, und ein Beispiel, das eine echte Warnung samt verständlichem Satz zeigt, beweist mehr als jedes Feature-Versprechen. Robert kann es beim Ansehen kippen. d3s Selbstmeldung steht daneben: „ein Bild angesehen" stand im Commit **vor** dem Ansehen — zweites Mal das Muster „plausibler Satz statt Messung", diesmal selbst gefangen. **Die Null steht endgültig: test_website 347/347** — 15s Stempellauf (`058038c4`, 450 Verweise) trug im Baum bereits die neun Maß-Korrekturen der Freigabe mit (zwei Handbuch-Bilder wuchsen mit dem Tag: Katalog 836→900, Op-Dialog 195→179; aus den echten Dateien gerechnet statt getippt — die Kette brach deshalb korrekt mit „nichts zu committen" ab, die Arbeit war schon unter ihrem Commit gelandet). 15s Reihenfolge-Lehre dazu: **Ein Bilderlauf braucht zwei Nachläufe — den Stempellauf und die handgepflegten Maßangaben**; die zweite Hälfte findet nur, wer den Test nach dem Bilderlauf fährt. **BILANZ für Roberts Morgen (Stand 01:15):** Die Tauschbörse ist vollständig — Server (Upload/Bestätigung/Suche/Download/Like/Kommentar/Rückzieh-Link/Verfall/Migration), Seiten ×6, App-Aus- und Eingang mit dritter Herkunft, Formatprüfung beidseitig mit 24 Grenzfällen, Befunde als code+values aus einer Textquelle (64 Sätze ×6), Datenschutz ehrlich. Website komplett konsistent (Seiten, Handbuch, Bilder, Stempel — alles zeigt die App vom 31.08.). **Auf Robert warten:** Upload-Freigabe (Sperre gilt), WB10 hell/dunkel (Bilder bei ihm), Dental-Zitat, Datenschutz-Leseauflage samt Kontaktzusagen-Frage (Weg 2 = verschlüsselte Adresse, halbe Stunde nachziehbar). **Für 0.2.3 noch nötig:** changelog-Einträge (50, läuft), Handbuch-Seite zur Börse (15, läuft), Hauptweg-Test (3a, läuft), Version hoch, CI-Bau nach grüner Suite, Server-Deploy der api/-Dateien samt SOLIDON_SHARED_SEED. **Die Beispielprojekte sind durch (`0dd57b72`, d3, reviewt — 49/49):** und der Lauf fand einen **echten Kundenfehler im eigenen Beispiel**: `schild-zweifarbig` setzte sein Schlüsselloch mit `axis="z"` — 1 mm³ Kratzer statt Loch, ein Schild ohne Aufhängung, seit dem 28.08.; der lies_flat-Befund vom Abend warnte zu Recht, `test_no_example_greets_the_customer_with_a_warning` war zu Recht rot, **und das Warndreieck im Website-Beleg hatte damit einen echten Fehler als Anlass** — der Grundsatz „der Prüfbericht, der mitdenkt, ist das Versprechen" bleibt, sein Beleg war falsch; make_web_images läuft nach (d3), dann Maße, dann Stempel (die Zwei-Nachläufe-Lehre angewandt). **Drei neue Bereiche haben kein Beispiel** (Bündel: nichts zu bündeln; Scharnier; Rezepte) **und die Skizzen als schwerster** — ein ganzer Bereich mit eigenem Löser, in neun Beispielen unsichtbar, und der Kategorien-Test fragt nicht danach (sein Docstring verspricht genau das); **freigegeben: das Skizzen-Beispiel** (Weg 2 in klein, Kreis nach Durchmesser, null Befunde beim Öffnen; Kategorie sketch kommt NACH dem Beispiel in den Test), Rezept und Scharnier bleiben bewusst ohne. d3s Sonden-Selbstkorrektur steht dabei (Document.sketches gibt es nicht — die erste Messung maß ins Leere und meldete brav „keine"). **72s Schluss-Doppel (reviewt):** Die Hook-Schärfung ist gebaut (`b7c6f1cc` — gefragt wird der **Diff gegen die Kataloge**, nicht die pytest-Ausgabe, weil ein Muster über die Ausgabe an der Hook-Locale scheitern kann und dann dasselbe meldet wie eines, das nichts findet; Gegenprobe beidseitig, mehrzeilige Texte bewusst durchgelassen — ein falsch angehaltener Commit kostet fremde Zeit). Und ein main-Rot fiel nebenbei: **`.githooks/commit-msg` stand mit 100644 im Repository — der Umlaut-Wächter lief auf zwei von drei Plattformen wortlos nicht** (`24d3a0ca`; der Hook-Ausführbarkeits-Test fand es — genau wofür er gebaut war). Falle dabei: `git commit -o` nimmt eine **Modusänderung nicht mit** — der erste Anlauf sah erfolgreich aus, der Commit ging ohne die Datei durch. **Die Ein-Zug-Regel hat ihre dritte Stufe (15, im Skill):** Alles, was in der Commit-Meldung als Behauptung steht, wird im selben Aufruf gemessen wie der Commit — ihre „24 von 24 rein Stempel"-Prüfung war beim Laufen korrekt und beim Commit falsch, und nützlich war der mitgenommene Inhalt nur zufällig. **Der Börsen-Hauptweg ist zugesichert (17 Tests in test_catalog_ui, 3a, reviewt — 17/17 in eigener Stichprobe):** die echte Kette über `action_catalog` statt nachgestellter Signale, vier Zusagen, zwei Gegenproben; drei echte Funde beim Bauen, die kein Kern-Rundlauf gezeigt hätte (Rezeptdatei fehlt bei nur-registrierten — Absage mit Handgriff; zwei Register je Rezept; ein modaler Fehlerdialog hält den Prüfstand an). **Die Handbuch-Seite zur Börse steht (`30971c5b`, 15):** beide Richtungen aus Kundensicht, Wortlaute aus 3as Dialogen. **Die Belegbilder zeigen das reparierte Schild (`0933aaa9`, d3):** „Auf dem Verkaufsbild ist jetzt ein Loch, wo vorher eine Warnung stand." **Neuer Betriebsfund (3a+15, `c80f94ec`): `.git/COMMIT_EDITMSG` gehört dem Repository, nicht der Sitzung** — 3as Test landete unter 15s Commit-Meldung (`9c76620f`), die Zuordnung bleibt stehen (History-Rewrite nach Push kostet mehr als eine falsche Log-Zeile); der private Index trennt, WAS committet wird, nicht WIE ES HEISST — dieselbe Gattung wie $TEMP und der Haupt-Index. Und die Kontroll-Hälfte dazu: **Was man am eigenen Commit prüft, muss den eigenen Commit meinen — auch was er behauptet, nicht nur wie viel er trägt** (3a las die Zahl und überflog den fremden Betreff). Für lange Arbeitszüge ist der eigene Worktree die saubere Wahl. **Das Skizzen-Beispiel steht (`87a96a90`, d3, reviewt — 419 grün):** `skizze-mit-massen`, der Durchmesser als erlebbare Bedingung (Tour-Schritt lässt den Kunden ihn ändern), handrechenbarer Sollwert (22 619 − 2400 = 20 219,5), null Befunde beim Öffnen, `sketch` in der Abdeckungsforderung **nach** dem Beispiel; drei ungeplante Pflichten der Reihe nach rot geworden (Tour, README zweimal, acht Katalogtexte mit am Bestand gemessenen Anführungszeichen — en typografisch, Romania Guillemets). Die Belegbilder v3 (`0933aaa9`) tragen den Beweis im Bild: „0 × Warnung" und das Schlüsselloch. **Der Changelog für 0.2.3 steht (`ffaf4601`+`324d12e5`, 50, reviewt — 50 Tests grün):** 15 Punkte in vier Gruppen ×6, reine Kundensprache, und die vierte Gruppe trägt den Titel des Tages: „Wenn etwas nicht hält, sagt es das jetzt" — drei Bausteine, drei Ursachen, ein Muster, als Leitfaden für die 0.2.3-Durchsicht übernommen. Der Presse-Wächter widerlegte sich beim Schreiben selbst (ein 0.2.2-Anschreiben kann nichts über 0.2.3 wissen — die Summe geht jetzt nur bis zur beworbenen Fassung), und der Apostroph-Rückfall ist der zweite „Wer warnt, ist nicht geschützt"-Fall der Nacht. **Die Neun-Zahl ist zu (`d0643954`, d3 — ihre Kette damit restlos):** 30 Stellen statt einer, darunter zwei **mitrechnende** Sätze („vier der neun … die übrigen fünf" wird sechs; die Aufzählung zählt auf — eine Zehn über einer Liste von neun Dingen findet kein Test und jeder Leser). Drei Lehren: **Feste Anker versagen an umbrechendem HTML** (17 von 30 verfehlt, halb geänderte Dateien — ersetzt wird im Kontext des Bezugsworts mit Umbruch-Toleranz, danach die Rest-Neunen gezählt: null); die eigene Kontrolle fragte zu weit (Dateinamen antworteten statt Sätze — „esquisse" stand im Bild-Pfad); und das Blob-Verfahren hielt 15s frische Stempel zwölffach aus dem Index. **DIE NULL STEHT: test_website 347/347** (Schlussstein `79a8ac10`, 15; eigenhändig nachgemessen) — Website, Börse, Handbuch, Bilder, Stempel sind vollständig konsistent. d3s Abschlusscheck: 1665 Tests grün über alles von ihr Angefasste, nichts Rotes hinterlassen. **Und die Commit-Regel hat ihre vierte Stufe (15, im Skill):** Die Erwartung wird vorher angesagt, die Prüfung **bricht ab statt zu berichten** — eine Prüfung, die nur berichtet, ist eine Notiz; eine, die abbricht, ist eine Zusicherung (ihr eigener Lauf gab „6 mit Inhalt" aus und committete trotzdem — sachlich eigener Handbuch-Inhalt, aber die Meldung sprach nur von Stempeln; selbst gemeldet statt unterschlagen). **Die Durchsicht v0.2.3 steht als Artifact** (https://claude.ai/code/artifact/4eb6f0b3-4e9a-4f1e-a7b8-07323d0cdaeb): Übertrag aus 0.2.2 vollständig nachgemessen (B10 durch den Verlaufsbaum eingelöst; die sieben Eventfilter-Fälle sind mit dem Release fällig geworden und stehen in der Restliste; Bedienzone B/C weiter bei Robert), Leitfaden „Wenn etwas nicht hält, sagt es das jetzt", die Börsen- und Website-Bilanz, und **Roberts sechs Entscheidungen als eigener Block** (Upload, WB10, Dental-Zitat, Datenschutz+Kontaktfrage, SHARED_SEED, Bedienzone). Kleinlandungen verbucht: d3s Zahl-im-Fließtext-Notiz (`0fff0048`), der it-Changelog duzt jetzt wie die Anwendung (`40a218de` — 15s Wortlaut-Prüfung trug), Zusicherungs-Notiz (`d76eca51`). **ROBERTS MORGEN-URTEIL (31.08., ~06:45, am lokalen Stand): „die webseite gefällt mir so überhaupt nicht … alles sollte modern, innovativ und den ‚will ich haben'-Eindruck vermitteln" — Runde 2 der Website läuft, mit allen Sitzungen.** Seine konkreten Punkte, von der Freigabe am HTTP-Server nachgemessen: (1) **Aufgeklärt (3a, `b0a9c194` — „Bedienung ist keine Bewegung"): Roberts System und die Freigabe-Messung hatten `prefers-reduced-motion` an** — die Scroll-Drehung war dort *korrekt* aus (sie läuft ohne Zutun), aber Regler und Ziehgeste stiegen fälschlich mit aus; der konsequent klingende Kommentar-Satz (keine Bewegung, auch nicht auf Geste) war die falsche Entscheidung: Ein Knopf, der auf einen Klick nicht antwortet, ist für jeden ein Fehler und für den Bewegungsempfindlichen eine Ausgrenzung genau dort, wo die Seite entgegenkommen wollte. Jetzt: Bedienung immer, autonome Bewegung nur ohne reduced-motion; nachgemessen Regler 73,3 mm MIT Bildwechsel, Ziehen 88,6 %. Der duration-Verdacht der Freigabe war falsch (steht auf auto, die Regel war in Ordnung — nur abgeschaltet). Offener Rest: Der Hinweis „scrollen dreht ihn" ist bei reduced-motion halb unwahr — Skript schreibt um, sobald das Markup `data-calm` liefert (Wortlaut 15, Markup 50 ×6). **Roberts Nachschlag (06:5x, Screenshot): Die Galerie-Kacheln stehen nicht in einer Reihe** (die Vorratsdose sitzt versetzt — ungleiche Bildhöhen/Ausrichtung) **und das Verwandlungspaar ist riesig** (zwei Kugeln füllen den halben Schirm — die SVGs skalieren ungedeckelt auf Spaltenbreite); dazu sein Auftrag: **alle Seiten durchschauen, ob es sauber ist** — die Größen-/Rhythmus-Durchsicht (50/15) gilt damit ausdrücklich für jede Seite, nicht nur index; (2) **der Regler ändert nur den Zahlentext**, ein sichtbarer Bildwechsel fehlt; Ziehen ungeprüft — der site.js-Block war für img-Tausch gebaut, die Bühne ist ein background-Sprite; (3) **das Schild-Belegbild wirkt „rausgeschnitten"** — die 2026-Lettern liegen abseits des Schilds, das Motiv liest sich als Fehler statt als Zweifarben-Beispiel; (4) **die Bilder sind zu groß** (Vollfenster in voller Breite — WB8 war nie umgesetzt) und **die Seite zu leer** (riesige dunkle Flächen zwischen kleinen Inseln); (5) boerse.html zeigt ohne erreichbare API dauerhaft „Die Liste wird geladen …" statt eines ehrlichen Zustands. Roberts Screenshots zeigen die **dunkle** Fassung (Browser-Dark-Mode) — beide Themen gehören in jede Nachmessung. **72s IST-Abgleich liegt vor (reviewt, mit einer Korrektur): 3 Muster eingelöst (M1/M7/M12-technisch), 4 behauptet, 5 fehlen** — die härtesten Löcher: **M2 fehlt ganz** (null video/mp4/webm im HTML, und gemessen liegen auch keine Loop-Dateien im Repository — die WD1-Werkzeuge existieren, das Material wurde nie für die Seite erzeugt), M11 fehlt (system-ui statt Markenschrift), M6 ist behauptet (Renderings unter der Überschrift „Was am Ende auf dem Bett liegt" — ein Rendering beweist die Software, ein Foto das Teil), M3 steht nur im Fließtext, und M12 ist technisch eingelöst und inhaltlich verfehlt (38 dekorative Animationen, keine beweisende — genau der Fall, vor dem das Muster warnt). **Korrektur an M10, von 72 nachgeschärft: substanziell eingelöst** — die Verwandlungsbilder sind Projektionen eines echten Netzes, nicht Cartoons; der Unterschied zu den #wege-Vignetten ist der zwischen Illustration und Beleg. Bilanz: 4 eingelöst (M1/M7/M10/M12-technisch), 4 behauptet, 5 fehlen (M2/M5/M8/M9/M11). **Ihre Lehre schärft benannte-falle-schuetzt-nicht:** Falle erkannt, korrekt angewandt — und einen Absatz weiter die Ersatzzahl aus derselben kaputten Suche genommen. **Ein Werkzeug, dessen Fehler man kennt, verleitet dazu, es korrigiert weiterzubenutzen statt es zu wechseln**; die fehlende zweite Frage: Ist der Wortlaut überhaupt die richtige Spur für ein Bild? 72s Prioritäten-Einsicht ist übernommen: **M2/M5/M6-Foto sind Materialfragen, nicht Gestaltungsfragen** — M5 und die Fotos liegen bei Robert (stehen in seinem Block), M2 ist sofort machbar. **Zwei Kernbefunde aus 3as erster M2-Messung:** Die Loop-Kette trägt (0,9 MB webm am ersten Lauf), aber das einstieg-Drehbuch fährt die flache Beispiel-Platte — Roberts gestern verworfenes Motiv; **Entscheid: Motiv schlägt Drehbuch** — je Weg das stärkste Schaustück statt der alten Drehbücher, das Werkzeug lernt `--morph <parameter>` statt einer Rezept-Umbenennung. Und **der Hero-Regler steht bei 800×900 unter der Falz** (Unterkante 1014) — das tragende Stück der Aussage ist ohne Scrollen unsichtbar, vermutlich ein Hauptanteil von „wirkt nicht modern"; 3as gemessene Straffung (Bühne 16/7, herangerückt: 136 Punkte) ist freigegeben, Einbau-Choreografie 3a↔50, die letzten 18 Punkte über kürzeren Text (15/50). **ERSTE SEITE BESTEHT DIE LATTE: `/index.html` samt aller fünf Sprachfassungen** (15s Serie bis `8051b12f`, von 72 nachgemessen statt geglaubt): Überschriften-Reihe sauber (die zwei Download-Karten waren Beschriftung, keine Gliederung — `e9ca3e50`), Mobil-Überlauf 4→0 (gemessen an der **französischen** Fassung, der längsten — null dort heißt null überall), 0 tote Verweise, alle Bilder mit alt. Drei Lehren daraus: **`overflow: clip` verbirgt genau den Überlauf, den es abschneidet** (kein Rollbalken verrät ihn, im Bild sieht es wie Umbruch aus — nur die getBoundingClientRect-Messung fand ihn); **`min-width: 0` am Elternteil schützt nicht das Layout** (die Kinder standen auf auto und wuchsen); und 15s Selbsteinordnung: **„soweit sie dieselben Bausteine tragen" war eine Annahme im Nebensatz — wer ihn liest, hält sie für geprüft.** Offen daraus: **43 Elemente warten unsichtbar auf ihre Einblendung** — beim zügigen Scrollen entstehen leere Flächen (15s eigenes fast schwarzes Bildschirmfoto), vermutlich ein Teil von Roberts „zu leer"; **bei 3a geschnitten** (Einblendungs-Choreografie gehört zur Interaktions-Spur). Und 72s Melde-Regel: Während ein Erzeugerlauf läuft, sind Bildmaße ein Zeitstempel und kein Befund. **ROBERTS ZWEITES MORGEN-URTEIL (31.08.): „mach es richtig, da hat es mir am anfang schon mehr gefallen, wie 2026 ist hier nichts"** — vier Befunde: Regler ändert das Maß nicht, Scrollen dreht und scrollt zugleich, die Spende weiter zu tief, die Paket-Links fehlen. **Wurzelursache gefunden (3a): `site.js` ist auf allen 42 Seiten absolut eingebunden (`src="/site.js"`) — unter `file://` zeigt das aufs Laufwerksstammverzeichnis, das Skript lädt nie**, die Konsole schweigt (Chromium meldet fehlende file://-Skripte still), und Regler, data-release-Links und alles Interaktive fehlen gemeinsam; das Stylesheet lädt relativ, darum sah die Seite richtig aus und tat nichts. Fix bei 50 (relative Pfade je Ebene, wie beim data-dial schon gebaut), Wächter in test_website.py gegen absolute Eigen-Verweise gehört dazu. **Entscheid: Die Scroll-Kopplung der Drehung ist gestorben** — Roberts zweiter Anlauf gegen dasselbe Feature; die Bühne dreht nur noch per Ziehen und Pfeiltasten, Standbild als Ruhezustand (3a). **Und meine 44 670-px-Zahl ist gefallen, an drei unabhängigen Messungen** (15: 12 301@1440 / 20 583@375; d3: 12 621@1440 / 24 007@322; 72: 12 433@1265 / 20 592@375): Ich hatte in einem Hintergrund-Tab mit **innerWidth 0** gemessen — bei Breite null bricht alles maximal um, die Zahl war ein Artefakt des Werkzeugs, nicht eine Eigenschaft der Seite. Lehre, und 72s Praxis wird damit Regel: **Jede Layout-Messung bestätigt ihre eigene Lage** (innerWidth und Protokoll gehören in jedes Messskript — 72 fing am selben Abend eine http-Messung, die sich als file:// ausgegeben hätte), und eine Höhe ist nur als Paar mit benannter Breite eine Zahl (fortan Höhe@1440 und Höhe@375). Folge-Entscheidungen: Die Desktop-Höhe ist erreicht; der Mobil-Hebel ist nicht Bild und nicht Kürzungs-Kampagne, sondern **Zusammenlegung mit Grund** (15: `warum` und `ehrlich` erzählen dasselbe, zusammen 1181 px, durch drei Sektionen getrennt); **kein Beleg-Zuschnitt auf der Startseite** (d3s Vorlage — 50s Regel trägt: „so sieht das Programm aus" *ist* die Aussage), stattdessen globaler CSS-Deckel `max-height: 60vh` bei 50; und 72s file://-Lücke ist benannt statt behauptet — die Lage deckt der statische Wächter, nicht der Browser-Pane. **50s Einspruch kippt den 42-Seiten-Umbau, und er trägt** (drei Messungen): Die eigene `.htaccess` leitet `index.html` per 301 auf `/` um — 72s `../en/index.html`-Ratschlag würde jeden Sprachwechsel der ausgelieferten Seite um einen Roundtrip verteuern; die drei `fetch` auf `/api/` können nie relativ werden (der Endpunkt liegt einmal im Stamm), und unter file:// gibt es ohnehin kein PHP — nach dem vollen Umbau wäre Roberts Seite immer noch halbtot (Zähler leer, Börse leer, Freischaltung tot). **Entscheid: kein Umbau, der Wächter wird eine Messung mit API-Ausnahme statt eines roten Laufs, und Roberts Prüfweg ist der lokale Server** — er prüft inzwischen selbst über localhost:8000, wo Regler und Paket-Knöpfe bei ihm laufen. **15 zieht die Zusammenlegung `warum`+`ehrlich` selbst zurück** — gelesen erzählen sie Verschiedenes („was es löst" / „was es nicht verspricht"), die Reihenfolge trägt, der halbe Doppel-Satz bleibt aus zwei Gründen; die Lehre wörtlich: **„Eine tote Zahl erzeugt Gründe, die ohne sie keine sind"** — an ihrem eigenen Vorschlag vorgeführt, zwanzig Minuten nach ihrer eigenen Widerlegung der Zahl. **Roberts weitere Vormittags-Befunde (31.08., mit Fotos):** boerse.html „Selbst etwas teilen" — Rezeptdatei-Knopf und Mailadresse-Feld versetzt, gehören auf eine Grundlinie (50); „Die Liste wird geladen …" braucht den Fehlpfad statt des ewigen Ladetexts (50); der obere Seitenabstand (Hero → Häkchen → Download-Kasten) zu groß; **und die Spende zum dritten Mal: „der paypal absatz immer noch nicht sinnvoll nach oben geschoben … die punkte hab ich aber auch schon 3 mal gesagt und nichts ist passiert"** — Entscheid: Spende direkt unter den Download-Kasten (Nachbarschaft „kostenlos laden → freiwillig unterstützen"); Eilauftrag an 50 mit Vollzugsmeldung, die Aufträge waren dreimal verteilt und nicht gelandet — Verteilen ist kein Liefern, ab jetzt trägt jeder Robert-Befund eine Vollzugsmeldung mit Hash. **50 liefert, Robert bestätigt und verwirft zugleich: „paypal ist zwar jetzt oben aber schön und neu und modern gestaltet wie du alles solltest ist ja sowieso nichts"** — die Runde verschiebt sich damit von Sauberkeit zu Schönheit; Design-Direktive an 50 in zwei Stufen: heute Spende-Kasten als hochwertiges Element statt umrandeter Box, Knopf- und Karten-Politur (Radien, Fokus, Hover, Tiefe statt Rahmen), Typo-Skala straffen — dann das M11-Schriftpaar **selbst gehostet** (OFL-Schriften in website/, „nichts von außen" bleibt wahr, Lizenzeintrag nach Checkliste), denn system-ui trägt keinen 2026-Eindruck. **ZWEITE SEITE BESTEHT DIE LATTE: `/ki-modelle.html` samt aller fünf Fassungen** (d3s `b3e63232`, von 72 nachgemessen): H-Reihe [1,2,2,2,2,2] ohne Sprünge (H3→H2 nur ohne Attribute — die Schaustück-Karten blieben verschont, am Bild gegengeprüft: 32 statt 20 Punkt sieht besser aus), 0 tote Verweise, 9 Bilder mit alt, Höhen 5 524@1440 / 8 870@375, Hero-Beleg 55 %/28 % — d3s Deckel-Meldung war derselbe Lagen-Fehler wie die 322er-Messung, durch den Quellen-Schnitt ohnehin gelöst; der 60vh-Deckel bei 50 bleibt trotzdem beauftragt (er gilt allen Bildern). Offen je Latte nur der Stempel als 15s Schlussstein nach den Erzeugerläufen. **Roberts Statistik-Auftrag erledigt (`877d44c3`):** stats.php sagt aus denselben Daten jetzt auch wann (Uhrzeit, Wochentag), wohin (Sprachfassung, Zielsystem, Einstiegsseite), wie tief (Seiten je Besuch, Besuchstiefe) und woher (Direktzugriffe beziffert), dazu Monate im Vergleich — Ende zu Ende gegen erfundene Zähldaten mit vorab gerechneten Sollwerten gemessen, geht mit der Upload-Freigabe live. **ROBERTS ORDER (31.08., nach „ja dann los", „nicht nur den spendenkasten sondern alle webseiten" und „modern und innovativ"): „sauber mit allen sessions durcharbeiten und verifizieren dass wir dann eine 2026 seite auch haben vom design."** Damit wird die DESIGN-ABNAHME eine Latte mit acht messbaren Kriterien, je Seite und am Ende als Ganzes: (1) **Schrift** — eigenes Paar statt system-ui auf jeder Seite aktiv (computed font-family), WOFF2 selbst gehostet, Fallback-Kette; (2) **Fläche statt Rahmen** — Tiefe über Fläche/Schatten, keine umrandeten Kisten-Bauteile, ein Akzent konsequent; (3) **Bauteile** — Knöpfe/Karten/Felder mit einheitlichen Radien und echten Hover-/Fokus-/Aktiv-Zuständen, Formulare fluchten auf null Versatz; (4) **Dichte** — keine Leerfläche über ~40 % Viewport zwischen Sektionen, Höhenpaare @1440/@375 im Ziel; (5) **Bewegung nur beweisend** — Loops der vier Wege, Bühne auf Geste, nichts wartet unsichtbar im Bild, reduced-motion sauber; (6) **Bilder** — gedeckelt, Kartenform, Zuschnitt wo das Detail die Aussage ist; (7) **beide Themen und beide Lagen** — hell/dunkel × Desktop/375, mit Bildschirmfotos als Beleg; (8) **Konsistenz über alle 43 Seiten** — dieselben Bauteile überall, erzeugte Seiten über ihre Erzeuger. Verifikation zweistufig: **72 misst das Messbare je Seite**, **die Review-Instanz macht die Schluss-Sichtung aus Kundensicht** mit Vorher/Nachher-Strecke für Robert. Arbeitsteilung in style.css (Choreografie-Entscheid): **50 nimmt Knöpfe, Zustände und Karten-Grundform; 15 behält Abstände, Rhythmus und Typo-Skala** — die Hälften berühren sich nicht. Vollzüge: 50s `36a45a63` — der Spendenkasten war die braune Box aus drei Gründen zugleich (Akzentrahmen + Verlauf + getönte Fläche) und **48 Punkt breiter als die Karte darüber** — zwei Kanten, die um wenige Prozent auseinanderliegen, lesen sich als Versehen, nicht als Gestaltung; jetzt gleiche Breite, gleicher Radius, Rahmen als Innenschatten, Knopf neutral gefüllt mit Akzent-Sprung beim Zeigen. 3as `38117058` — die zwei gealterten Docstring-Absätze raus, und die 43er-Zahl korrigiert: **Der Ruhezustand war die falsche Messgröße** — gezählt gehört, was **im Bild** steht und durchsichtig ist: am breiten Fenster nirgends eines, am schmalen zwei (cover 22 % rechnet über die Sichthöhe — am Schreibtisch unsichtbar); nach `entry 0% cover 12%` bleibt eines, und das letzte kostete die Einblendung selbst. Review-Stand der übrigen Vormittags-Landungen: f1f5b647 sauber (ein Docstring-Befund, behoben in 38117058); e32b2978 gelesen und schlüssig — die 5 940 Zeilen sind Zeilenenden-Normalisierung, der Wort-Diff klein (subgrid statt flex-end-Unterkanten, `:has(button)` statt Reihenfolge, aria-busy-Dauerzustand raus); 15s Matrix (23 Seiten) durch die Latte; 72s Drei-falsche-Befunde-Lehre: **Rohes HTML ist nicht, was der Leser bekommt** (CSS-Kommentar, Aufbau-Zeitpunkt, display:none — dreimal die falsche Ebene, nicht das falsche Muster), Überschriften zählt die Latte fortan am gerenderten Dokument; und die Regel: **Wo zwei Prüfungen sich widersprechen, ist die Frage zu klären, nicht die Prüfung abzuschaffen.** **Kriterium 5 hat eine neue Messgröße, und der Weg dorthin trug zwei Lehren:** 72 fing ihren vierten Fehlbefund vor dem Melden — `getComputedStyle().opacity` gibt nach abgeschlossener Animation ohne `fill-mode: forwards` den **Regelwert** zurück, nicht den Bildzustand (die komplette Hero-Botschaft „unsichtbar" bei sichtbarem Screenshot); gemessen wird fortan mit `checkVisibility({opacityProperty, visibilityProperty})`, und jeder Unsichtbar-Treffer wird am Bildschirmfoto nachgesehen, bevor er ein Befund wird. 3as Gegenmessung drückte die Einblende-Zahl auf **null**: Das letzte Element stand bei 0,48 mitten in seiner eigenen Bewegung — ihre 0,5 war eine Design-Grenze, keine Sichtbarkeitsgrenze; Lehre: **„Eine Zahl ohne ihre Schwelle ist so wenig aussagekräftig wie eine ohne ihre Einheit."** Der Fix (`entry 0% cover 12%`, 38117058) bleibt richtig. **50s Stufe-1-Funde an style.css:** 19 verschiedene Eckenradien (0.3 bis 1.25 rem, dazu ein `var(--radius)` ohne Definition) → vier Stufen plus Pille; **32 Rahmen gegen 10 Schatten** — darum wirkt die Seite wie ein Formular statt wie ein Produkt; und ihre Selbstkorrektur: „3 focus-visible bei 20 hover" war Zählung statt Abdeckung — eine globale Regel deckt alles, **gezählt gehört die Abdeckung, nicht das Vorkommen**. **Schriftpaar gewählt und beschafft:** Archivo (Omnibus-Type, variabel wght 100–900 + wdth 62–125 — die Breitenachse ist der Charakterträger, Messgeräte-Anmutung statt Startup-Logo) und Source Sans 3 (Adobe, wght 200–900), beide OFL 1.1, beide Latin-Extended-geprüft per cmap (ß ä ñ ç ã õ è ù vollständig); Beschaffung durch die Review-Instanz von den GitHub-Quellen, Archivo aus der Variable-TTF konvertiert (nur statische WOFF2 im Repo), klammerfreie Dateinamen (eckige Klammern sind URL-reserviert), 360 KB gesamt in `website/fonts/` — 50 committet sie mit Einbindung und Lizenzeintrag als eine Einheit. **Roberts nächste Befunde (Vormittag, localhost:8000): „dass das mit herunterladen und unterstützen so viel platz einnimmt stört mich auch noch, sollte aber oben sein" + „der abstand über entwickelt in deutschland ist auch zu groß."** Entscheid zur Verdichtung (bei 50): Beide Karten bleiben oben und werden zusammen etwa halb so hoch — das Kleingedruckte der Download-Karte wandert hinter den vorhandenen Aufklapper (sichtbar: Titel, drei Paket-Knöpfe, Versionszeile, ein Satz), die Spende wird einzeilig (Titel + Satz + Knopf); Ziel unter ~350 Punkt für beide zusammen. Der Abstand-Befund nahm einen Umweg über einen Lesefehler der Review-Instanz: Ich las Roberts Foto als **leeren Hero** und diagnostizierte einen halben Schreibvorgang — Roberts Antwort: **„ist doch alles da … war nie weg"**, und er hat recht: Sein Foto war ein **gescrollter Ausschnitt** (H1 und Bühne bereits oberhalb des Bildes, nur die Regler-Zeile noch darin), keine leere Seite. Sechste Instanz des Musters, diesmal am Foto selbst: **Auch ein Bildschirmfoto trägt seine Lage** — ohne Scroll-Position gelesen ist es eine Zahl ohne Einheit, und die Scrollleiste im Bild war die Lage-Angabe, die ungelesen blieb; das Foto bleibt die Instanz für Sichtbarkeit, aber erst Foto **plus Lage** ist ein Befund. Roberts Platz-Punkt bleibt dabei voll bestehen: Zwischen Regler-Zeile, Häkchen-Zeile und Download-Karte liegt echte Luft (15s Rhythmus-Hälfte), und die Verdichtung von Download+Spende ist beauftragt. **ZWEITER „X-MAL GESAGT"-VORFALL (31.08., mit App-Screenshot): „bei dem bild soll der ausschnitt in dem rechteck auch raus, hab ich jetzt auch schon 5 mal gesagt"** — die rechteckige Aufhängungs-Tasche oben links auf der Schild-Platte (Verlaufsschritt „Aufhängung" in `schild-zweifarbig.p3d`); fünfmal genannt, immer noch im Motiv. Eilauftrag an d3 mit Vollzugspflicht: raus aus dem Beispielprojekt und aus jedem Bild, das das Schild zeigt, Zeitstempel-Kontrolle nach dem Neubau. Zu klären danach: wo die vier früheren Nennungen versickert sind — der erste Vorfall (Spende) lag am Verteilen-ohne-Landen, dieser womöglich am selben Muster eine Ebene tiefer; d3s parallele Kontrollrunden-Antwort führte das Schild als „fertig, Robert hat nicht widersprochen" — von der Wirklichkeit überholt, die Nachrichten kreuzten sich. **Und Roberts nächster Befund (ki-modelle.html, localhost:8000): „hier auch viel leer"** — der Hero der KI-Seite trägt links drei Textzeilen und rechts das Fenster-Bild, darunter und daneben klafft dunkle Fläche bis zur nächsten Sektion; die Seite bestand die Struktur-Latte, aber Kriterium 4 der Design-Latte (Dichte) misst anderes als tote Verweise — **bestanden nach der alten Latte heißt nicht bestanden nach der neuen**; Straffung bei d3 (deren Seite), Abstimmung mit 15s Rhythmus-Regeln. **72s Gesamtstand der Kontrollrunde (alles nachgemessen): DIE EIGENE SCHRIFT IST LIVE** — `getComputedStyle(h1).fontFamily` gibt **Archivo**, M11 ist eingelöst, nicht nur beauftragt; Startseite nach den frischen Landungen: Höhe@1440 11 994 (die Landungen kürzten), Hover 22, Fokus-Abdeckung global, Radien-Varianten 7 (von 19 kommend, Ziel 4+Pille), Leerflächen keine; alle Seitengruppen bestanden (index ×6, ki-modelle ×6, funktionen/changelog/handbuch ×18 Stichprobe, Rechtstexte, boerse). **Kriterium 5 brauchte beim ersten Einsatz sofort die neue Regel: 19 von der Abfrage gemeldet, 0 im Foto bestätigt, 0 Befunde** — die Zahl wird fortan immer als Paar gemeldet, und ein Foto wird mit seiner Lage gemeldet (Breite, Scrollposition, Thema). Offen als Rest der M-Liste: M2 (Loops — rechnen bei 3a, erster Lauf 0,9 MB webm/12 s, vier zusammen unter 5 MB; **Motive sind die vier Weg-Beispielprojekte** aus app/examples — Weg 2 sofort mit `--morph breite:40:90`, Weg 1 braucht Drehung statt Morph, das Projekt hat keine Parameter und morph_step stiege still aus, Weg 4 bleibt Drehbuch-Neubau), M8 (#wege ohne Bild/Video — die Loops lösen es mit), M9 (Zahlenband ohne Belege), M5 (fremde Stimmen — Material liegt bei Robert). **M6 ist GESTRICHEN, Entscheid Robert (31.08.): „wir zeigen unsere software, keine fertigen drucks"** — keine Fotos gedruckter Teile; die Galerie bleibt gerenderter Software-Output, und jedes Bild der Website zeigt die Anwendung oder was sie erzeugt. **50s `c8999e7a` reviewt: Schrift-Einbindung und Verdichtung in einem Zug** — beide Karten zusammen 600→375 Punkt (Download 235, Spende 140; die 25 über dem Ziel sind die Versionszeile, sie beantwortet „ist das aktuell"), **verschoben statt gekürzt** als bewusste Entscheidung (sechs Fassungen, geprüfte Wortlaute, „bis zum 30. Oktober 2026" ist eine Zusage — wer verschiebt, muss nur zeigen, dass der Text noch da ist); dabei ein Test geschärft: Die FAQ-Zählung fragte „was ist KEIN FAQ" über eine Ausnahmeliste, die mit jedem Aufklapper wachsen musste — jetzt zählt sie `<details>` ohne Klasse („was IST eines"), Mutationsprobe wieder rot, 347 grün. Merksatz aus dem Bau: **Eine `url()` in CSS zählt vom Ort der CSS-Datei, nicht der ladenden Seite** — die Schriftpfade treffen von allen 43 Seiten dieselbe Datei, der Mechanismus, den HTML-Verweise nicht haben. Entscheide: Rahmen→Flächen-Umbau wartet auf 15s Typo-Landung (nicht zwei Hände auf derselben Fläche); sichtbarer Schrift-Lizenzhinweis in die EULA-Quelle (`EULA.md`, führt die Fremdlizenzen ohnehin, make_legal erzeugt daraus). **ROBERTS MENÜ-BEFUND MIT HARTEM KERN: „die seitenverteilung ist auch bisschen schwer über das menü … wo kommt man denn jetzt einfach und schnell zu der katalog/bausteinbörse?" — gemessen: `boerse.html` hat NULL eingehende Verweise**, der Name kommt in keiner anderen Seite vor; die komplette Tauschbörse ist eine unerreichbare Insel, und der Verweis-Test sah es nie, weil er tote **ausgehende** Links prüft, nicht verwaiste Seiten — eine Testlücke der Klasse „die Frage wurde nie gestellt". Dazu je Seite verschiedene Kopfmenüs (index: Funktionen·Preis·Handbuch·Neuerungen; ki-modelle: Funktionen·Vier Wege·Handbuch). Eilauftrag an 50: EIN identisches Kopfmenü auf allen 43 Seiten (Funktionen · KI-Modelle · Tauschbörse · Preis · Neuerungen · Handbuch + Sprachumschalter + Demo-CTA, aria-current auf der aktuellen Seite, Fußzeile ergänzt), plus Wächter „keine veröffentlichte Seite ohne eingehenden Verweis". **Roberts Abstands-Fall oben ist behoben (15, nachgemessen @1440×950 mit neuer Schrift):** Aufmacher 675→611 Punkt (71→64 % des Bildschirms), Regler→Häkchen 106→63, die Downloadkarte liegt bei 767 im ersten Bildschirm — eine Zeile in style.css:717; ihr Commit dazu kommt **nicht**, denn `c8999e7a` hat die Zeile im Wettlauf des geteilten Baums mit eingesammelt, und ein Nach-Commit wäre eine zweite Wahrheit über denselben Stand. **Die vierte Stufe der Commit-Kette hat dabei im Ernstfall gehalten:** Ihr Guard meldete 3 Dateien statt der angesagten 7 mit fremden Schriften in der Liste und **brach ab statt zu berichten** — ohne ihn wären die woff2-Dateien unter einer Meldung über Aufmacher-Polster gelandet. Und 50s Fugen-Messung (Aufmacher→Häkchen 20, Häkchen→Download 49: dort ist nichts zu holen) bewahrte 15 davor, an der einzigen Stelle zu drehen, die schon stimmte. Offen als eigener Punkt: **die linke Aufmacher-Spalte endet 128 Punkte früher als die rechte** — zwei Spalten verschiedener Höhe, keine Polsterzahl; 15 macht einen Vorschlag mit Bild. Danach bei ihr: Typo-Skala (Kriterium 1), dann die max-width-Kanten-Paare. **SCHILD-VOLLZUG (d3, `9ee12c67`): Die Aufhängungs-Tasche ist raus, an allen vier Stellen** — Bauplan des Beispiels (insert_keyhole), Katalogtext („zweifarbig und aufhängbar"→„zweifarbig"), Tourschritt, Beispielprojekt; alle dreißig Belege neu erzeugt (eine Sprache je Lauf, Zeitstempel geprüft, null älter als fünf Minuten), Handbuchbilder gemessen unbeteiligt, das Bild angesehen: vier Verlaufsschritte, kein Rechteck. **Und die Aufklärung der versickerten Nennungen:** Der Punkt kam bei d3 zweimal an und wurde zweimal als *Bild*ausschnitt gelesen — die 2026-Lettern-Reparatur war ein echter, aber anderer Fehler; erst „Verlaufsschritt 3, das Rechteck oben links" löste es. Lehre: **„Ausschnitt" hieß im einen Gebiet Bildzuschnitt und im anderen Ausfräsung — dasselbe Wort, zwei Gebiete; beim zweiten Mal fragen statt eine zweite plausible Deutung bauen.** Ihr gemeldeter roter Galerie-Test (`gehaeuse`) reproduziert auf HEAD **nicht** (4 passed, Exit 0 nachgemessen) — ein Zeitpunkt-Artefakt des geteilten Baums, die bekannte Klasse. **3as ehrlicher Loop-Stand:** 0 fertig, Vorlauf bei 203/360 (die 20-Minuten-Ansage war zu optimistisch — 79 134 Dreiecke, jeder Morph-Schritt wertet die Szene neu; keine Zahl mehr vor dem ersten fertigen Lauf); Reihenfolge Weg 2→1→3→4, **Übergabe an 50 einzeln statt am Ende** — wenn Robert nach dem ersten „so nicht" sagt, sind drei Läufe gespart. **72 bestätigt f3e3bdb2 vollständig:** Hinweistext in allen sechs Fassungen mit „ziehen" und Pfeiltasten, kein data-calm; der Börsen-Fehlpfad im Bild bestätigt (Nochmal-Knopf, Lage 1100×2400 dabei) — und dabei bewährten sich beide neuen Regeln am ersten Tag: Die **siebte Instanz** des Musters war die billigste (vier `calm`-Treffer in site.js waren eine lokale Countdown-Variable — nach dem **Wort** gesucht statt nach der Sache), und die Lage-Regel fing das Gegenteil eines Fehlbefunds (Abfrage „sichtbar", Foto bei scrollY 0 zeigt den Knopf nicht — beides richtig, er lag bei y=1064 unterhalb des Bildes; ohne „auch ein Foto trägt seine Lage" wäre genau hier ein Befund gemeldet worden). **15s Typo-Skala gelandet (`6aadde29`, reviewt):** 44 Schriftgrößen → **acht Stufen in :root** (--t-xs bis --t-2xl, 78 Regeln greifen zu), **aus dem Bestand gewachsen statt aus einer Formel** (0,9rem stand neunmal — die häufigen Werte bleiben, die Einzelgänger ziehen zu ihnen); zwei begründete Ausnahmen (Preis, Zeichnungsmaß), die elf clamp-Werte blieben — ihr erster Entwurf hatte sie eingezogen und nur die Oberkante gemessen, vier Überschriften hätten ihre Beweglichkeit verloren: **Eine Spanne ist keine Zahl.** 347 grün, Rechtstext-Überschriften bewusst verschoben. 50 kann den Rahmen-Umbau starten; der 128er-Vorschlag wartet auf 3as Hero-Loop-Höhe (nicht gegen einen Stand messen, den es in zwei Stunden nicht mehr gibt) und kommt als Bild an Robert und die Review-Instanz, nicht als Commit. **Der Loop-Vorlauf ist durch und von der Review-Instanz am Bild gesichtet:** Anwendung mit 24-Schritte-Verlauf, oranger Deckel mit Schriftzug, Prüfbericht 0 Fehler/0 Warnungen/12 Hinweise — der arrange_bed-Fix ist im Werbebild bewiesen; 1,1 MB webm/12 s; offener Folgepunkt Kontaktschatten („wirkt schwebend bei z=0,00" ist ein Kundeneindruck, kein Geometriethema — die Messung sagt nur, dass die Geometrie stimmt, nicht was jemand sieht); Weg 2 rechnet. **15s Drei-Wege-Vorschlag zum Spaltenversatz kam als Artefakt mit Maßzeichnung, Entscheid: Weg C plus B** — die Zusagen-Zeile wandert ins Hero-Loch (Versatz 134→10, das Band entfällt, die Download-Karte rückt von 766 auf ~370 in den ersten Bildschirm), Weg A wurde gemessen und verworfen (align-items:center teilt das Loch statt es zu schließen — die beanstandete Luft käme oben zurück); Zusatz-Entscheid: **im Aufmacher gibt es keine Scroll-Einblendung**, was im ersten Bildschirm steht, ist sofort voll da; gebaut wird nach 3as neuer Bühne. Dass Review-Instanz und 3a unabhängig dieselbe Häkchen-Zeile vorschlugen, wog schwerer als die Zahl. **ROBERTS DOWNLOAD-FRAGE DECKTE EINEN BEFUND AUF: „das jetzt kostenlos herunterladen lädt sofort die windows version? wie ist das bei mac oder linux?"** — gemessen: Das Release-Skript setzt dem Hero-Knopf `href` und `download` auf die Windows-Setup-Datei; **jeder Klick lädt sofort 199 MB Windows, auch auf macOS und Linux.** Entscheid an 50: OS-Erkennung als progressive enhancement — Windows behält den Direkt-Download, macOS/Linux führen zum Kasten bzw. zur passenden Wahl, im Kasten wird der erkannte Primärknopf vorangestellt; die Silicon/Intel- und AppImage/Flatpak-Wahl bleibt Dialog (der Browser kann sie nicht sicher erkennen), ohne JS nie blind Windows. Der Kasten selbst ist gut gebaut (Wahl-Dialoge mit Erklärsatz). **Und Roberts Gestaltungs-Go: „bei manchen absätzen mit stilvollen tabs arbeiten oder aufklappen, einblenden, bisschen moderner"** — nächste Welle nach Menü und Rahmen-Umbau: Tab-Komponente und gestylte Aufklapper als Bauteile (Tastatur und aria vollständig, nichts wartet unsichtbar), 15 und 50 bestimmen gemeinsam, welche Absätze Tabs verdienen (parallel Lesbares) und welche Aufklapper (Vertiefendes). **DAS MENÜ IST GELANDET (50, `34a27461`, reviewt): Die Tauschbörse steht in Kopf- und Fußzeile aller Seiten, aria-current markiert die aktuelle, der Erreichbarkeits-Wächter läuft grün** — die Insel ist angebunden, 45 Dateien in einem Zug. **Und der Weg-2-Loop fiel bei 3a durch die eigene Latte, mit der besten Selbstüberführung des Tages:** Das Beispielprojekt ergab eine flache Platte mit zwei Löchern — technisch tadellos, werblich wertlos, und ihr eigener Docstring in make_showpiece.py hatte es zwei Tage vorher wörtlich vorhergesagt („Wer dort eine flache Platte sieht, denkt ‚das kann jeder' und liest nicht weiter"); sie fand die vier weg*-Projekte, freute sich über die Passgenauigkeit und stellte die Frage nicht, die sie selbst schon beantwortet hatte — **die richtige Antwort auf die falsche Frage: nicht „welches Projekt gehört zu welchem Weg", sondern „welches Teil will jemand haben".** Entscheid: Der Gehäuse-Loop (der „Vorlauf") IST Weg 2 und geht sofort an 50 — ein starkes Video schlägt vier magere, die Sektion startet mit einem; die drei anderen Wege bekommen echte Schaustücke in der Reihenfolge Weg 1 (Produktkern: eingelesene STL mit sichtbarer Anpassung, Kandidat Rollenhalter — Bühne und Video erzählen dann dieselbe Geschichte), Weg 3 (das aufbereitete Eulen-Netz der ki-modelle-Seite — dasselbe Motiv ist Konsistenz), Weg 4 zuletzt; jeder landet einzeln. **ROBERTS ORDER (31.08., ~08:35): „kein registerpunkt soll offen sein, alle sauber abarbeiten, verteilen und wieder alle anderen sessions im 30-minuten-takt kontrollieren für die nächsten 5 stunden, mach alles sauber, auch wenn du die webseite von grund auf neu baust."** Der Kontrolltakt läuft (30 min × 10 bis ~13:40). Das Register ist in drei Klassen sortiert: **(a) Heute von Sitzungen schließbar, verteilt als Welle 3** — 50: Rahmen→Flächen, OS-Erkennung, Tabs-Komponente, G11, Flug-Marken-Nachweis; 15: Zusagen-Zeile, max-width-Paare, G12/D11, Belegungs-Entscheid, Ø-Bedingung im Kern, Mehrfachtexte als Beifang; d3: Update-Fenster-Gruppen, Schemabild Skizzeneditor (durch D-Abschluss entsperrt), danach P6/P9 Grundsteuerung; 3a: Weg-2-Übergabe, Schaustücke 1/3/4, Kontaktschatten, Offscreen-Zusagen (Schnitt: die drei belegten Lügen als Sichtfenster-Tests), Prüfstand-Modal (Entscheid: ein Prüfstand darf report_error abschalten, die Anwendung nie); 72: style.py-Serie G1/G2/G3/G7 mit Belegbildern, Messdienst, overlay-Wirkungsnachweis im nächsten Torlauf, dann G16/G18. **(b) Sofort geschlossen:** „Ein gescheiterter Merge ist ein Eingriff" — die Regel steht jetzt als eigener Abschnitt im liefern-Skill, Kästchen zu, Registerzeile raus; Belegungs-Entscheid gefällt (es → `bandeja` bei 65:18 — eine Quelle, keine Geschmacksfrage; pt bleibt Bestand bei 69:69, geschlossen mit Begründung statt vertagt); rtree auf dieser Maschine gemessen sauber (nicht installiert). **(c) Ehrlich extern wartend, an Robert als Liste:** DMARC-TXT im CCP, dl/-Archiv-Entscheid, 3D-Drucker-Ordner-Sicherung, Flächenrückgewinnungs-Phase (Konzept liegt), P16.10 und Sonnet-5-Läufe (kosten Geld), Versicherungsangebot, Verkaufs-/Rechtsblock (Vermerk, USt-IdNr, Bestellstrecke, MoR-Vertrag, 15.10.), Feldbestätigungen (Linux-Gerät/Mac/fremder Rechner), 3D-Maus-Gerät, M5-Zitate; die Absturzfamilie (fünf Zeilen) wartet auf eine ruhige Maschine und viele Läufe — unter sechs parallelen Sitzungen und Loop-Rechnungen ist heute das falsche Fenster, das ist eine Entscheidung, kein Vergessen. Resin Stufe 1 ist durch den Panels-Abschluss **entsperrt** und startet als nächste Serie nach dieser Welle. **Takt-1-Stand der Welle:** 50s Menü-Zug im Detail nachgereicht — `tools/site_nav.py` ist jetzt die EINE Quelle für drei auseinandergelaufene Menü-Orte (statische Seiten, make_manual, make_changelog), 40 von 43 Seiten verweisen auf die Börse (vorher sechs, nur auf sich selbst), und **der Insel-Wächter bewies sich beim ersten Lauf selbst**: zweite Insel `offline-aktivierung.html` gefunden — mit Absicht unverlinkt (die Adresse nennt das Handbuch), jetzt belegte statt stillschweigende Ausnahme. Dazu `4b5a5475`, **von Robert direkt bei 50 beauftragt**: das Mobil-Aufklappmenü — auf dem Handy waren zwei von sechs Einträgen sichtbar, jetzt alle sechs hinter einem Griff, dieselbe details-Bauart wie der Sprachwechsel, keine Zeile JavaScript; Lehre daraus: **`display: contents` löst den Kasten auf, und was keinen Kasten hat, kann der Browser nicht verstecken** — wer display benutzt, um etwas sichtbar zu halten, nimmt es dort ausdrücklich zurück, wo etwas verschwinden soll. **50 dreht (1) und (2), bestätigt:** „Der Rahmen-Umbau macht eine funktionierende Seite hübscher; die OS-Erkennung macht eine kaputte Seite richtig" — ein Mac-Kunde bekommt heute 199 MB Windows ungefragt auf die Platte; erst richtig, dann hübsch. Der OFL-Eintrag in EULA.md hängt am nächsten Erzeuger-Commit (ehrlich nachgemeldeter Rückstand). **15s Takt-Stand:** ed6fe730 — die Seite hat vier Kapitel, und der Abstand wusste nichts davon (Regelfall 56→48, Kapitelanfänge 88, Seite 437 Punkte kürzer); dazu ihre ehrliche Fehler-Trilogie als Bestätigung dreier Fallen-Klassen: **eine zu weit gefasste Dateiliste bestätigt sich am eigenen Guard selbst** (eecea53d nahm eine fremde Erinnerung mit, nachgetragen 12a95bd9), ein als Kette gefahrener roter Wächter wurde überfahren (beim zweiten Anlauf brach sie richtig ab), und der /tmp-Meldungs-Wettlauf gab 12a95bd9 eine fremde Überschrift — sitzungseigener Ordner ab jetzt. Entscheide dazu: tote Regel `.hero .download` 39rem wird mit Messbeleg gelöscht (eine gemeldete tote Regel altert nur weiter), die 58rem-Schreibweisen werden vereinheitlicht. **Und mein eigener Verteilfehler des Takts:** G1/G3/G7 waren fertig und G2 gegenstandslos, als ich sie 72 zuteilte — das Register trug es; **auch der Verteiler misst, bevor er verteilt.** 72s app/images-Kopplung an den Paketbau ist bestätigt (jetzt wäre es verlorene Rechenzeit). **Ihr overlay-Wirkungsnachweis ist gefahren, Urteil: fortschreiben, nicht schließen** — test_ui.py riss am Stück bei 107 von 378 (Exit 139, 0 Fehler davor), aber in `op_dialog.py:223` statt `overlay.py:294`: Der oberste Frame ist der Zeitpunkt der nächsten Allokation, nicht die Ursache; die Typprüfung kommt in diesem Riss nicht vor, ein echter Wirkungsnachweis bräuchte ohne/mit-Läufe, die der frühere Riss verhindert. **Die G18-Doppelwirkung fällt damit aus** — die Portionierung bleibt für diese Datei die einzige Fahrweise mit vollständigem Ergebnis. Und die achte Instanz des Tages, erstmals in der **Auswertung** statt in der Messung: Die Hülle meldete „exit code 0" über einer 139 in der Datei, und ihr Zähl-Regex fand ein „F", das ein Traceback-Zeilenanfang war — Rohausgabe gelesen, Lauf war bis zum Riss sauber. Dazu ein Vorfall am Verfahren selbst: **git.exe starb beim Registerzug mit Segmentation fault** (die native Familie der Maschine, erstmals git selbst), Lock des privaten Index blieb zurück und wurde entfernt, der Zug wiederholt — die Ein-Zug-Kette hielt: kein halber Commit, HEAD unberührt. **Takt 2:** 50s `794541a8` (OS-Erkennung, reviewt) — im Markup war nichts falsch (`#download` war immer der Grundzustand), falsch war die Umbiegung in site.js, deren eigener Kommentar es sagte („…ist das die für Windows" — als Reihenfolge-Notiz gemeint, als Kundenaussage wirksam); umgebogen wird nur noch für sicher erkanntes Windows, macOS/Linux führen zum Kasten mit vorangerücktem eigenen System, **Android zuerst ausgenommen — sein Kennzeichen enthält „Linux", wer danach sucht, schickt einem Telefon ein AppImage**; vier Systeme mit gefälschtem Kennzeichen durchgespielt. **Und 50 verortete den Schriftnachweis besser als mein Entscheid** — Impressum statt EULA, offen gemeldet statt still getan: EULA §8 regelt, was mit der Anwendung ausgeliefert wird, die Schriften bekommt der Kunde nie (sie liegen auf dem Webserver); der Nachweis steht jetzt im Impressum mit Urhebern und Lizenzverweisen — angenommen, die genauere Zuordnung schlägt die ursprüngliche Anweisung. **72s vollständige Registervermessung: 190 Zeilen mit Statusspalte, 11 ohne Abschluss** — davon 4 echte Feldbestätigungen (Flatpak-Eingabe, Wayland, Slicer-Übergabe, macOS-CA), Absturzfamilien-Messpunkte, Z6/Z7 und G13 (ungemessen, Schnitt-Regel gilt); ihre eigene Vorzahl („neun Feldbestätigungen") zu ihren Ungunsten berichtigt. Z7 ist in der Zeichenmodus-Statustabelle nachgezogen (war Code-zu, Register-offen — 72s Alterungs-Befund in Reinform), Z6 ebenso mit Vermerk der laufenden Fenster-Gegenprüfung. **SCHILD, DRITTE UND RICHTIGE FASSUNG (d3, `290012c5`): Roberts Direktauftrag „aufhängung nach hinten, von vorne nicht sichtbar" überschreibt meinen Raus-Entscheid** — er wollte die Funktion nie weg, nur die Sichtseite sauber; die Tasche sitzt jetzt an der Rückfläche (`at_feature` mit Normale 0,0,−1 statt frei platziert — die Warnung der Operation dazu stand groß im alten Belegbild und niemand las sie als Befund), `depth` 2,0 statt der 4,0-Vorgabe (die war dicker als das 3-mm-Schild — der Schlitz kam vorn wieder heraus, obwohl der Baustein hinten saß), Titel/doc/Tour/Kataloge zurück. Gemessen: Rückseite −81,5 mm² in z=0, Sichtseite verliert nur die Schrift, Prüfbericht 0/0, Belegbild im Commit. Drei Ursachen, nicht eine — und die Lehre über dem Ganzen: Der fünfmal wiederholte Wunsch hieß nie „weg damit", sondern „von vorn unsichtbar"; **auch die dritte Deutung gehört gegen den Auftraggeber geprüft, nicht nur gebaut.** Dazu aufgeräumt: Der rote `test_directory_docs` (zwei Erinnerungen ohne Indexzeile — im /tmp- und Commit-Wettlauf der Frühe verloren gegangen) ist durch zwei nachgetragene MEMORY.md-Zeilen grün; d3s Beleg-Rendering-Rauschen bleibt absichtlich unkommittet. **Takt 3:** 3as Weg 1 ist fertig und von der Review-Instanz am Bild gesichtet (Lagerbock, vier lesbare Verlaufsschritte, Prüfbericht 0/0/3 mit erzählenden Hinweisen) — **beide Standbilder liegen Robert zur Machart-Freigabe vor**, die bewegte Fassung unter /bilder/weg1-anpassen.webm; 3a hat ehrlich NICHT inszeniert (die saubere STL wurde nicht künstlich beschädigt, damit der Bericht dramatischer aussieht), das Schaustück ist als dritte Bauart in make_showpiece verankert (ein Temp-Skript wäre beim nächsten Klon weg gewesen), und ein alter Zählfehler der Schlusszeile fiel dabei (sie maß eine leere Szene: „4 von 4 Schritten, 0 Körper"). **Entscheid: Videos vor Tabs** — derselbe Seitenbereich, Robert wartet auf Bewegung, der Einbau ist lokal rücknehmbar. **50s `50628e2d` (Rahmen→Flächen, reviewt):** 16 Karten auf das Token `--edge` (themenverschieden mit Grund: Schatten trägt auf Hell, auf Dunkel macht ein feiner Lichtrand oben die Arbeit — Schwarz auf Fast-Schwarz ist nichts), 16 Rahmen bleiben mit Bedeutung; im selben Commit ihr selbst gefundener und behobener **Desktop-Nav-Fehler** (sechs Verweise zwei Commits lang in einer 160-px-Spalte untereinander): drei Lehren — **eine gültige Messung über einen vergangenen Zustand fällt nie auf** (die alte Desktop-Messung war richtig für einen Stand, den es nicht mehr gab; ein Markup-Umbau betrifft jede Lage, die das Markup rendert), **wo Anordnung geprüft wird, gehören Koordinaten ins Protokoll, nicht eine Anzahl** (offsetParent/getBoundingClientRect beantworten „hat es eine Box", nicht „ist es zu sehen"), und gefunden hat es ein Foto; Ursache: Chromium nimmt den Inhalt eines geschlossenen details über content-visibility aus dem Layout, eine display-Regel gibt keinen Platz zurück — das Panel steht jetzt als Geschwister. **15s drei Punkte zu:** `d6fde6c5` (zwei Kanten-Paare auf null Versatz, tote Regel mit Messbeleg gelöscht, die 58rem-min-Klammer war „eigene Vorsicht ohne Messung" und ist vereinheitlicht), Belegung umgesetzt **mit der wichtigen Differenzierung: Nicht jede „Platte" ist die Druckplatte** — von 17 placa/panel-Texten meinen nur 4 die Druckfläche, eine runde Platte aus einer Skizze ist eine Scheibe, eine Lochwand ein Paneel (die 9 bleiben; pt geschlossen wie entschieden), `736cabd8` (D11 Stufe 2: Slicer-Suchaliase aus derselben keys_for-Tabelle wie die Übergabe — ein zweites Verzeichnis würde altern, ohne dass es jemand merkt; Trennschärfe gemessen: „mm" träfe 22 von 56, der breiteste echte Schlüssel trifft 6); dazu zwei gefangene Messfehler (eine Pipe fraß die Umlaute des Schlüssels — Guard meldete 0 und byteweise war alles da; Text-Adressierung über den Anfang scheitert, wenn zwei Texte gleich beginnen). **d3s ki-modelle-Hero ist fertig — und der Befund war nicht der Weißraum: Im Hero fehlte jeder Handlungsaufruf** (223 px Text neben 574 px Bild, null Knöpfe — die Startseite hat dort fünf Teile, die KI-Seite hatte zwei; die Leere war das Symptom); Knopf und Preiszeile wörtlich von der jeweiligen Startseite, Anker in allen sechs nachgesehen, Kicker-Längengrenze gemessen (fr 85 Zeichen brachen um — der Teil fiel weg, den h1 und Lead schon sagen), 348 grün. **Aber: dritter Einsammel-Vorfall des Tages** — 50628e2d nahm d3s sechs fertige Dateien mit (wie zuvor 15s Zeile in c8999e7a); Inhalt vollständig angekommen, nichts nachgebaut, doch die Meldung der History beschreibt fremde Arbeit nicht. **Auflage an 50: Commits nur noch mit expliziter Pfadliste der eigenen Änderungen, kein breites add über website/** — im geteilten Baum ist die Pfadliste die Grenze zwischen eigener und fremder Arbeit. Und 15s Lage-Hinweis: test_print_settings_ui reißt bei 33/113 (auf dem Vorstand identisch — die bekannte Familie), die 80 dahinter einzeln grün; die Zusammenfassung der Datei sagt nichts über zwei Drittel ihrer Tests. **DIE VIDEOS SIND AUF DER SEITE (50, `a1c6892c`, gegengeprüft):** Weg 1 und Weg 2 in allen sechs Fassungen — M2 ist eingelöst, das erste echte Produktvideo statt SVG; Gegenprobe per numstat bestätigt die Pfadlisten-Auflage beim ersten Einsatz (exakt acht Dateien mit Substanz, 36 reine Stempel). Drei begründete Abweichungen vom Markup-Vorschlag: kein `src` am video-Tag (es gewänne gegen die source-Zeilen, der mp4-Rückfall käme nie zum Zug), **`controls` statt `autoplay`** mit Sichtbarkeits-Start per Skript (3as eigene Regel angewandt: automatische Bewegung gehört abgeschaltet, Bedienung nicht — ohne Skript oder bei reduzierter Bewegung bleibt Standbild plus Abspielknopf), Anhalten beim Verlassen des Sichtfelds. `stamp_assets` fing vor dem ersten Betrachter 30 Leerverweise (den Sprachfassungen fehlte das `../` — dieselbe Familie wie der morgendliche /site.js-Fund: ein Pfad bedeutet je nach Ort etwas anderes; diesmal stand ein Wächter davor). **Und die Auflage brachte sofort eine Verfahrens-Lehre:** 50s neuer Dateizähl-Guard meldete beim ersten Lauf 36 fremde Dateien, die es nicht gab — `git diff` ohne Ziel läuft gegen den gealterten **Haupt-Index**, der die privaten Commits nicht kennt und die eigene gelandete Arbeit als offene fremde meldet; **`git diff HEAD`, nie `git diff`** steht jetzt im liefern-Skill. **Takt 4: Die Ø-Bedingung war längst gebaut — die Registerzeile wusste es zwei Tage nicht** (15, `c8c16a16`): alle vier Ebenen nachgemessen, die Zeile ist geschlossen; was wirklich fehlte, war der Rundreise-Wächter über die Zusage „bis in die Projektdatei" — der bestehende Test maß die Rechnung, nicht das Überleben der Art beim Speichern (eine Bemaßung, die beim Schreiben zur Distanz zurückfiele, wäre beim nächsten Öffnen wieder das 6,4-Loch, und keiner hätte es gesehen); jetzt fährt einer lösen→schreiben→lesen→lösen, Gegenprobe rot. Das ist der CLAUDE.md-Fall „Konzept-Statustabellen altern" — **diesmal im Register selbst**, zum zweiten Mal heute (Z7 morgens, Ø mittags). Und 15s Fehlschluss fürs Register: Sie stellte eine „ältere Fassung" nach, indem sie die Art aus der bekannten Menge nahm — die gemeldete Kunden-Schuldzuweisung war eine erfundene Lage, denn eine echte alte Fassung lehnt über die Formatversion vorher sauber ab; **wer eine alte Version nachstellt, stellt sie ganz nach — sonst misst er eine Lage, die es im Betrieb nicht gibt.** Und die Mehrfachtexte-Zeile ist mit einem **Negativbefund samt Messung** geschlossen (15): 3882 Texte, 497 mehrfach — gewünschtes Verhalten bei Parameterbeschriftungen; die gefährliche Gegenrichtung (ein Wort, zwei Bedeutungen) an 45 gebietsübergreifenden Kurzwörtern gezielt geprüft, null Funde — „lohnt keinen eigenen Durchgang" hat jetzt eine Zahl hinter sich statt eines Gefühls, und für einen sauberen Negativbefund kommt kein Hash. 15 sieht sich derweil an, ob test_print_settings_ui in die Fenstergruppe von suite-getrennt.sh gehört — die Datei reißt wie die bekannten drei, steht aber in keiner Liste, und wer sie im Tor fährt, hält zwei Drittel ungelaufener Tests für grün. **DER SAMMLUNGS-BLOCKER, vollständiger Hergang (Takt 5):** Um 01:05 benannte `18e6ae5b` (72) `_NAME_PATTERN` → `NAME_PATTERN` um — sachlich richtig (was zwei Module brauchen, ist nicht privat; `inspect` prüft jetzt dieselben drei Regeln wie `PartRegistry._check`, statt ein abgeschriebenes Muster auseinanderlaufen zu lassen), aber die Reichweiten-Messung „Reste: 0" durchsuchte nur registry.py selbst — `tests/test_recipe_dialog.py` importierte weiter den alten Namen, und **die volle Testsammlung war ab da acht Stunden still gebrochen, ohne dass es jemand sah, weil alle pfadbeschränkt fahren** (jeder Teillauf grün, `--collect-only` über alles Exit 2); aufgefallen erst um ~09:15 (15), entsperrt mit dem Zwei-Zeilen-Nachzug (`ef11e87c`), 8111 gesammelt, Exit 0. Lehren: **Nach jedem Rename einmal `pytest --collect-only -q` über alles — 3,5 Sekunden, und er sieht, was kein Teillauf sieht.** Die Urheber-Suche kostete vier Nachrichten (3a und d3 verneinten mit Numstat-Beleg), **und meine eigene Diagnose war dabei falsch: „ungestaged, halbfertig" — der MM-Status war reines Zeilenenden-Rauschen** (72s Python-Schreibweg hinterlässt CRLF), der Diff gegen HEAD leer; eine HEAD-Messung vor dem Rundruf hätte Urheber und Zustand sofort genannt — der Status beschreibt den Index, nicht den Inhalt. Dazu 72s Such-Warnung: `git log -S` auf eine Zeichenkette, die in beiden Fassungen vorkommt, nennt den **Einführungs**-Commit, nicht die Umbenennung — wer Urheber sucht, nimmt den neuen Namen mit Wortgrenze oder `-G`. Und 15s zwei Beifänge: **Ein xdist-Worker-Absturz schreibt den gerade laufenden Test rot — der genannte Name ist zufällig, nicht schuldig**; und sie maß zunächst mit der grep-Regel, die suite-getrennt seit dem Umbau auf list_windowed_tests.py gar nicht mehr benutzt — eine Regel gemessen, die es nicht mehr gibt. **3as Weg-3-Beifang ist ein echter Kundenfehler im Einleser:** Ein geschlossenes generiertes Netz kam offen aus dem Import — das Entfernen entarteter Dreiecke reißt in einem geschlossenen Netz genau dort Löcher (TripoSG-Beweis: 20 offene Kanten mit, 0 ohne `remove_degenerate`); vier Kundenmeldungen aus einer Ursache, der Nutzer lief im Kreis; Fix als wörtliche Übertragung des Verschweiß-Wächters zwanzig Zeilen darüber, Befund `ingest.degenerate_kept`, Zwillingstest; der zweite Befund (`decimate_mesh` öffnet das Netz) wird nach ihrer Heilungs-Messung ein eigener Punkt. Ihr Commit wartet auf 15s Katalog-Landung. **Nachtrag zur Urheber-Klärung (50), und er verschärft die Lehre:** Die Arbeit lag nie im Baum — die PHP-Hälfte (`c59dbf03`, 01:03) und die App-Hälfte (`18e6ae5b`, 01:05) liegen zwei Minuten auseinander seit acht Stunden auf origin, `shared-rules.json` ist aktuell, kein Erzeugerlauf nötig. **3a und d3 verneinten „mit Beleg" aus ihrem jeweils eigenen Diff — richtig gemessen, falsche Quelle: Der Diff gegen den gealterten Haupt-Index zeigt jedem etwas anderes**, je nach Alter seines Index, und eine längst gelandete Arbeit wäre fast als verwaist verbucht worden. Verfahrensregel daraus im liefern-Skill: **Urheber-Fragen beantwortet `git log -S '<symbol>' -- <pfad>`** (Commit, Uhrzeit, Meldung — ein Beleg statt einer Vermutung), mit 72s Wortgrenzen-Caveat; der Status beantwortet sie nie. Die verbliebenen offenen Stände sind vollständig zugeordnet (figures.py → d3s Schemabild, loader.py/test_ingest.py → 3as Einleser-Fix). **15s Welle-3-Liste ist als erste des Tages leer** (offen nur der Zusagen-Einbau, der auf 3as Bühnenhöhe wartet) — mit einem Beifang aus eigenem Antrieb: **Das Handbuch führte sieben eigene Schriftgrößen in seinem eigenen style-Block** — derselbe Wildwuchs, den die Skala in style.css beseitigt hat, nur in einer zweiten Datei (es lädt die Stufen längst, benutzte sie nur nicht); die Umstellung liegt in make_manual.py und wartet choreografiert (50s Nav-Zeilen zuerst, 15 zieht mit eigenem Commit nach — kein viertes Einsammeln). Ihre Lehre dazu (`d7f26533`): **Ein Befund im eigenen Gebiet endet im Werkzeug eines anderen — bei einer erzeugten Datei fühlt sich der Gebietswechsel wie ein Zwischenschritt an**, weil man nie eine fremde Datei öffnet. Und für alle, die Erinnerungen committen: **Der Index-Wächter prüft beide Richtungen** — ein Zeiger ohne Datei macht den Lauf so rot wie eine Datei ohne Zeiger; beide Richtungen VOR dem Commit prüfen. **d3s Schemabild ist zu (`90e0eaad`), und die offene Frage ist beantwortet statt umgangen:** Die Flächenebene gehört NICHT ins Bild (ein zweiter Eintrag machte aus dem Schema eine Aufzählung, die der Handbuchtext daneben vollständig führt) — was fehlte, war der Wahl-Pfeil am Ebenen-Kasten: Er sah aus wie eine Anzeige, wo der Editor eine QComboBox hat; das Label „Zeichenebene:" wäre richtig und passt gemessen nicht (Endmaße über alle sechs Kataloge belegt). Zwei Messfehler-Lehren: `QFont(name, 10)` sind zehn **Punkt** (bei 96 dpi ein Drittel breiter als `font-size="10"` im SVG), und unter offscreen meldet Qt **null Schriftfamilien** — die Ersatzschrift misst glatte zehn Pixel je Zeichen; **wer Textbreiten misst, prüft zuerst, welche Schrift antwortet.** Ihre Warnung dazu ist eine Choreografie-Auflage: **Ihr make_manual-Lauf hat fremde uncommittete Arbeit (manual.py, es.json — 3as wartende Katalogtexte) in die sechs Handbuch-HTML gebacken** — die liegen ungestaged und werden NICHT committet; der Handbuch-Erzeugerlauf der make_manual-Kette (50s Nav → 15s Schriftstufen) läuft erst, **nachdem 3as Katalog-Commit gelandet ist**, sonst backt er Zwischenstände mit. Und der vierte Register-hinkte-Fall: **P6 und P9 führt das Register selbst als zu** (`eb53d8ce`, `f9059280`) — meine Zuteilung an d3 war erneut ungemessen; sie prüft am Code und meldet. **Weg 3 ist fertig (`d4f7ab8b`): Das Motiv ist eine echte TripoSG-Ausgabe aus Roberts ComfyUI-Ordner** — ohne Maßstab (1,9 Einheiten), zu fein vernetzt, beliebig gedreht, und die fünf Verlaufsschritte sind genau die Antwort darauf; am Ende 0/0/4, 107 g. Drei gemessene Netzbefunde daraus stehen als neuer Abschnitt mit Kästchen (decimate-Mannigfaltigkeit mit Schwellen, der repair↔remesh-Meldungszirkel, Vollzugsmeldung ohne Zahl). **Und eine Richtigstellung mit Ansage (3a): Der Schlussabsatz der `4b6a1d97`-Meldung behauptet fremde Arbeit in den Katalogen — das stimmt nicht** (je Katalog genau eine eigene Zeile; `git diff --stat` ohne HEAD las den 96 Schlüssel alten Index als offene Arbeit, und die eigene Erinnerung „Geteilter Index hält alten Stand" warnt wörtlich davor, dass genau diese Zahl in einer Meldung landet — **eine benannte Falle schützt nicht**, zum wiederholten Mal belegt); wer die Meldung zitiert, lässt den Absatz weg. Offen bei 3a: Weg 4 und die Sprite-Neuaufnahme in 16/7 samt der Bühnenhöhe, auf die 15 wartet. **Takt 6:** 15s zwei Landungen — `44b5be2f` (Handbuch-Schriftstufen über die Choreografie, sechs Fassungen aus einem Werkzeug) und `665cfceb` (Rechtstexte auf 38rem/73 Zeichen; die Lehre dazu ist die Schwester des Guard-Funds: **Ein Kommentar, der eine Eigenschaft behauptet, stellt sie nicht her — und er beruhigt jeden, der ihn liest**: „bleibt schmal" stand seit je über einer Zahl, die 90 Zeichen ergab). Ihr Zeilenlängen-Befund wurde binnen einer Stunde entscheidungsreif nachgemessen: **sieben der acht schlimmsten Fälle sind FAQ-Antworten** (891 px Textlauf in der Klappe — genau wo jemand konzentriert liest) plus der Untertitel; aus dem befürchteten Großeingriff wurden zwei max-width-Regeln, Zuteilung an 15 („die Klappe darf breit bleiben, begrenzt wird der Absatz darin"), 50 bekommt die Selektoren vor dem Commit. **72s Z6-Messung am gebauten Fenster: halb behoben, halb offen** — der Knopf heißt längst „Rechteck" (der Registertext beschrieb einen alten Stand; ihre Vorab-Frage verhinderte, dass jemand Richtiges umbenennt), aber die sechs vollbemaßten Formen liegen weiter hinter dem schmalen Pfeil; sie arbeitet einen Vorschlag mit Bild aus (zwei Kandidaten: Entdeckbarkeit über die Leerzustands-Zeile gegen sichtbare Palette, beide gegen den Acht-Umschalter-Deckel gemessen), Entscheid dann aus Kundensicht, Robert nur bei Deckel-Bruch. **Und zwei Verfahrenssätze:** Die Urheber-Spur einer Notiz ist `originSessionId` im Frontmatter — `git log --format=%an` gibt bei allen dasselbe zurück (meine Fehlzuordnung von 0a9f6ae3 an 72 war der Beleg); und der nächste vollständige make_manual-Lauf committet die sechs angebackenen Handbuchseiten mit einer Meldung, die alle drei Urheber nennt. Der 16-Karten-Kanten-Fund im dunklen Thema ist **behoben (`51c55922`)**, und der Fix trägt seine eigene Weisheit: Der Sammelblock (Zeile 993, Einzelregeln bei 794, gleiche Spezifität) bekam **denselben Wert statt einer neuen Reihenfolge** — „eine richtige Reihenfolge wäre nur bis zur nächsten Änderung richtig"; sein Kommentar hatte behauptet, er stehe früh genug: **Ein Kommentar, der eine Reihenfolge behauptet, ist keine Reihenfolge** — die dritte Instanz der Kommentar-Familie an einem Tag. Der Fundweg gehört dazu: 15 meldete roh, fand einen Fehler in der EIGENEN Messung (veralteter Browser-Stand) und zog zurück — **beides war gleichzeitig wahr**; ohne 50s Rückfrage mit frischem Profil hätte niemand mehr gesucht. Die richtige Meldung heißt „meine Messung war unbrauchbar, der Befund ist damit **offen**", nie „Entwarnung" (15s Notiz 0a9f6ae3 — sie war die Urheberin, per originSessionId belegt). **Und Weg 3 ist eingebaut (`bcd24426`): Drei von vier Wegen tragen jetzt eine echte Aufnahme.** G11 war beim Nachschlagen längst fertig (`e2839acc`, Statusspalte trug es — meine Zuteilung an 50 war der fünfte ungemessene Schnitt); bei 50 bleiben die laufende Unterseiten-Durchsicht in beiden Themen und der Warnprojekt-Nachweis. **Und 15s Muster des Tages, dreimal in drei Dateien belegt** (Rechtstexte-„bleibt schmal", Reihenfolge-Kommentar, Untertitel-Reichweite): **Die Absicht stand jeweils schon da — die Bedingung oder Zahl daneben löste sie nicht ein.** Was man sucht, ist selten eine fehlende Regel, sondern eine vorhandene, deren Reichweite nie nachgemessen wurde. **Z6-Entscheid ist gefallen:** Kandidat (a) mit allen drei Teilen — die Leerzustands-Zeile nennt den Knopf beim richtigen Namen, wird anklickbar (Muster-Grenze in den Code-Kommentar: **Leerzustands-Einladungen dürfen handeln, laufende Statusmeldungen bleiben reine Auskunft**) und erscheint nur im Leerzustand; (b) vertagt, bis (a) gemessen ist; 72 baut, mit §18- und Tastatur-Auflagen. **Takt 7, drei 50er-Landungen reviewt:** `bcd24426` (Weg 3 drin — die Eule steht kopfüber, und die Beschreibung sagt in sechs Sprachen, dass das die stützärmste Drucklage ist: ohne den Satz liest man ein schiefes Bild), `73a934d5` (42-Seiten-Gleichstand; zwei Reste: die Changelog-Seiten trugen noch die abgeschafften Ausblendeklassen — ein Erzeugerlauf holt es nach —, und das Impressum hatte die Börse nicht in der Fußzeile, weil das Skript sie an einen Verweis hängte, den Rechtstexte nicht führen), `016189e5` (Wegkarten-Absätze auf Lesebreite — 15 maß, 50 baute: die Innen/Außen-Grenze trug). **Erzeuger-Entscheid: 50 fährt den nächsten vollen Lauf** (make_manual + make_changelog in einem Aufräumzug — 3as Katalog-Zeilen sind gelandet, die Wartebedingung ist erfüllt) und committet die sechs Handbuchseiten mit der Drei-Urheber-Meldung. **Und ein Fremdalarm-Aufräumer:** `z6t.py` (72s Z6-Prüfskript) lag im Repository-Stamm und machte ruff für alle rot — Lehre: **Ein Prüfskript im Stamm wird mitgeprüft und damit Teil des Projekts; eines im Scratchpad bleibt ein Werkzeug.** **Z6 ist gelandet (`6e1170b5`, 72, reviewt): Die Leerzustands-Zeile ist der Weg, den sie behauptet** — anklickbarer Verweis, Klick/Enter öffnet das Formen-Menü mit Fokus darin, die Muster-Grenze steht in Code UND Test (Leerzustand lädt ein, laufende Meldungen bleiben PlainText und unanklickbar), Kataloge ×5 mit 15s Übersetzungen, 1001 Sprachtests grün. **Ihre erste Testfassung war zu schwach und die Gegenprobe fing es — die Qt-Gestalt der Vormittags-Lehre:** Der Verweis-String steht auch in einem PlainText-Label (dort als sichtbare spitze Klammer), die Mutation RichText→PlainText blieb grün; jetzt prüft der Test `textFormat()` mit — **der String allein beweist nichts.** Das Foto fand den präzisierten Rest: Der Knopf trägt am Bildschirm nur Symbol+Pfeil, `text()` sagt „Rechteck", sichtbar ist es nicht — **Z6b als eigener Punkt angelegt** („nicht die Formen fehlen, der Knopfname fehlt"), Form-Entscheidung nach Messung; die alte Sammel-Kopfzeile der Z-Serie ist damit abgelöst. Dazu 15s `84448dd5`: Die Band-Regel (halbes Polster, wo die Rahmung größer wäre als das Gerahmte) stand einen Tag nur in der deutschen Fassung — fünf Seiten trugen 48 statt 24 Punkte und wurden jetzt kürzer; die Eigenschafts-Lehre steht als neue Falle in `website/CLAUDE.md`: **Wer eine Klasse von Hand in eine Seite schreibt, schreibt sie in eine — Strukturänderungen an index.html werden über alle sechs Fassungen gezählt.** **Kriterium 5 endgültig (72, `7e95ce2f`): Das Bildschirmfoto ist die Instanz, nicht die Absicherung.** Auch `checkVisibility` log einmal — bei der Review-Instanz direkt nach einem Viewport-Wechsel mitten in der rise-Animation UNSICHTBAR bei sichtbarem Foto; 72 konnte es am ruhenden Tab nicht reproduzieren, und genau das trägt die Lehre: **Eine Abfrage, die manchmal lügt, ist als Instanz unbrauchbar — eine, die immer lügt, wäre harmloser, die fiele beim ersten Gebrauch auf.** checkVisibility bleibt Vorfilter (sagt, wo hinzusehen ist), das Urteil fällt am Foto; Kriterium 5 meldet fortan zwei Zahlen („von der Abfrage gemeldet" / „im Foto bestätigt"), und dass die zweite kleiner ist, ist der Normalfall, kein Widerspruch. Fünfte Instanz des Zahl-beschreibt-etwas-Benachbartes-Musters an einem Vormittag, erstmals mit der falschen Zahl bei der Review-Instanz statt bei 72 — es liegt nicht an der Sorgfalt des Messenden: **Diese Werkzeuge beschreiben einen Zustand, der sich unter ihnen bewegt; wer zweimal dasselbe misst, misst zwei Momente.** **Verteilung Runde 2 (konkretisiert):** 3a die drei Interaktions-Bugs, **danach M2: die vier Weg-Loops erzeugen und einbetten** (encode_loop und Drehbücher existieren — die Videos ersetzen zugleich die SVG-Cartoons in #wege und geben M12 endlich seine beweisende Bewegung); 50 Börsen-Leerzustand + Bildgrößen ×6 + **M11: die Markenschrift** (selbst gehostetes woff2, style.css ist ihre — „modern" hängt an der Typografie vor jedem Wort); d3 das Schild-Motiv neu; 15 Leerraum/Rhythmus + **M3: das Kaufmodell in den Hero-Kopf** („Einmal kaufen. Für immer deins." o. ä. — die größte Kaufangst zuerst); 72 nach dem Abgleich frei für Review-Verstärkung. Offen: Roberts Freigaben. **Hook-Schärfung vorgemerkt (72, nach dem Schnitt):** Die pre-commit-Frage wird von „legt dieser Commit tr()-Texte an" zu „steht einer der **fehlenden** Texte in einer Datei dieses Commits" — 3as messungsgestütztes KEIN_TOR (fremde Texte in fremdem Modul) war die dritte legitime Abschaltung der Nacht, und die Sorte wird zur Gewohnheit, wenn der Hook die genauere Frage nicht stellt. **Nächste Strecke d3: `make_web_images.py`** — die Verkaufsseiten-Belegbilder (`beleg-*.png`) sind vom **27.08.**, also vor Statusleiste, Akzent-Reform, Titelleiste, D-Serie und Beleuchtung; dieselben Fallen wie eben (eine Sprache je Aufruf, echte Plattform, Sichtprüfung), danach folgt mein make_manual+stamp-Zug; **Und 3as Gehäuse-Fix (`bcca1207`, reviewt): vier Gruppen statt einer** — Dome, Füße, Rastnasen, Rippen, alle auf die Vorgabe 120×80 gerechnet, jetzt Formeln vom Rand/aus der Breite; gemessen 70–200 mm **und beide Materialien** (50s Dosen-Lehre angewandt), 50 kann die Testspanne 110→80 senken. 50s verallgemeinerter Satz steht: **Eine Zahl aus dem Materialprofil in einer Formel ist derselbe Fehler wie eine aus der Geometrie — sie sieht hergeleitet aus.** WB10 liegt mit beiden Farbwelten am neuen Hero bei Robert (Kontrollfälle grün); bleibt offen bis zu seiner Antwort. 3as App-Hälfte hat ihren ehrlichen Zuschnitt (der Kern kann beides längst — for_upload hinaus, adopt_payload herein mit §32-Ausweis; es fehlen zwei Katalog-Knöpfe und zwei Fenster-Handler nach dem saveRequested-Muster), ihre Klärfrage ist mit dem D5-Muster beantwortet: **Der Veröffentlichen-Knopf ist bei eingebauten Bausteinen gesperrt mit Grund** (Tooltip/Statuszeile/Bildschirmleser — „Nur eigene Rezepte lassen sich veröffentlichen"), nie versteckt und nie folgenlos klickbar. **Offene Richtungs-Entscheidung 72+50, Richtung der Freigabe: findings sechssprachig über dieselbe Ein-Quelle-Bauart** — die Prüf-Befunde tragen `code`-Schlüssel, beide Anzeigeorte übersetzen aus derselben Quelle (App via tr(), Seite via shared-texts.json), damit App- und Server-Anzeige je Sprache wörtlich gleich bleiben; **3a** nach WB10 die App-Hälfte (Katalog-Export/Import von Rezepten, Kennzeichnung fremder Herkunft §32, Größengrenze, Hochlade-Bestätigung §3.6); **d3: Der Rezept-Kern ist zu (`f3e1d9cf`, reviewt)** — `license`/`author` an `Recipe`, beide **nicht** im Fingerabdruck (wer eine Lizenz korrigiert, ändert eine Angabe über das Teil, nicht das Teil; die Kehrseite — außerhalb der Versionsdeckung — steht ehrlich im Docstring), `FORMAT_VERSION` bleibt 1 (optionale Felder mit Vorgabe, Test öffnet eine Datei ohne sie), `RECIPE_LICENSES` liegt im Kern und die Börse importiert (richtige Richtung). **Die Feldnamen-Entscheidung wurde an 72s Argument gedreht, gemessen besser:** Der Feldname IST der Schlüssel (`dataclasses.fields` → shared-rules.json → PHP), also **eine** Schreibung `license` statt Datei/Code-Trennung; nur der Parametername weicht wegen Builtin-Schattung ab. §4.2 zieht 15 entsprechend nach. Freigabe-Stichprobe: 878 Tests (language_rules + recipes) grün. **Dazu der Bezeichner-Nachzug (`f20bc7c2`), größer als gemeldet:** sechs deutsche Bezeichner statt einem, und mein GERMAN_STEMS-Vorschlag wurde gemessen verworfen — `anker` als Stamm träfe `banker`/`tanker`, fünf Wörter stehen als GERMAN_WORDS (ganzes Segment), nur `laeng` als Stamm; Gegenprobe hält neun englische Namen frei. `oben` in tools/upload_website.py wartet auf die Freigabe der Release-Datei. **d3s Anschluss ist zu (`4931d7c4`, reviewt):** Lizenz im Klartext im Dialog („Namensnennung, und Abwandlungen unter derselben Lizenz" statt CC-Kürzel — die Kennung reist in der Datei), nichts vorgewählt, `RECIPE_LICENSES` aus `LICENCE_LABELS` abgeleitet (keine zulässige Lizenz ohne Erklärung), neun Katalogtexte im selben Zug, 88 Tests in eigener Stichprobe grün. **Dazu die Revision des Abends: Der Lokalisierungs-Wächter war KEIN Fehlalarm** — drei Sitzungen (72, Freigabe, fast 15) hielten die panels-Treffer für harmlos, d3 las die Kette zu Ende: Die Breitenmessung wird mit dem **lokalisierten** `spin.text()` verrechnet; behoben mit gemessener Null-Wirkung im Kommentar statt einer plausiblen Behauptung. 15s Meta-Lehre dazu steht: **Die auferlegte Gegenprobe („echter Nutzer-Text bleibt verboten") hätte bestanden — sie prüft die andere Richtung; gefunden hat es nur, dass jemand die Stelle las.** Eine Ausnahme, die genau den einen echten Fall deckt, kommt durch ihre eigene Absicherung. Dazu d3s mypy-Fund (TranslatableText-Reexport ohne Export — vier grüne Testläufe sahen ihn nie) und 15s gezählte Übersetzungsdurchsicht (36 britische licence als Substantiv gegen 6 US als Verb/Eigenname; zwei echte Bedeutungsfehler im Italienischen). **Betrieb der Nacht:** Der Haupt-Index lag 74 Rücknahme-Einträge zurück (mehrere Commits alt, von d3 gemeldet statt angefasst) — per `git reset` (mixed) auf HEAD gesetzt, alle Sitzungen committen privat, niemand verlor Staging. **15s Stempellauf ist durch und `test_website` steht auf 300/300** — der komplette Website-Testblock ist grün, §4.2 trägt die drei Lizenz-Ebenen (`031e0a18`). 3as Schaustück v2 ist gelandet (`33c55c51`, reviewt — gesteckte Achsen, die Dreifälle-Gegenprobe stand im Commit **vor** der Umkonstruktion). **Das §3.6-Rot ist zu (`bdfe5864`, 72):** Die Nummer stimmte, das Wort fehlte — „Konzept §3.6", wie fünf Schwestern in derselben Datei; 15s Zählung (es gibt gar keine Bauplan-§3.x-Ebene) schloss die verrutschte Ziffer aus. In eigener Stichprobe grün. **Und die Kür am Mengen-Riss trägt einen Befund, der die Zuschreibung dreht** (`72bdd31d`, `.claude/.state/mengen-riss-2026-08-30/BEFUND.md`): Nichts wächst — Handles, Arbeitssatz, gc-Objekte flach, der Fensterpin sammelt nicht an, tote Wrapper null. Der Riss liegt nicht im Teardown, sondern im **Aufbau des nächsten Dialogs**, und der oberste Frame wandert zwischen drei Stellen, die alle **Allokationen** sind: das Bild einer Heap-Beschädigung, die erst beim nächsten größeren Block zuschlägt. **Der Satz dazu ist Regel: Wer die nächste Zuschreibung an eine Codezeile schreibt, schreibt einen Zeitpunkt auf** — die Zeile ist unschuldig, sie fasst nur als Erste den beschädigten Heap an; darum halfen gc, deleteLater und der Pin nie. Zwei Verdächtige erledigt (Messwerkzeug per Ohne-Plugin-Gegenlauf, Sprachwechsel per --deselect), und das Werkzeug war zuerst selbst der Fehler (ctypes ohne argtypes schneidet das Prozess-Handle ab — aufgefallen allein an der **konstanten** Zahl, danach an bekanntem Ausgang geeicht). **Nächster Schritt Page Heap, mit Auflage: NICHT während parallele Sitzungen bauen** — `gflags /p python.exe` trifft jeden Python-Prozess der Maschine; der Lauf gehört in eine ruhige Stunde. 72s panels-Selbstkorrektur steht dabei: „Zwei Zeilen sind nicht die Funktion — ich hatte die Grenze meiner Messung nicht mitgemeldet, sondern eine Schlussfolgerung"; **72: Die Prüfseite ist zu (`4a23613d` + Umbenennung `0e397f42`, reviewt)** — `shared.py` prüft auf **Erlaubtes statt Verbotenes** (eine Sperrliste muss vollständig sein, eine Erlaubnisliste ist abgeschlossen), nichts darin ist aufgezählt (Ops aus dem Register, Schlüssel aus der Dataclass, Version aus dem schreibenden Modul), und `make_shared_rules.py` schreibt die **eine Quelle** `website/api/shared-rules.json`, die auch 50s PHP liest — der geplante Zwei-Listen-Wächter ist damit gegenstandslos, es gibt keine zweite Liste. Zwölf Missbrauchsfälle als Tests plus die Gegenprobe, ohne die alles nichts sagte: der Normalfall kommt durch. Drei Wächter (Datei aktuell, vollständig, Werkzeug läuft als eigener Prozess — daran war eine Erlaubnisliste schon einmal leer geblieben). Freigabe-Stichprobe: 12/12 grün, Werkzeug-Frischeprobe im eigenen Prozess gefahren, erzeugte Datei unverändert. Der `exchange`-Umweg (eine Stunde unter dem verworfenen Namen, weil im Konzept statt in §4.2 gesucht) kostete vier Dateien und zwei Handgriffe — nach der PHP-Seite wären es zwei Sitzungen gewesen; und ein zu breites `git add -A tools/` hätte 3as laufende Arbeit mitgenommen — der Ref-Lock-Wettlauf war diesmal der Schutz. **Offener Anschluss bei 72: Wer ruft `inspect`?** Ein Prüfer ohne Aufrufer ist die Kette, die am letzten Glied endet; **15** nach Z5/Z6 Datenschutz-Kette (make_legal, §3.4: Mailadresse und Browser-Kennung in datenschutz.html **bevor** der erste Kommentar möglich ist) und alle Börsen-Texte ×6. **15s Vorabprüfung hat vier Kaperungen verhindert, bevor sie entstanden:** `Teilen` liefert heute `Split` (Geometrie!), `Suchen`→`Find`, `Neu`→`New`, `Geteilt` ebenso belegt — der deutsche Text ist der Schlüssel, ein zweiter Verwender mit anderer Bedeutung erbt die falsche Übersetzung, und kein Test schlägt an (der Schlüssel ist ja übersetzt; die Frage „bedeutet es dasselbe" ist ohne Absicht nicht stellbar — darum Vorabprüfung statt Wächter). **Die Regel steht:** Ein deutsches Wort mit zwei Bedeutungen kann im Katalog nur eine haben — wer die zweite braucht, wählt einen **längeren eindeutigen Wortlaut** („In der Tauschbörse teilen" ist besser als „Teilen" *wegen* seiner Länge, nicht trotz ihr). Fünf Schlüssel sind schon richtig belegt und werden benutzt statt neu angelegt (Autor, Lizenz:, Baustein→Part, Teil, Rezept→Recipe); 27 Kandidaten gegen 3827 Schlüssel gemessen, zehn belegt; 50 und 3a haben die Alternativen direkt. Upload-Sperre gilt unverändert; jede Landung Review bei der Freigabe. **d3s licence-Messung ist da: Der Datei-Schlüssel heißt im Bestand `"license"`** (serialise.py schreibt ihn so in die Projektdatei), der Code-Bezeichner bleibt `licence` — Datei folgt Datei, Code folgt Code, 3a baut darauf. **Erste Börsen-Landung: die Datenschutz-Kette (`d688e649`, 15, reviewt)** — der §3.4-Absatz steht (Like: zufällige Browser-Kennung im localStorage, Zweck/Dauer/Löschweg; Kommentar/Upload: bestätigte Mailadresse, nicht angezeigt, Rückzieh-Link mit Schlüssel; „Was Sie selbst schreiben, ist öffentlich"; „Ein Konto entsteht dabei nicht"). Der eigentliche Fund lag daneben: Der Bestandsabsatz versprach „keine Cookies, keine Kontaktformulare" — die Verneinungs-Suche fand ihn, der neue Absatz hätte ihm widersprochen; präzisiert statt gestrichen, die App-Zusagen (kein Konto, keine Telemetrie) bleiben unberührt wahr. Die drei §4.2-Begriffe stehen im Bauplan (shared/flag/licence, als HEAD-Blob neben d3s ungestagtem P9-Text). `website/CLAUDE.md`-Lücke geschlossen (datenschutz.html **und** impressum.html waren in keiner Liste). **Auflage an Robert vor Veröffentlichung:** Rechtsgrundlagen und Fristen des neuen Abschnitts einmal lesen (Art. 6 lit. b/f DSGVO, § 25 TDDDG, Sieben-Tage-Frist ist 15s Vorschlag) — er ist der Verantwortliche, wir sind keine Rechtsberatung. **Der Anschlusspunkt (b) ist zu (`a0e67668`, 15, reviewt):** `DATENSCHUTZ.md` ist die Quelle, `make_legal.py` erzeugt die Seite als viertes Dokument. Die Kette lernte Sprungmarken (`{#anker}` — greift nur, wo eine Marke steht, die drei Geschwister inhaltlich unverändert) und ein Verträge-Feld: Der englische Sprachhinweis gilt nur Kaufdokumenten, nicht einer Erklärung für Nicht-Käufer — **vermutlich der Grund, warum die Datei nie in der Tabelle stand: sie passte nicht in die Form, und statt die Form zu erweitern, hat sie jemand danebengelegt.** Roberts Leseauflage steht jetzt als Entwurfshinweis im Dokument selbst. Freigabe-Stichprobe: test_website 299 grün, genau die eine bekannte Stempel-Stelle rot (style.css trägt 50s laufende Arbeit; der Stempellauf kommt nach deren Landung, dann null). Zweite Phantom-Mine des Abends bei einer neuen Datei, von der Kontrolle gefangen — das Netz hält |
| G17 | **Dreimal an einem Tag dieselbe Falle** (D9 mittags, `f1ddb4ca` abends, 50s Z9-Baustand): Ein neuer `tr()`-Text ist im eigenen Diff unsichtbar, weil die fünf Kataloge nicht zu den angefassten Dateien gehören — die Ansage-Zahl prüft die Absicht, nicht die Änderung. Der strukturelle Schluss: ein pre-commit-Hook neben dem bestehenden post-commit, der bei Berührung von `app/` die zwei Sprachprüfungen fährt (`test_translations`, `test_language_rules` — zusammen ~10 s) und rot abbricht; was drei Sitzungen je einzeln als Reflex beschlossen haben, gehört einmal ins Werkzeug. **Gebaut** (`89e122c5`): `.githooks/pre-commit`, ~10 s, `SOLIDON_KEIN_TOR=1` schaltet ab. Die Sollprobe mit sechs bekannten Ausgängen fing drei falsche Anläufe — der wichtigste: Bei fremder Arbeit im Baum nannte er die eigene saubere Datei als Verursacherin, weil pytest auch bestandene parametrisierte Dateien listet; gesucht wird jetzt nur in Fehlerzeilen, sonst wäre der Hook nach dem zweiten Fehlblock dauerhaft abgeschaltet worden. Nebenfund als alte Schuld: `post-commit` stand mit Modus 100644 im Repository — Linux und macOS überspringen ihn **stumm**; beide Hooks auf 100755, und der neue Wächter fragt HEAD statt des alternden Index. Und die Regel aus dem einen Ausrutscher (zwei Probe-Commits gingen kurz als Zweig nach origin, gelöscht und nachgeprüft): **Eine Probe, die Commits erzeugt, schaltet den Push ab** | S | **fertig** |
| G16 | **Einunddreißig Zeilen sagen, dass sie warnen, und sehen aus wie Text** (Fund aus der B26-Vermessung, 72): `set_level` wird 31-mal mit Stufen gerufen, die `LEVELS` nicht kennt (`warning` 15×, `info` 11×, `ok` 4×, `note` 1×) — pixelgleich mit gewöhnlichem Text gerendert, denn eine Qt-Eigenschaft ohne Selektor ist still; darunter die drei Muss-Zustände des Freischaltdialogs. Beschlossen nach 72s Einschätzung: Die vier Stufen sind **Lautstärke**, Bedeutung bekommt einen zweiten Mechanismus — `set_role` neben `set_level`, der Farbe **und** Zeichen setzt und aus derselben Quelle liest wie der Prüfbericht (Regel 18; eine fünfte Stufe könnte nur Farbe). 31 Stellen in mindestens sechs Dateien; der Wächter, der `set_level` auf `LEVELS` festnagelt, kommt **nach** dem Umbau, sonst macht er 31 Läufe rot. Der G6-Anteil „Ablaufdatum → Warnfarbe" geht hierin auf — gemessen trug das Datum nie einen Akzent, die Durchsichts-Beobachtung war gealtert Dazu aus B26 die zwei Lautstärke-Entscheidungen: Die Anleitung des Freischaltdialogs steht als `caption` (leiseste Stufe), obwohl sie der wichtigste Satz ist, und „Solidon kaufen" ist ein Nebenknopf. **G16 ist zu (`377c6a1f`, reviewt):** `set_role(widget, role, text)` setzt Text und Bedeutung in **einem** Aufruf — in der Lücke zwischen `setText` und `set_level` war die Kodierung verlorengegangen; `set_level` wirft jetzt bei Bedeutungen einen Handlungsvorschlag Richtung `set_role`, Bestand: caption 33, section 8, title 4, body 1, null ungültige. `ROLE_MARKS` in `palette.py` liest warning/info aus `SEVERITY_ENCODING` statt sie zu wiederholen; `ok` bekam nach der Freigabe-Entscheidung **Zeichen ohne Farbe** (U+2713, auf echter Plattform gemessen darstellbar — offscreen ist Zeichenmetrik Phantomschrift), Begründung im Docstring, ein Test wird rot, wenn jemand ein Grün nachrüstet; `note` wurde `caption` (Nebentext, keine Stufe). **Der Bau drehte die Farbfrage um, gemessen:** Rollenfarbe als Fließtext bringt 3,76/4,28 gegen WCAG-4,5 — die Farbe trägt jetzt das Zeichen (Grafik-Grenze 3,0 hält), der Satz bleibt Fließtext (9,39/13,46), dieselbe Bauart wie die Befundzeile im Prüfbericht. Bildschirmleser bekommt das Wort, nicht das Zeichen. Freigabe: Diff gelesen, `tr("Erledigt")`-Katalogdeckung eigens geprüft (war vorhanden — die Fünf-Dateien-Falle griff nicht), 193 Tests (`test_translations` + `test_style`) in eigener Stichprobe grün. **Daneben ein Bestandsbefund, jetzt mit Basisrate (72) — und die Fremdlast-Hypothese der Freigabe fiel an der Messung:** `test_print_settings_ui.py` reißt auf reinem `origin/main` in ruhiger Maschine **7 von 7** (null fremde pytest-Prozesse, per Wächter bestätigt), immer nach 33 von 112, Zugriffsverletzung, null `FAILED` — Bestand, kein G16, keine Fremdlast. Der Riss hängt an der **Sammelmenge, nicht an einem Test**, und nicht monoton: 23 reißt (127, nach vollständigem Durchlauf), 24–34 laufen, 40 reißt bei 37 (139), 112 bei 34 (139); der Test an Stelle 34 läuft allein grün. Halbierung ist damit als Methode untauglich (kein einzelner Auslöser — `native-bibliotheken-speicher`). Ursachensuche ist ein eigener, nicht begonnener Punkt; unmittelbar wichtig ist die G18-Folge: **Portionsgröße 60 ist eine Beobachtung an `test_ui.py`, keine Konstante** — für diese Datei waren 24–34 sicher. Anschluss bei d3 (siehe G18-Zeile) | M | **zu — 72, reviewt** |
| G13 | Kleinserie: Filamentkarten-Höhe, Befehlspaletten-Spalten, Statuszeilen-Verschmelzung, Startbildschirm-Resthöhe, Filamentdialog (Farbfeld winzig, „OK"), Verlauf ohne Symbole, Menü-Icon-Lücken, Handbuch-Ränder, helle Kanten, sprachabhängiger Symbol-Schwellwert (B21/B23/B24/B27/B28/B31/B33/B34/B35/B40) | M | **zu** (Verlauf): B35 ist zu (`5a921d07` — die Ursache war die Platte, nicht die Kante: Körper/Fläche 2,05→3,36, dunkles Thema und Kante unangetastet, Unmöglichkeits-Rechnung im Code; Belegbilder beider Themen liegen vor und sind von der Freigabe angesehen — die Trennung steht, B35 ist endgültig zu). **Neu aus dem Bilder-Ansehen (3a): die Themen sind unterschiedlich plastisch** — im dunklen zeichnet die Schattierung die Innenwände deutlich, im hellen wirkt der Körper flächiger; das ist eine Beleuchtungsfrage (ambienter Anteil je Thema), nicht eine Farbfrage, und läuft als eigener kleiner Punkt bei 3a nach B21 — Messung zuerst, die frischen B35-Kontraste bleiben unbewegt. **Plastizität ist zu** (`b0a72e14` — die Vermutung „ambienter Anteil je Thema" trug nicht: Es gibt keinen, die Beleuchtung ist in beiden Themen dieselbe. Ursache ist Multiplikation — der Körper ist im hellen Thema 2,45-mal dunkler, also sind alle Helligkeitsunterschiede auf ihm 2,45-mal kleiner (0,0155 gegen 0,0380 zwischen zwei Außenwänden). Ambient und Glanz gemessen und verworfen (ambient macht flacher: 1,19 → 1,12); getragen hat das Frontlicht, 0,25 → 0,45 im hellen Thema, +39 % absoluter Wandunterschied, dunkles Thema bitgleich unverändert. In derselben Reihe der Schatten-Auftrag: 0,18 ergab 1,44 Kontrast auf heller und 1,05 auf dunkler Platte — auf Roberts Entscheid „wie im dunklen thema reicht" jetzt 0,03 im hellen, gemessen 1,06. Jedes themenabhängige Paar trägt jetzt drei Zusagen: Richtung der Werte, `set_theme` setzt sie, die Zeichenstelle liest den gemerkten Wert — eine Mutationsprobe hatte gezeigt, dass „was die Methode tut" nichts über „dass sie gerufen wird" sagt. **Folge für die Auslieferung: die Bildschirmfotos in `app/images/` sind veraltet, `/erzeugen` vor dem nächsten Paketbau**); Roberts Schatten-Entscheid dazu: Der Kontaktschatten bleibt — er ist Auskunft, nicht Schmuck (§18.6). **B21 ist zu** (`a381e717` — der Befund war größer als notiert: Bei zwölf Spulen im Regal schob die aufgeklappte Karte ihren Hinweis und **beide Knöpfe** aus dem Bild, also den einzigen Weg zu einer neuen Spule, und drückte den Verlauf auf drei Zeilen (102 von 262 gewollt). Zwei Ursachen: Die Karte war kein `RoomTaker` und nahm sich, was sie wollte (614 statt 255); und `fit_to_rows` rechnet mit *einer* Zeilenhöhe mal Zeilenzahl, was bei fetten Zwischenüberschriften zu wenig ist — 172 gebraucht, 156 gesetzt, die letzte Zeile fehlte auch bei freiem Platz ringsum. Der Test fand beim ersten Lauf einen dritten Fall, das Nachziehen einen vierten (der verlorene `MAX_ROWS`-Deckel). **Fünf von sechs Sonden maßen etwas anderes als den Befund** — drei die zugeklappte Karte, zwei mitten in der Animation; `processEvents` lässt keine Zeit vergehen, die fünfte Gestalt in `qt-pruefstand-misst-zu-frueh.md`. Belegbilder vorher/nachher liegen vor und gingen an Robert); **B23 ist zu** (`4ceb179c` — die Morgenzahlen waren zu freundlich: real 475 Punkte Spanne durch Qts Tabstopp-Raster; ein Zeilenzeichner setzt Kürzel rechtsbündig, Restspanne 13 Punkte aus der Zeichenbreite). **B24 ist zu** (`c7765da4` — senkrechte Trennlinie zwischen den Auskünften in Status- und Kopfzeile, der Mittelpunkt trennt weiter innerhalb einer; der Commit nahm ~18 Zeilen von 50s laufender G11-Arbeit mit — `commit -o` statt Blob, Zurechnung falsch, Stand heil, 50 informiert). **B31 ist zu** (`a6d197d4` — Symbole aus derselben Quelle wie Menü und Katalog, unbekannte Ops bekommen keines statt eines falschen; Nummern für alles, was genau einen Schritt vertritt, weil der Fehlerdialog „Operation: 4" sagt und die Zahl die Verbindung ist; die zwei Punkte Zeilenhöhe je Symbol stehen als bewusste Abwägung im Code); B26 läuft (72): zwei Teilbefunde widerlegt — das Schlüsselfeld ist zu **klein**, nicht zu groß (242 Zeichen brauchen 110 Punkte, gesetzt sind 90; ein QLineEdit zeigte 18 %), und das Ablaufdatum trug nie einen Akzent; **B26 ist zu** (`22ea55ca` — die Feldhöhe folgt dem Inhalt aus der Schriftmetrik, Rollweg null, der ganze 242-Zeichen-Schlüssel steht im Bild; die Gegenprobe fand dabei eine wirkungslose, plausibel kommentierte Zeile im eigenen ersten Anlauf: „eine Änderung, deren Rücknahme nichts rot macht, war nicht die Ursache"). Der Katalog-Sofort-Fix `ea41b319` machte origin wieder grün (142 passed, verifiziert), und 72s Verfahrens-Konsequenz gilt ab jetzt: test_translations vor jedem app/-Commit — die Ansage-Zahl prüft die Vollständigkeit der Absicht, nicht der Änderung, und ein neuer tr()-Text ist eine Änderung an sechs Dateien. Die zwei Lautstärke-Teile (Anleitung als leiseste Zeile, „Solidon kaufen" als Nebenknopf) gehören zu G16 — dieselbe Frage: welche Zeile wie laut; B33 ist komplett zu (`249d4241` + `c1ccc90d`) — und die Nachmessung korrigierte die Entscheidungsgrundlage: Bearbeiten war mit 0 von 8 in sich konsistent, der Sprung liegt **zwischen** den Menüs (Qt reserviert die Spalte je Menü: 16 gegen 36 Bildpunkte Texteinzug). Die Entscheidung trug mit erweiterter Reichweite: je ein Zeichen mit echter Metapher auch für Ansicht und Hilfe (redo gespiegelt, manual, about), sonst wandert der Sprung nur; das Zahnrad für „Einstellungen" wurde gezeichnet und verworfen — im Strichstil wird es eine Sonne, und Deko ohne Metapher bleibt verboten. Der Dreifach-Testfund dazu ist der allgemeinste des Tages: **Die Suite fährt ohne Stylesheet** (apply_style steht nur in app.py) — wer Einrückung, Breite oder Farbe misst, stellt die Betriebslage selbst her); B28 ist zu (`3430c7d0` — die Spulenfarbe ist so groß wie ihre Listenzeile; die „OK"-Hälfte war gegenstandslos, der Knopf hieß längst „Übernehmen"); B27 ist zu (`473a5b16` — ausbalanciert statt gefüllt: Die Spalte sitzt mit 1:3-Bias über der Mitte, und die Bilder entschieden die Zahl gegen die bessere Klangzahl — „69 % klingt besser als 62 %, sieht aber schlechter aus"; die viermalige Tour-Zeile steht einmal über der Gruppe und bleibt je Kachel beim Bildschirmleser, der Handbuch-Knopf nennt seine Handlung, der Satz steht im Hinweis; ein Nachbartest zog an den neuen Ort). B34 und B40 sind zu (`5719d4c7` — der Handbuchtext läuft auf 80 Zeichen statt 96, mitwachsender Rand; 80 statt der typografischen 60–70, weil Code und Tabellen die enge Fassung nicht vertragen. Qt-Fund darüber hinaus: `setViewportMargins` zieht die Dokumentbreite nicht mit, der Text hätte waagerecht gerollt — wer eine Textspalte begrenzt, setzt beides. Die Verzeichnis-Kürzung war korrekt, es fehlte der Weg zum ganzen Namen — der Hinweis trägt ihn. B40 als Mechanismus dokumentiert, nicht als Befund: Ein fester Umschaltpunkt wäre in genau einer Sprache richtig). **Damit ist die G13-Kleinserie komplett zu** |
...
## Befunde aus dem Weg-Dreh (31.08.2026)

Beim Dreh des dritten Weg-Videos lief eine echte TripoSG-Eule durch die
ganze Kette, und drei Befunde blieben übrig — alle gemessen (3a), keiner
...
- [ ] **`decimate_mesh` erzeugt bei starker Reduktion nicht-mannigfaltige
      Kanten.** Gemessen an derselben Eule: 150 000 und 100 000 Dreiecke
      sauber, bei 60 000 zwei Kanten mit drei Nachbarn, bei 30 000 vier.
      Der Prüfbericht meldet es korrekt („Der Körper war geschlossen und
      ist es jetzt nicht mehr") — die Ursache steht.
- [ ] **Der Meldungs-Zirkel zwischen `repair` und `remesh_uniform`.** Ist
      ein Netz aus anderem Grund offen, schickt die Reparatur den Nutzer
      zu „Kanten verfeinern", und das antwortet „Erst reparieren" — zwei
      Meldungen, die aufeinander zeigen, und der Nutzer läuft im Kreis
      (Verstoß gegen den Geist von Regel 17: ein Handlungsvorschlag, der
      zurückführt, ist keiner).
- [ ] **`repair` meldet Vollzug ohne Zahl.** „Offene Stellen wurden
      geschlossen" erschien auch, als vierzehn von zwanzig zu waren —
      die Meldung nennt weder Anteil noch Rest.
- [ ] **Der Prüfbericht wird von einem Befundtyp geflutet.** Das
      Weg-4-Schaustück (geformter Griff) endet mit 622 Befunden, fast alle
      `perceive.orphaned` — die info-Schwere ist in `evaluate.py:1170`
      bewusst und überzeugend begründet, die **Menge** ist dort unbedacht:
      Jede formende Operation verliert jedes erkannte Merkmal, auf einem
      fein vernetzten Körper Hunderte. 622 gleichlautende Zeilen sind so
...
- [ ] **Das elfte Beispiel: „Wenn eine Passung nicht mehr passt" (50, im
      Bau).** Der Überhang-Zuschnitt fiel an der Messung, bevor eine Stunde
      ...
      Ausgangslage" gibt es mit Überhang schlicht nicht. Die verletzte
      Passung dagegen meldet sich bei jeder Auswertung (§14) und IST der
      Inhalt, der Solidon von einem Mesh-Editor unterscheidet: Deckel und
      Dose, ein verstelltes Maß, die Warnung beim Öffnen, der Fix ist eine
      Zahl. Auflagen: Wächter-Ausnahme als scharfe Zusicherung, Tour ×6,
...
- [ ] **Ein generiertes Modell zu öffnen kostet eine Minute — der Punkt ist
      ...
      **Zweite Schrumpfung (15, nachgemessen am gezeigten Fenster): Die
      Ansage EXISTIERT und ist gut** — Ladeschleier durchgehend, Balken,
      benannte Schritte, ab zehn Sekunden Restzeitschätzung; §2.8 ist
      eingelöst (ihr erster Messort war Qts leere statusBar statt des
      eigenen status_message-Labels — ein leeres Widget, das niemand
      benutzt, sah aus wie ein Befund). Und die 61,77 s waren
      Reihen-Fremdlast: **einzeln 9,4–18,5 s** (vier Läufe). Was bleibt:
      die gemessene Ursache (orient_for_print auf ungefiltertem Netz) und
      **eine Produktfrage an Robert statt eines Fehlers: Sind zehn bis
      zwanzig Sekunden für das Öffnen eines generierten Beispielprojekts
      in Ordnung?** Ursprünglich (3a, 31.08.): Am
...
## Die Zeilen laufen zu lang (31.08.2026)

Gemessen bei 1440, Zeilen über die Zeilenhöhe gezählt, das Werkzeug an einem
Fall mit bekanntem Ausgang geprüft (15). Lesbar sind 45–75 Zeichen; die
Rechtstexte stehen nach `665cfceb` auf 73. Die übrigen Textseiten:

| Seite | Zeichen je Zeile |
|---|---|
| funktionen.html | 89 |
| Startseite | 101 |
| ki-modelle.html | 101 |
| boerse.html | 134 (ein Absatz gemessen) |

- [x] **Zu (`be1ed2be`, 15):** FAQ-Antworten auf 70ch (96 → Median 72
      Zeichen), Untertitel auf 56ch — dort war es keine neue Regel, sondern
      eine **Reichweiten-Lücke**: Die Begrenzung existierte längst, galt aber
      nur unmittelbar hinter h1/h2, und drei Untertitel hinter h3 oder Liste
      liefen über 1385 Punkte. Zwei lange Wegkarten-Absätze sind benannt an
      50 übergeben (Karteninneres). Beinahe-Fehler dank weitergegebener
      Messfalle vermieden: Zugeklappte Antworten liegen über
      content-visibility außerhalb des Layouts — jede eingestellte Breite
