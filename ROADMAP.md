...
| Zwei zusätzliche Aktionen auf der App-Startfläche | Rückmeldung und freiwillige Unterstützung gehören in die App-Startfläche (31.08.2026) | **Rückmeldung** öffnet den vorhandenen App-Dialog; **Freiwillig unterstützen** zeigt zuerst lokal den PayPal-/Datenschutzhinweis und öffnet PayPal erst nach einem zweiten Klick. Auf der Website kein Fragebogen, Formular, Link zu einem Website-Feedbackfluss oder neue Erhebung. Live-Abnahme in beiden Themen bei 1920/1040/800/640, großer Schrift, Tastatur und QAccessible |
| Datenschutztexte zum App-Rückmeldungsweg | Rückmeldung und freiwillige Unterstützung gehören in die App-Startfläche (31.08.2026) | Hilfeweg, 30-Minuten-Einladung und Startfläche vollständig nennen; keine Website-Erhebung behaupten. Transparent beschreiben: Der App-Transport begrenzt Anforderungen über ein privates, zweckgetrenntes HMAC-Pseudonym; nur die zugehörigen Support-Anforderungszeiten bleiben höchstens 3.600 Sekunden erhalten |
| Roberts Ton-Direktive auf den Bestand | Rückmeldung und freiwillige Unterstützung gehören in die App-Startfläche (31.08.2026) | das Durchgehen von Website, Handbuch und Oberflächentexten gegen die sieben Merkmale aus `.claude/memory/nicht-nach-ki-klingen.md` — der Maßstab ist vom 31.08.2026 und der Bestand nie daraufhin gelesen; an alle vier Sitzungen weitergegeben |
...
| Vorgabe „auf das Bett setzen" beim Import | Review vor der Demo 0.3.0 (02.09.2026) | Roberts Entscheidung: (b) `place_on_bed=True`, wenn die Datei keine Bettkoordinaten mitbringt (nicht bei 3MF aus Slicern) — empfohlen; (a) Vorgabe bleibt; (c) nur der Import vom Startbildschirm setzt auf |
| Prompt-Budget des lokalen Modells neu messen | Review vor der Demo 0.3.0 (02.09.2026) | eine Messung mit qwen3:14b über `/api/chat`: `backends/llm.py` führt `PROMPT_TOKENS = 19641` als gemessene Grundlast, und das kompakte Werkzeugschema ist am 02.09.2026 um rund 7 000 Zeichen gewachsen, weil es Einheit, Bedingung und die *Wann nicht*-Zeile nicht mehr abschneidet — die Wartezeit, die dem Kunden vorgerechnet wird, ist damit um grob 1 700 Token zu kurz. Raten wäre falsch; die Zahl gehört am laufenden Modell neu abgelesen |
| Fremdes GLB kommt liegend herein | Review vor der Demo 0.3.0 (02.09.2026) | eine Entscheidung mit Migrationsweg: glTF ist Y-up, Solidon Z-up. Der Export dreht seit dem 02.09.2026 richtig (`export/writer.py`), der Import (`geom/mesh.read_mesh`) bewusst nicht — derselbe Leser liest den eingebetteten Payload jedes Weg-3-Projekts bei jeder Auswertung, und der TripoSG-Knoten schreibt Rohkoordinaten ohne Drehung; eine Drehung dort kippte jedes bestehende Projekt beim nächsten Öffnen, und die Lage steht in keinem Parameter, den eine Migration umschreiben könnte. Weg: `format_version` erhöhen, GLB-Quellen im Projekt mit einem Lagekennzeichen versehen, den Leser erst dann standardkonform drehen (Durchsicht Einlesen/Export, 02.09.2026) |
| Aushöhlen: eine gewählte Seite öffnen | Review vor der Demo 0.3.0 (02.09.2026) | Roberts Beispiel vom 02.09.: ein Puppenhaus, Räume ausgehöhlt, die Vorderseite offen. *Oben öffnen* nimmt heute nur die Decke (`hollow.py`, `open_top`); der Weg zur offenen Seite ist Aushöhlen plus eine Tasche oder ein Schnitt durch die Wand. Gewünscht: „Öffnen an: <Fläche>“ als Merkmalsparameter statt des Schalters, *Deckel erzeugen* findet die Öffnung dann wie bisher |
| Regel 23 — Verständlichkeit für Laien | Review vor der Demo 0.3.0 (02.09.2026) | Roberts Ansage für `AGENTS.md`: „Jeder sichtbare Text nennt die Sache, die der Kunde meint; ein Fachwort steht nur, wo der Kunde es im Slicer wiederfindet" — testbar als kuratierte Sperrliste (Mesh, Manifold, Boolean, Voxel, Tessellation) über alle Kataloge |
| Bauplan-Nachträge aus dem Review | Review vor der Demo 0.3.0 (02.09.2026) | Roberts Ansage je Punkt: Absatz „Für wen" vor §1 (Produktkompass §1.2), §2.2 mit Weg 1 als Einstieg und um Reparatur/Orientierung/Teilen/Übergabe verlängert, §2.5 mit der unteren Werkzeugzone, §18.5 Klarname statt `hole_3` im Bild, §43 ohne „P9 abgenommen", §12 ohne „0.2.2", §11.3/§35 eine Zahl für die Zufallsstellen, §22.3 ehrlich auf Achsen bis zur T-Serie, §25/§42 und die AGENTS-Zeile zu Verrundungen auf Netzen, ein eigener § für die Slicer-Übergabe (56 Einstellungen, 146 Begriffe), §37.2 mit HMAC-Pseudonym und Frist, §4.2 mit Spalte „in der Oberfläche", die Liste „Was NICHT gebaut wird" um „kein Netzwerkdruck an die Maschine" |
| Übergabestatus als Serie? | Review vor der Demo 0.3.0 (02.09.2026) | Roberts Entscheidung, ob die fünf Erlebnisse des Produktkompasses (§4.1–4.5: Druckziel, Übergabestatus, Befundkarte, Änderungsvorschau, Übergabebeleg) eine Serie werden — bisher ohne Kästchen und ohne Produktaussage |
| CRA vor dem 11.09.2026 | Review vor der Demo 0.3.0 (02.09.2026) | drei Schritte außerhalb des Repositorys: EU-Login, Registrierung von RS Digital auf der ENISA-Meldeplattform (Robert als Primary, eine zweite erreichbare Person als Secondary) und den Probelauf aus `SECURITY-INCIDENT.md`. Die Recherche vom 02.09. (`marketing/kanzleianfragen/ANTWORTEN-2026-09-02.md`, Punkt 1) hält die Demo für bereits „auf dem Markt bereitgestellt"; Fristen, BSI als CSIRT und der Satz zur Meldung ab 11.09. stehen in `SECURITY-INCIDENT.md`, `SECURITY.md` und den sechs Sicherheitsseiten |
| Verkaufskonzept vor dem 15.10.2026 | Review vor der Demo 0.3.0 (02.09.2026) | ein kurzes Konzept (Preis, Merchant of Record, Bestellstrecke mit der Zwei-Klick-Zustimmung nach § 356 Abs. 5 BGB, Signierung), das `konzept-veroeffentlichung-1.0.md` ablöst — das Demo-Konzept verweist dorthin, der Index führt es als überholt |
| Rechtstexte: was vor dem Verkauf gebraucht wird | Review vor der Demo 0.3.0 (02.09.2026) | die Antworten in `marketing/kanzleianfragen/ANTWORTEN-2026-09-02.md`: § 356 Abs. 6 Nr. 2 BGB ist umgesetzt (Belehrung mit vier Voraussetzungen); offen sind Roberts Telefonnummer fürs Impressum (zweiter Kontaktweg nach EuGH C-298/07), die Wahl des Zahlungsanbieters und damit die verkäuferneutrale Fassung von `AGB.md` § 3/§ 4/§ 6 und `WIDERRUF.md`, die schriftliche Klärung der Widerrufsfunktion nach § 356a BGB beim Anbieter, die TMview-Recherche „Solidon" in den Klassen 9 und 42 mit anschließender Wortmarke (DE 290 €, EU 900 €) |
| LGPL-Quelltextnachweis im Bau | Review vor der Demo 0.3.0 (02.09.2026) | `build.yml:594/1140/1511` verlangen `build/release-evidence.json` (Qt, OCCT, GEOS, AppImage-Runtime), kein Schritt erzeugt sie — klären, womit 0.3.0 gebaut wird, und die Erzeugung in den Workflow oder in `/erzeugen` |
| Löschlauf auf dem Server belegen | Review vor der Demo 0.3.0 (02.09.2026) | `cleanup_private_state.php` als geplante Aufgabe in Plesk mit den drei Pfaden, einen Lauf protokollieren, Häkchen in `PRIVACY-COMPLIANCE.md:156` — die Zusage „höchstens 62 Kalendertage" hängt nur daran |
| PHP-Endpunkte: kleine Härtungen | Review vor der Demo 0.3.0 (02.09.2026) | nur noch einen eigenen Rate-Key statt `activation_seed()` als Pseudonymwurzel (`activation_common.php:203`, braucht eine neue Schlüsseldatei auf dem Server). Erledigt 02.09.: sodium-Prüfung vor Rumpf und Ratenbegrenzung in `activation.php`/`deactivation.php`, `stats.php` antwortet bei nicht öffnbarem Zähler 503 statt „Zu viele Versuche", Test-Schlüssel aus der Umgebung nur unter `cli-server`, `support.php`-Docblock nennt `appdata/` 0700 |
| Kern: Restpunkte aus dem Review | Review vor der Demo 0.3.0 (02.09.2026) | `_read_linked_source` (falscher OSError-Text, `require_checksum` bei leerem Hash) vor dem Verknüpfungsweg; „Deckel"/„Prüfstück" mit Zähler; `_fell_apart` einmal statt zweimal. Erledigt 02.09.: die Summengrenze beim Speichern liegt beim Doppelten der Einzelgrenze (`project.py`), Autor und Lizenz im Katalogdetail baut der zweite UX-Agent. Aus den Durchsichten vom 02.09.: `NOISE_VOLUME` in `scene/compare` aus dem Profil statt 1e-3 mm³ (Faktor 35 zur Düse; Rezept in `boolean.py:506`, Aufrufer `ui/session.py`); `BRIDGE_FROM` und `WIDTH_INTERESTING` der Schichtanalyse als Parameter, sobald ein Aufrufer sie aus dem Profil füllt; `support_on_model` als Feld in `SliceResult`, damit der Durchgang einmal statt zweimal läuft; der Menüeintrag *Druckeinstellungen …* → *Drucken und slicen …* (Handbuch, Tour, sechs Kataloge) |
...
| `geom/mesh.py` kennt `export/threemf.py` | Architektur-Durchsicht (02.09.2026) | Roberts Entscheidung, wohin der 3MF-Leser gehört — die untere Schicht ruft heute die obere, träge und begründet |
| `backends/scripted.py` reist im Kundenpaket | Architektur-Durchsicht (02.09.2026) | Roberts Entscheidung: ein Test-Double im Kern, weil Bauplan §35/§40 die Agenten-Suite darüber fährt |
| 22 Kernfunktionen über 150 Zeilen | Architektur-Durchsicht (02.09.2026) | die Messung mit `--durations` an `test_parts.py` (3d-druck-7b) — erst dann steht fest, ob `has_self_intersections` (806 Zeilen) die vier CI-Stunden erklärt |
---
...
- [~] Doku, Website, Lizenzhinweise — README mit Erwartungsmanagement, den drei
      ...
      **Auffindbar erst seit dem 20.08.2026.** Die Seite stand zwölf Tage
      ...
      von außen sind geprüft. Offen bleiben DMARC und der belegte Abschluss des
      Auftragsverarbeitungsvertrags im CCP. Die öffentliche Aussage, der AVV
      bestehe, ist bis zu diesem externen Beleg nicht freigabefähig. Der Umfang
      reicht über Seitenabrufe hinaus: Mail und Anhänge, App-Rückmeldungen,
      Aktivierung, Protokolle/Statistik und Sicherungen liegen ebenfalls bei
      netcup. Ein Zahlungsdienstleister für den späteren 1.x-Verkauf ist in den
      aktuellen AGB nicht benannt; Anbieter und Rollenverteilung bleiben bis
      zum Vertrag offen. PayPal für freiwillige Unterstützung ist davon
      getrennt und eigenständig Verantwortlicher. Entwurf bleiben die
      Rechtstexte bis zur fachlichen Prüfung
...
- [x] **Zwei Pakete lösen den Deadlock noch nicht auf**, und beide aus
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `32bac7ec`, `e62eeb03`; `test_core_isolation.py` fährt den Wettlauf ohne Ausnahmeliste.**
...
- [~] **Resin Stufe 1: entschieden und beratschlagt — bauen, als
      ...
      0.3.0-Zyklus, Paketschnitt bei Serienstart.
...
- [x] **Erledigt in zwei Stufen, und das Register hinkte zum dritten Mal an
      ...
      erst beim 0.3.0-Zug, sonst zeigte die Website eine ungebaute
...
- [x] Die zwölf Pakete abarbeiten — je Paket Zahlen, Gegenproben und
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — alle vierzehn zu, zuletzt D6 `2d7c46ea`.**
...
**Roberts Entscheidung zu T1 ist noch kein Produkt-Iststand:** Die öffentliche,
gehostete Tauschbörse soll entfallen; übrig bleiben soll ausschließlich der
verlustfreie lokale Rezeptdatei-Export und -Import mit Autor, Lizenz,
Herkunft, Größen-/Integritätsprüfung und ohne ausführbaren Quelltext. Auf HEAD
`aa3d84f0` bestehen `website/boerse.html`, `website/api/shared.php` und die
optionale Community-Tauschstelle aus dem vorrangigen Bauplan §24.5 jedoch
weiter. Die Entscheidung liegt deshalb nur als uncommittierter, noch zentral
zu reviewender und commitpflichtiger Bauplan-/Code-Kandidat vor. Erst ein
gemeinsam freigegebener Bauplan- und Code-Commit darf Galerie, Uploads,
Serverdownloads, Likes, Kommentare, Profile, Konten, Moderation und
Bestätigungsmails als entfernt ausweisen.
...
- [ ] **`decimate_mesh` erzeugt an 3as Eule nicht-mannigfaltige Kanten —
      ...
      nicht reproduzierbar, generated_figure war Vererbung (8 kranke Kanten
      schon im Eingang — kein Punkt). Liegt bei 3a. Die dritte Sache ist
      **behoben** (`0d1edb8b`): `SIMPLIFY_MISSED` maß gegen den **Eingang**
...
- [ ] **Der Reduzierer erreicht bei Euler-0-Körpern sein Ziel nicht** —
      ...
      `0d1edb8b` erfährt der Kunde es wenigstens. Was fehlt, ist eine
      **Entscheidung**: anderer Reduzierer oder Nachbearbeitung — kein
      Commit, eine Abwägung (Lizenz Regel 15/22 bei jedem Kandidaten).
- [x] **Der Meldungs-Zirkel zwischen `repair` und `remesh_uniform`.** Ist
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `63b2d4c8`.**
...
- [x] **`repair` meldet Vollzug ohne Zahl.** „Offene Stellen wurden
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `63b2d4c8`, `repair.py:398`.**
      geschlossen" erschien auch, als vierzehn von zwanzig zu waren —
      die Meldung nennt weder Anteil noch Rest. **Statusbeleg 31.08.:** Ein
      uncommitteter Kandidat deckt Reparatur, Bericht und den unmittelbaren
      Wiederholungsweg mit vier gezielten grünen Fällen ab. Der Punkt bleibt
      bis zum zentralen Review und einer ausdrücklichen Commit-Freigabe offen;
      aus dem Zwischenlauf wird keine Release-Aussage abgeleitet.
- [x] **Der Prüfbericht wird von einem Befundtyp geflutet.** Das
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `f3f3f993`, `7761ad88`.**
...
- [x] **Das elfte Beispiel ist gebaut (`39c595bc`, 50): „Wenn eine Passung
      nicht mehr passt"** — alle drei Auflagen eingelöst: die scharfe
      Wächter-Ausnahme als benannter Code mit Begründung („ist der Inhalt
      des Beispiels, nicht sein Fehler"; dabei eine Doppel-Verbotsliste im
      Nachbartest aufgelöst — zwei Listen für dieselbe Frage laufen
      auseinander), Tour ×6 in vier Schritten (gesiezt wie die zehn
      Nachbarn; nennt auch den zweiten Lösungsweg, denn **ein Beispiel, das
      nur eine Lösung kennt, erzieht zum Rückgängigmachen**), und der
      Materialwechsel als letzter Schritt rücknehmbar vorgeführt
      (Warnung → Strg+Z → Bericht grün, mit Abschlussprüfung). Die Passung
      steht bewusst auf nacktem `auto:` (die Geschichte IST der
      Materialwechsel; §12-Begründung im Bau). Die Beispielzahl stand in
      **sieben** Dokumenten — die Sechs-Fassungen-Zählregel fing sie alle.
...
- [x] **Kundenfehler aus der Abnahme des elften Beispiels (31.08., an
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `36884af2`, `lid.py:461`.**
      ...
      **(1) Der Test meldet die Zahl der geprüften Texte mit (heute 58) und
      ...
      nicht bricht.
- [x] **Der Klick auf eine Warnung fliegt die Kamera ins Leere — dritter
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `8d15960f`.**
...
- [x] **Die Flughöhe nach einem Befund-Klick: entschieden auf 1,4 × die
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `9c6bc85d`, `main_window.py` `reach=1.4 * diagonal`.**
      ...
      **Wenn zwei Läufe verschieden antworten, ist die Frage nicht „welcher
      ...
      eine allein.
- [x] **Ein `return` macht aus drei gleichzeitigen Stufen ein
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `8b24dc54`.**
...
- [x] **`sculpt_strokes` meldet Vollzug, wenn nichts getroffen wurde**
      **Zu (02.09.2026, Review vor 0.3.0): am Code belegt — `b6ecf9ca`, `sculpt.py` `no_effect`/`strokes_missed`.**
      (1b, 31.08., gemessen am Weg-4-Rezept): **Statusbeleg 31.08.:** Vier
      gezielte Wirkungsfälle sind im uncommitteten Kandidaten grün
      (`misses`, `no_effect`, unter Schichthöhe und Gegenprobe). Der Punkt
      bleibt bis zum zentralen Review und der Commit-Freigabe offen. Die Züge sitzen **18 mm über
...
- [ ] **61 Prozent der mehrzeiligen Texte der Startseite laufen über 75
      ...
      Die Punktfixes von heute (FAQ, Untertitel, Rechtstexte, funktionen,
      ki-modelle) haben ihre Stellen zu; dies ist die **Grundsatzfrage
      dahinter**: eine Grundregel auf `main p, main li` mit bewusster
      Ausnahmeliste für Karten- und Spalten-Kinder, gemessen an den
      längsten Fassungen (70 ch ≠ 70 Zeichen). Entwurf bei 15 **nach** T1,
      Abstimmung mit 50, Nachmessung 72. Und eine Register-Klarstellung
      zur Fußnote: `3d9ed2d0` hebt `d6fde6c5`s 736er-Angleichung dort
      **bewusst** auf (450 statt 736 — „die Kante hatte gewonnen, der Text
      verloren"); wer später 450 gegen die alte Zusage misst, findet
      keinen Fehler, sondern die jüngere Entscheidung.
...
| Paket | Kern | Größe | Stand |
|---|---|---|---|
| T1 | Nahtlage nach Kanten und Krümmung: Krümmungsterm entlang der Schnittkontur in `_judge`, dazu die Einschnürungs-Heuristik („Naht nicht am Querschnittsminimum") — löst den `caveat` der Op ein | M | **zu** (`2c21323f`, 15, reviewt): Auto Split legte die Naht BEVORZUGT in den kleinsten Querschnitt — kein Versehen, sondern Folge der drei Terme (die erste Ableitung ist am Minimum null, eine Kerbe sah aus wie ein prismatischer Abschnitt und bekam an der Hantel die beste Punktzahl überhaupt); die Einsicht war das enge Suchfenster, in dem Taille und Mulde ununterscheidbar flach sind — über die volle Achse trennt eine zählbare Eigenschaft (mehrere Punkte am Minimum = Strecke = gut, genau einer = Tal = meiden), 13 Schnitte, 3 ms, Suche 6,2 → 7,1 ms, deterministisch. **Der Testkörper brauchte drei Anläufe, dreimal grün ohne zu prüfen** (Spitze-an-Spitze: nichts zu schneiden; gleicher Hals: keine Kerbe im Fenster; steile Verjüngung: fängt der alte Term) — erst die **sanfte Mulde** traf die Lücke, Gegenprobe NOTCH_WEIGHT=0 jetzt rot. **Abwägungs-Entscheid zum nicht gebauten Kantenterm** (er läuft dem Prismenterm entgegen — Naht auf Kante ist ein Querschnittssprung): erst den Testkörper bauen, der eine Sichtkanten-Naht von einer Freiflächen-Naht bei gleicher Prismatik trennt und den Kundennutzen zeigt; existiert er nicht überzeugend, bleibt der Term draußen und Kriterium 3 wird über T8 (die Sichtflächen-Geste) eingelöst statt automatisch — kein Term ohne Test, der ihn hält (15s eigene Drei-Anläufe-Lehre) |
...
## Rückmeldung und freiwillige Unterstützung gehören in die App-Startfläche (31.08.2026)

Roberts Klarstellung ersetzt den früheren Website-Entwurf vollständig: Der
Fragebogen ist die bestehende **App-Rückmeldung** aus *Hilfe* und der Einladung
nach 30 Minuten. Derselbe Weg kommt zusätzlich auf die Startfläche der
Anwendung. Auf der Website entsteht daraus nichts: kein Fragebogen, kein
Formular, kein Link zu einem Website-Feedbackfluss und keine dortige
Datenerhebung. `website/api/support.php` bleibt ausschließlich der vorhandene
Transport der Anwendung und ist kein öffentliches Webformular.

Die Startfläche erhält exakt zwei zusätzliche, klar getrennte Aktionen:

- **Rückmeldung** öffnet den vorhandenen `KIND_SURVEY`-Dialog. Inhalt,
  Freiwilligkeit, Fehlerbehandlung und Versandweg werden wiederverwendet; es
  gibt keine zweite Erhebung und keine abweichenden Texte.
- **Freiwillig unterstützen** öffnet zuerst einen lokalen Hinweisdialog. Erst
  ein zweiter ausdrücklicher Klick öffnet PayPal extern. Der Text nennt PayPal
  und dessen eigene Datenschutzverantwortung. Kein „Spenden“-Wortlaut, keine
  Gegenleistung, Freischaltung, Kaufanrechnung oder Zuwendungsbestätigung.

- [ ] **Beide Aktionen auf der echten Startfläche bauen und live prüfen:**
      dunkles und helles Thema bei 1920×1080, 1040×760, 800×600 und 640×720;
      große Schrift, 44-Punkt-Ziele, vollständige Tastaturbedienung,
      QAccessible-Namen und keine Überlagerung. Der App-Wächter muss den
      vorhandenen Rückmeldungsdialog nachweisen; der Website-Wächter muss
      Fragebogen, Formular und Website-Feedbackfluss verneinen.
- [ ] **Datenschutztexte an den tatsächlichen App-Ort nachziehen:** Hilfeweg,
      30-Minuten-Einladung und Startfläche nennen; keine Website-Erhebung
      behaupten. Die Missbrauchsschutz-Verarbeitung des App-Transports
      transparent beschreiben: Anforderungen werden über ein privates,
      zweckgetrenntes HMAC-Pseudonym begrenzt; nur die zugehörigen
      Support-Anforderungszeiten bleiben höchstens 3.600 Sekunden erhalten.
- [x] **Kopfzeilenpaket freigegeben und gelandet:** `84e65dc5` bewahrt Name,
      Maßeinheit, Plattenzustand, Drucker und Material bis 640×720. Der
      Root-Review war ohne P0–P3-Befund; 13 fokussierte Tests, Sprachregeln,
      Ruff, Format und Mypy sowie die native Windows-Oberfläche sind grün.
...
## Review vor der Demo 0.3.0 (02.09.2026)

Dreizehn Review-Agenten haben am 02.09.2026 vor der Demo 0.3.0 jeden Bereich
durchgesehen — Kern, Oberfläche, Sprache, Website, Changelog, Register, Tests,
Bauplan, Bedienung, Regelwerk, Rechtliches, Konzepte, Auslieferung. Was sich
am selben Tag beheben ließ, ist behoben und steht im Commit; hier steht, was
bleibt. Die Rangfolge ist die Kundenwirkung. **Entscheidung Robert (02.09.):
TripoSG ist nicht gesperrt und wird benutzt, solange die Rechtsprüfung nichts
anderes ergibt** — der Punkt dazu bleibt unter P9.

- [ ] **Vorgabe „auf das Bett setzen" beim Import.** Wartet auf Roberts Entscheidung: (b) `place_on_bed=True`, wenn die Datei keine Bettkoordinaten mitbringt (nicht bei 3MF aus Slicern) — empfohlen; (a) Vorgabe bleibt; (c) nur der Import vom Startbildschirm setzt auf.
...
- [ ] **Regel 23 — Verständlichkeit für Laien.** Wartet auf Roberts Ansage für `AGENTS.md`: „Jeder sichtbare Text nennt die Sache, die der Kunde meint; ein Fachwort steht nur, wo der Kunde es im Slicer wiederfindet" — testbar als kuratierte Sperrliste (Mesh, Manifold, Boolean, Voxel, Tessellation) über alle Kataloge.
...
Robert: „prüfe die gesamte Architektur und mach es sauber". Gemessen wurde
mit einer Sonde über den Importgraphen von `app/` (242 Dateien, 161 000
Zeilen): Richtung der vier Schichten, Zyklen ohne `TYPE_CHECKING`, Waisen,
identische Funktionsrümpfe, Namen ohne Aufrufer, Funktionen über 150 Zeilen,
Kernmodule ohne Testimport, die drei Listen der sieben Lazy-Pakete. Der Baum
ist in guter Verfassung: null eifrige Zyklen, ein Richtungsverstoß, kein Qt
unter `core`, kein `eval`, kein `random` ohne Startwert.

- [x] **Behoben am 02.09.2026.** `app/i18n/catalog.py` holte seinen Logger aus `app.core.log` — die Schicht, die an nichts hängen soll, hing am Kern (`logging.getLogger` direkt, derselbe Name). Die drei TOML-Leser mit Regel-17-Fehler (`profiles`, `print_settings`, `calibration`) sind einer in `knowledge/tables.py`. `DeviceActivationRequired` und `DeviceDeactivationPending` trugen denselben `__init__`, um einen Vorgabetext zu setzen — `AppError.default_detail` ist jetzt das Muster neben `default_title`. `types.Millimetres` war ein Alias, den keine Signatur benutzte. `certificate.parse_deactivation`, der Python-Spiegel der PHP-Prüfung, hatte keinen Aufrufer und keinen Test — jetzt einen Rundweg und eine Manipulationsprobe. `session_board._common_dir` war eine Kopie aus `gate_lock`. Drei Wächter, die es vorher nicht gab: `test_layer_direction.py` (alle vier Pfeile der Schichtenkarte, auch träge und für Typen), `test_lazy_exports.py` (jeder `_EXPORTS`-Eintrag löst auf, steht in `__all__`, passt zum `TYPE_CHECKING`-Block), `test_affected_tests.py`. Und ein Werkzeug für Roberts Anweisung „nur die betroffenen Tests fahren": `tools/affected_tests.py` liest den Importgraphen rückwärts statt zu schätzen.
- [ ] **`geom/mesh.py` kennt `export/threemf.py`.** Wartet auf Roberts Entscheidung, wohin der 3MF-Leser gehört. `read_mesh` in `geom/mesh.py` ruft träge `export.threemf.read_objects`, weil trimesh Komponenten mit externen Objektdateien falsch auflöst; `threemf.py` importiert eifrig `geom.mesh.MeshData`. Die Karte sagt „`ingest/` liest, `export/` schreibt", und `ingest/ops.py` ruft denselben Leser aus `export/`. Der Zyklus ist träge, begründet und läuft — aber die untere Schicht kennt die obere. Sauber wäre ein Codec-Modul unterhalb beider (`geom/threemf_codec.py` oder `ingest/threemf.py`), das `export/threemf.py` nur noch fürs Schreiben lässt. Ein Umzug von 1414 Zeilen, nicht nebenbei.
- [ ] **`backends/scripted.py` reist im Kundenpaket.** Wartet auf Roberts Entscheidung. Das Modul ist ein Test-Double („Ein Modell, das sagt, was man ihm aufgetragen hat"), importiert von sieben Testdateien und keiner Anwendungsdatei; `app/CLAUDE.md` sagt „Nichts hier ist ein Hilfsprogramm". Bauplan §35/§40 fährt die Agenten-Suite darüber, und `tools/run_agent_suite.py` braucht es womöglich außerhalb der Suite — dann bleibt es und bekommt in `backends/CLAUDE.md` den Satz, warum. Sonst nach `tests/`.
- [ ] **22 Kernfunktionen über 150 Zeilen.** Wartet auf die Messung mit `--durations` an `test_parts.py` (3d-druck-7b, 02.09.2026). Die Spitze: `range_check.has_self_intersections` 806 Zeilen (acht innere Funktionen), `evaluate._with_features` 520, `evaluate.evaluate` 472, `part_file._strict_shape` 332, `range_check.large_mesh_intersects` 286, `handover.slice_model` 231, `mounting.pegboard_hook` 229. Keine davon ist falsch, alle laufen — aber `range_check.py` ist der Bereichstest jedes Bausteins, und die CI hängt in `test_parts.py` vier Stunden bei 49–52 %. Erst die Verteilung sagt, ob die Länge auch die Zeit ist; für heute verteilt 7b die Datei in der CI auf `-n auto`. Ein Umbau geschieht mit Messung davor und danach, nicht nebenbei.
