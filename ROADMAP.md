...
| Neun Z-Pakete des Zeichenmodus | Der Zeichenmodus und der Viewport bekommen ihre Durchsicht (30.08.2026) | die Abarbeitung — Z1 bei 50 (Escape vernichtet die Zeichnung; entschieden: merken statt vernichten), dann Maß-ändern und die Ausgänge; Review je Paket |
| Neun V-Pakete des Viewports | Der Zeichenmodus und der Viewport bekommen ihre Durchsicht (30.08.2026) | die Abarbeitung — V1 (`7d46a87f`) und V4 (`4c584b51`) sind zu, V2/V5/V7 bei 72 (Commits fallen mit dem Tor), V8 bei d3; Review je Paket |
...
| D14a | Roberts Order („materialauswahl und farbe sind sinnlos, da wir nach den filamenten gehen"): Das Farbfeld sollte fliegen — **die Messauflage fing, dass es nicht ersatzlos kann**: `slicer_keys.py:234/340` bildet es für beide Slicer-Familien ab, `handover.py:1285` überschreibt nur bei bemalter Spule — das Feld ist der Rückfall für den ungefärbten Fall, gestrichen verlöre der Slicer die Farbe. Die Kopfzeilen-Felder verschwinden trotzdem, aber mit der D14b-Ableitung statt ersatzlos | S | **geht in D14b auf** (gemessen, 15/53) |
| D14b | Material und Farbe ergeben sich aus den Spulen des Projekts — Roberts Präzisierungen (direkt an 15, 30.08.2026): kein Filamentwähler in der Kopfzeile („warum ein filamentwähler? es sind ja die die dem material zugeordnet sind", „kann ja mehrere geben") — die Richtung ist Filament→Material, mehrere Slots = mehrere Materialien, und der Rückfall „noch keine Spule gewählt" wird ausdrücklich benannt statt still zu verwaisen. Kernberührung: Dokumentfeld, Migration, auto:-Toleranzkette, Passungen, Override-Dialog | M | **Konzeptnotiz vor dem Bau** (15 schreibt sie, Review bei der Freigabe); Baueinsatz nach Notiz-Review |
...
| Paket | Kern | Größe | Stand |
|---|---|---|---|
| Z1 | **Schwer**: Escape und „Verwerfen" vernichten die Zeichnung ohne Rückweg und ohne Ansage — die Formsitzung nebenan entscheidet umgekehrt („die teuerste Taste des Programms"). **Entschieden (Kundensicht): Variante b** — die verworfene Zeichnung wird gemerkt, die Statuszeile nennt den Rückweg, Strg+Z holt sie zurück; keine Nachfrage, keine geänderte Geste | M | **50, beauftragt** — mit Ansage an 3a (die erhebt gerade lesend den Zug in `sketch_editor.py`) |
