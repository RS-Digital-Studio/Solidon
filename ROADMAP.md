...
| Handbuch, SEO und Stempel vor dem Upload neu erzeugen | Review vor der Demo 0.3.0 (02.09.2026) | **am 02.09.2026 gelaufen** (`make_manual.py` → `make_changelog.py` → `make_seo.py` → `stamp_assets.py`, sechs PDFs in `Releases/`). Offen bleibt `make_figures.py`: Das Ansichtsmenü hat eine Zeile mehr (*Druckplatte zeigen*), und die Bilder zeigen den alten Stand — vor dem Upload einmal fahren, danach `stamp_assets.py` erneut |
