...
| V8 | Leisten bei Mindestbreite messen (sechs Sprachen, echtes Fenster) — die fehlende Messung, kein Befund. **Roberts Order dazu (30.08.2026)**: „wir können das ganze panel ruhig noch breiter machen, damit es in manchen fällen nicht zu hoch wird" — Breite ist ein Freiheitsgrad, Höhe ist der Befund; die Messung nimmt die Höhe je Leiste mit auf, und wo eine heute stapelt, lautet die Empfehlung breiter machen statt stauchen, mit konkreter Soll-Breite | S | **d3, läuft** — Bericht wird die Umsetzungsliste |
...
| G4 | **Kritisch**: Der Bausteinkatalog — zehn Sekunden Textwüste ohne Wartesignal, dann Kacheln ohne Ruheform, nicht normierte Renderings, springende Titelzeilen (B4+B25) | M | **B4 zu** (`482a3aac`, 3a): Platzhalter von der ersten Millisekunde, gemessen 2 s statt der notierten 10 bis zum letzten Bild — der Balken ist begründet abgelehnt (unter der wartezeit-Grenze, Lärm für eine halbe Sekunde); die Grundlinien fluchten nebenbei. **B25-Rest, gemessen zerlegt** (3a): Normierung war schon da (`scale=(size−16)/span`), Grundlinien mit dem Platzhalter zu, die Kachel-Ruheform liegt in `style.py` (→ 72, mit B32/B37), und die Umbruchlöcher bleiben offen — die erste Sonde meldete bei sieben Breiten dieselbe glatte Zahl („eine Spalte“ überall), das Layout lief nicht durch, die Messung trägt nicht; 3a misst mit echtem Fenster nach |
...
| G6 | **Notiz liegt vor und ist reviewt** (`9714dabe`, `konzepte/konzept-akzentfarben-haushalt-2026-08.md`): Bernstein trägt neun Bedeutungen, vier leuchten ohne Anlass. Die Trennlinie ist **flüchtig gegen dauerhaft** (beim Review übernommen — sie erklärt auch die Grenzfälle Fortschritt und aktives Werkzeug ohne Ausnahme): Der Akzent gehört dem Flüchtigen und genau einem Dauerhaften, dem Hauptknopf — der auch gesperrt sichtbar bleibt, und „Schließen“ ist nie einer. Leiser: aktives Werkzeug (gedämpfte Fläche), Kartenkanten (Linienfarbe). Eigene Farbe: Ablaufdatum → Warnung, „nimmt Material weg“ → Rollenfarbe. B16: Farbe bleibt am Symbol, der Satz wird Fließtext. Wächter: höchstens ein Akzent-Element im Ruhezustand, am echten Fenster. Umsetzung in Teilen: Kartenkanten sind zu (`ffb76529`: Umfragekarte auf Linienfarbe; die Startkachel-Kante bleibt — sie steht nur unter dem Zeiger und ist damit flüchtig, die eigene Trennlinie schlug die pauschale Vorgabe), Katalogfarbe war schon eigen (#d19e57 ≠ Akzent, gemessen); Ablaufdatum-Warnfarbe mit G10 (72), Werkzeug-Dämpfung in style.py (72, nach G10), B22-Hauptknöpfe je Dialog (15, nach G8), Ruhezustands-Wächter (d3, nach P2) | L | **in Umsetzung, verteilt** |
| G7 | Widget-Grundformen in `style.py`: Slider ungestylt, Spinbox-Pfeile 10×11 mit verdecktem Trennstrich, Splitter-Fuge Kontrast 1,0, gesperrte Zustände kaum unterscheidbar, Werkzeugknopf rahmenlos (B7/B15/B30/B32/B37) | M | **72, läuft** — Teil 1 fertig (`5adc63f7`): der Regler hat als letztes Bedienelement seine Form, am Einbauort gemessen (trug in beiden Themen dieselben 21 Qt-Vorgabefarben); alle Zustände inklusive gesperrt, drei Regler hängen daran. die Splitter-Fuge ist zu (`3a789db0`) und Zahlenfeld/Combobox auch (`b220ea78`: Knöpfe vier Punkte breiter und ins Innere gerückt, Pfeilfeld mit Radien — dabei fing der geschärfte Wächter, dass Qt bei eigener ::drop-down-Regel den Pfeil nicht mehr zeichnet, und verlangt jetzt von jedem gestalteten Pfeil sein Bild). **Offen aus dem Bündel: B32 (gesperrte Ankreuzfelder) und B37 (ToolButton-Ruheform)** — nachliefern oder am Einbauort als gegenstandslos belegen |
| G8 | Formular-Raster der Dialoge: springende Feldkanten im Operationsdialog, zwei Raster im Einstellungsdialog, halbleere Gruppen der Druckeinstellungen, uneinheitliche Nachkommastellen, fehlender Rhythmus im Erststart (B8/B10/B11/B36/B38/B39) | M | **15, läuft** — der Rasterteil ist zu (`b0a25328`): `align_forms` legt alle Beschriftungen eines Dialogs auf die breiteste, nur sichtbare Zeilen zählen (der Prüfstand maß erst Phantome zugeklappter Zeilen mit); Einstellungen 2→1, Bohrdialog 2→1, Druckeinstellungen 10→1 Kanten. Rechte Kanten bleiben per getroffener Entscheidung (kein Feld breiter als sein Wert). B10 ist gemessen und **entschieden: zweispaltige Tiefe als eigenes Paket** — die Leere rechts (29–50 %) ist Folge der breiten Kopf-, Reiter- und Vorschlagszeilen, nicht der Felder; schmaler bräche jene, breiter wäre die Rückkehr des 726-Punkte-Kastens. Zwei Spalten je Gruppe füllen die Breite mit Inhalt und halbieren die Höhe der größten Gruppe (dieselbe Linie wie Roberts Breite-statt-Höhe-Order). 15 baut es nach B36/B38/B39 |
