...
| Neun Z-Pakete des Zeichenmodus | Der Zeichenmodus und der Viewport bekommen ihre Durchsicht (30.08.2026) | die Abarbeitung — Z1/Z2/Z3/Z4/Z8 sind zu, Z9 ist zu (`7a71154b` — Ende-zu-Ende gemessen: Klick, Dialog endgültig zu, die vier Rechteckseiten reisen mit in den Modus, „Fertig" ändert denselben Schritt; der Knopf steht nur, wo es etwas zu ändern gibt), vermessen und entschieden war: Dieselben fünf Skizzen-Operationen laufen beim **Anlegen** im vollen Viewport-Modus (Ziehgriff, Maßeingabe im Bild, Ebenenkarten, räumlicher Bezug) und beim **Ändern** im nackten Dialog — wer eine Tasche korrigiert, kann dort nicht ziehen, obwohl die Geste sie erzeugt hat. **Gewählt: der Knopf „Im Raum zeichnen" im Dialog** — der Dialog bleibt der schnelle Weg für die Zahl-Korrektur, die Geste ist einen Klick entfernt, und der Kunde entscheidet statt der Anwendung; mit der Auflage, dass der Knopf den Dialog **endgültig** schließt und „Fertig" im Viewport den Schritt direkt ändert (die Skeletteditor-Mechanik aus `b3cfd578` existiert dafür) — kein modaler Dialog, der wiederkommt, keine neue Zustandsmaschine; **Z5/Z6 sind zu (`84589c99`, 15, reviewt) — damit ist die komplette Zeichenmodus-Serie abgearbeitet.** Z5: Der Escape-Code der Zeichenfläche war richtig und lief nie — das Kürzel am Fenster greift, bevor ein keyPressEvent die Taste sieht; jetzt drei Stufen (Kette, Werkzeug, Skizze), und der alte Zweig bleibt für den fensterlosen Gebrauch stehen. **Dritter Fund der Familie „Empfänger auf der Zeichenfläche, Ereignis kommt nie an"** (nach Z4-Doppelklick und dem Escape-Fall) — wer der Zeichenfläche einen Ereignis-Handler gibt, prüft zuerst, ob das Ereignis sie je erreicht. Z6: Der Knopf heißt „Rechteck" (was sein Klick tut), die sechs Formen bleiben hinter dem Pfeil (der häufigste Fall zahlt keinen Klick), und die Statuszeile nennt sie — 50s dritter Gedanke, der beste. Drei Selbstfunde von 15, alle von Mutationsprobe/test_translations gefangen (Marke achtmal in der Datei; Sollwert aus dem Prüfling in Reinform; „Grundform" ist Titel eines Registerparameters und kam aus HEAD zurück). Freigabe-Stichprobe 5 Tests grün; **Z7a ist zu (`2f5d0920`, reviewt, Format 19)**: Wer für eine M3 die Zahl 3,2 tippte, bohrte 6,4 — der Kreis maß den Radius und hieß „Abstand". Entschieden wurde Variante C (Freigabe): `radius` **und** `diameter` als echte Arten, die Migration deutet nur eindeutige Kreis-Bemaßungen um und ändert **keine gespeicherte Zahl** (vier Zweifelsfälle bleiben als Test `distance`), Ø ist Vorgabe am Kreis, R am Bogen; die v18-Beispieldatei entstand **vor** dem Umbau auf dem Weg der Anwendung (`place_measured`) — zehn Minuten später wäre das unmöglich gewesen. Beim Bauen gefunden: zwei Listen derselben Bedingungsarten (Löser und Serializer) standen auseinander — der Löser rechnete, das Einlesen wies ab; sie halten sich jetzt gegenseitig fest. Freigabe-Stichprobe 9 Migrationstests grün. **Der Z7-Rest ist zu (`6f252023`, reviewt) — Z7 ist damit ganz auf:** Maßfeld und Ausdrucks-Dialog heißen „Maß" statt „Abstand" (der neue Katalogtext in allen sechs Sprachen), dazu ein Fehler, den der Bestand fing: `radius`/`diameter` standen als „zwei Punkte" in `_NEEDS` und wurden bei **jeder** Zweipunktauswahl angeboten — jetzt heraus, mit Begründung (sie gelten für Mittelpunkt und Rand desselben Kreises; ein Nachträglich-Anhängen wäre ein eigener Bedienentwurf). **Und ein Lesefund von Robert selbst:** Im Test stand „Ø 3,2 ist ein Kreis von 1,6" — die Zahl stimmte, der Satz nicht, und die Wurzel war ein unklarer Hilfsfunktionsname (`circle_of` statt `radius_of`); der Fehler, gegen den das Paket gebaut ist, stand in seiner eigenen Prüfung. Freigabe-Stichprobe: 72 Skizzentests grün, nur noch die eine korrekte „Abstand"-Stelle im Editor; Review je Paket |
...
| Z6 | Der Knopf heißt „Grundform", ist das Rechteckwerkzeug und versteckt das beste Anfänger-Angebot (sechs vollbemaßte Formen) hinter einem Pfeil | S | zu (`84589c99`, reviewt) — 72 prüft am gebauten Fenster einen gemeldeten Rest („Grundform einfügen" in der Leerzustands-Zeile) |
