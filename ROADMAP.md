...
| Zwei Fenstertests kippen mit der Reihenfolge, und die passendste Erklärung ist widerlegt | Drei Läufe am 29./30.08.2026: `test_parameter_rows_fit_the_left_card…` und `test_the_shortcut_list_knows_the_drawing_keys` sind im Torlauf rot und einzeln grün — auf `e8dc8651`, dem Stand vor dem Kachel-Umbau, identisch rot, sie hängen also an keiner der neuen Zeilen. Beim ersten passte ein Wert exakt, und die Kette daneben war erfunden: 270 Bildpunkte entsprechen genau dem französischen Wert (de 258 · en 246 · fr 270 · es 234 · it 222 · pt 234, Grenze 260) — aber eine Sonde über alle 407 Tests meldete **keinen einzigen Sprachwechsel**. Sechs Sprachen, sechs Breiten, eine trifft auf den Punkt: Das ist eine Korrelation mit sechs Kandidaten, kein Beweis. Ursache offen; gemessen wird jetzt der tatsächliche Zustand beim Fehlschlag (Schrift, Stylesheet, Metriken) | die Ursachenmessung am Fehlschlag selbst. Unabhängig davon und am Code belegt: In `tests/conftest.py` fehlt die `set_language`-Rücksetz-Fixture nach dem Muster der Anzeigeeinheit, deren Begründung wörtlich auch für die Sprache gilt — eine echte Lücke, nur keine, die diese zwei Tests erklärt. Für das `['Home']` des zweiten fehlt die Ursache noch — die Sprache erklärt ihn nicht. Und eine Wegwerf-Sonde vom 30.08. lieferte im Vorbeigehen eine Spur zur Exit-127-Familie: Ein `Viewport()` **ohne Elternfenster** reißt den Prozess mit Exit 127 weg, ohne eine Zeile Ausgabe — die Markierung abzufragen genügt und tut das nicht. Wer nachmisst: nur unter dem Schloss (ohne riss der Lauf zweimal mit Zugriffsverletzung ab), und die zwei reproduzieren **nur** mit dem vollständigen `test_ui.py` davor — einzeln, zu viert und in kleiner Kombination sind sie grün. Vergleichsbaum: `git worktree add --detach <pfad> e8dc8651`, dann mit `cd` hinein statt `--rootdir`, sonst importiert der Lauf `app` aus dem Hauptbaum |
...
| Die Übergabe soll mit jedem installierten Slicer laufen, und bei mehreren fehlt die Auswahl | Weg-Prüfung Ende zu Ende (30.08.2026) und Roberts Auftrag vom selben Tag: „schau dass das mit allen slicern ohne probleme funktioniert", „auswahl bei mehreren slicern wäre auch sinnvoll". `discover.find_program("slicer", …)` nimmt den ersten Treffer — auf Roberts Maschine ElegooSlicer, obwohl PrusaSlicer und Cura 5.13 danebenstehen. Der zuerst gemeldete ElegooSlicer-Fehlschlag war ein Prüfstandsfehler: Der Direktaufruf übersprang die Maschinenprofil-Vorprüfung, die die Anwendung vor jedem Orca-Lauf macht | der Ende-zu-Ende-Lauf je Slicer mit echtem Maschinenprofil (PrusaSlicer ist grün, ElegooSlicer und Cura stehen aus) und eine Auswahl, wo mehrere gefunden werden |
