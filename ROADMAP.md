...
| Der erste Meter: ein Modell vom Startbildschirm öffnen | Review vor der Demo 0.3.0 (02.09.2026) | einen Knopf „Modell öffnen …" neben „Projekt öffnen …" (`action_import`), die Ablagefläche klickbar (`mousePressEvent` → Dateidialog) und den Ablagetext ohne „Bausteindatei" |
| Der letzte Meter: ein sichtbarer Weg zum Slicer | Review vor der Demo 0.3.0 (02.09.2026) | heute den Menüeintrag „Druckeinstellungen …" in „Drucken und slicen …" umbenennen und den leeren Prüfbericht mit „Das Teil ist druckbereit" plus Hauptknopf *An den Slicer übergeben …* versehen; als Umbau der achte Knopf in der Werkzeugleiste |
| Fachwörter ohne Erklärung im Hauptweg | Review vor der Demo 0.3.0 (02.09.2026) | Tooltips an den sieben Analysekarten (`analysis_bar.py`), die Skizzen-Statuszeile mit Klartext vor „Freiheitsgrade", eine zweite Zeile mit dem `doc`-Anfang bei den vier Zwillingspaaren der Palette, das Untermenü „Netz" umbenennen, „Modell einfügen"/„Modell laden" vereinheitlichen, „Kollisionen prüfen" → „Überschneidungen prüfen", Chat-Startsatz ohne „Mutternfalle", den Aufwand im Chat-Hinweis, einen Lage-Satz im Erzeugen-Dialog, Verweise ins Wörterbuch, das Schloss-Label als Symbol, die Kürzungsreihenfolge der Werkzeugleiste, einen Schlussschritt in Tour 2 und 4, die Einheitenrückfrage beim Import mit „Zoll (in)" statt „in" (der Antwortwert muss vom Anzeigetext getrennt am Listeneintrag hängen) |
...
| CRA vor dem 11.09.2026 | Review vor der Demo 0.3.0 (02.09.2026) | Meldeweg und Stellvertretung in `SECURITY-INCIDENT.md`, die Stückliste je Artefakt, die erklärte Unterstützungsdauer — und die Kanzleifrage, ob die kostenlose Demo mit Spendenknopf schon „in Verkehr gebracht" ist |
...
| Rechtstexte: was vor dem Verkauf gebraucht wird | Review vor der Demo 0.3.0 (02.09.2026) | die fünf Punkte aus `marketing/kanzleianfragen/FRAGEN-2026-09-02.md` (Entscheidung Robert 02.09.: nur das Nötige): CRA-Meldeweg bis 11.09., `AGB.md:111` § 356 Abs. 5 oder 6 mit `tests/test_legal.py:188`, Merchant of Record, Markenrecherche, Impressum-Kontaktweg; alles Weitere (Speicherdauer der Aktivierungsdaten, Datenschutz in den Zielsprachen, Schweiz, LGPL-Quelltextangebot, Modelllizenzen) nur, wenn die Kanzlei es anspricht |
...
| PHP-Endpunkte: kleine Härtungen | Review vor der Demo 0.3.0 (02.09.2026) | `activation.php:59`/`deactivation.php:92` die sodium-Prüfung vor die Ratenbegrenzung; `stats.php:414` bei fehlendem Zählerordner 503 statt „Zu viele Versuche"; `activation_common.php:508/813` Test-Schlüssel nur unter `cli-server`; `support.php` Docblock um `appdata/` 0700; ein eigener Rate-Key statt `activation_seed()` als Pseudonymwurzel (`activation_common.php:203`) |
| Kern: Restpunkte aus dem Review | Review vor der Demo 0.3.0 (02.09.2026) | `project.py:77/583` Summengrenze über der Einzelgrenze (zwei 300-MB-Modelle sind sonst unspeicherbar) oder ein Hinweis in der Meldung; `_read_linked_source` (falscher OSError-Text, `require_checksum` bei leerem Hash) vor dem Verknüpfungsweg; „Deckel"/„Prüfstück" mit Zähler; `_fell_apart` einmal statt zweimal; `catalog.detail()` zeigt Autor und Lizenz |
| Regelwerk-Nachträge | Review vor der Demo 0.3.0 (02.09.2026) | Roberts Ansage für `AGENTS.md`/`CLAUDE.md`: Regel 5 und 8 als Haltungssätze ohne Testzusage, „nach jedem Schritt die vollständige Suite" zweistufig (betroffene Dateien je Schritt, Tor vor dem Commit), „Verzweigungen im Op-Stack" mit „in dieser Ausbaustufe (§41)", der Commit-Satz `CLAUDE.md:281` als „auf Ansage"; ohne Ansage: Wächter für Regel 2/6/10/19 in `tests/test_hard_rules.py` mit Gegenproben, die Suite-Fahrweise nur in `/pruefen` und `tests/CLAUDE.md`, die Fallteile von `tests.md`/`oberflaeche.md`/`ansicht.md` nach `.claude/memory/`, eine `.claude/rules/auslieferung.md` für `tools/` und `packaging/`, ein Regelnummern-Test wie `test_plan_references.py`, eine Frontmatter-`paths`-Prüfung |
...
| Handbuch, SEO und Stempel vor dem Upload neu erzeugen | Review vor der Demo 0.3.0 (02.09.2026) | `make_manual.py` → `make_seo.py` → `stamp_assets.py` in dieser Reihenfolge — die fünf übersetzten `manual.html` und `llms.txt` stammen von einem älteren Katalogstand, und `manual.py` hat heute drei Absätze geändert |
...
- [ ] **Der erste Meter: ein Modell vom Startbildschirm öffnen.** Wartet auf einen Knopf „Modell öffnen …" neben „Projekt öffnen …" (`action_import`), die Ablagefläche klickbar (`mousePressEvent` → Dateidialog) und den Ablagetext ohne „Bausteindatei".
- [ ] **Der letzte Meter: ein sichtbarer Weg zum Slicer.** Wartet auf heute den Menüeintrag „Druckeinstellungen …" in „Drucken und slicen …" umbenennen und den leeren Prüfbericht mit „Das Teil ist druckbereit" plus Hauptknopf *An den Slicer übergeben …* versehen; als Umbau der achte Knopf in der Werkzeugleiste.
- [ ] **Fachwörter ohne Erklärung im Hauptweg.** Wartet auf Tooltips an den sieben Analysekarten (`analysis_bar.py`), die Skizzen-Statuszeile mit Klartext vor „Freiheitsgrade", eine zweite Zeile mit dem `doc`-Anfang bei den vier Zwillingspaaren der Palette, das Untermenü „Netz" umbenennen, „Modell einfügen"/„Modell laden" vereinheitlichen, „Kollisionen prüfen" → „Überschneidungen prüfen", Chat-Startsatz ohne „Mutternfalle", den Aufwand im Chat-Hinweis, einen Lage-Satz im Erzeugen-Dialog, Verweise ins Wörterbuch, das Schloss-Label als Symbol, die Kürzungsreihenfolge der Werkzeugleiste, einen Schlussschritt in Tour 2 und 4, die Einheitenrückfrage beim Import mit „Zoll (in)" statt „in" (der Antwortwert muss vom Anzeigetext getrennt am Listeneintrag hängen).
...
- [ ] **CRA vor dem 11.09.2026.** Wartet auf Meldeweg und Stellvertretung in `SECURITY-INCIDENT.md`, die Stückliste je Artefakt, die erklärte Unterstützungsdauer — und die Kanzleifrage, ob die kostenlose Demo mit Spendenknopf schon „in Verkehr gebracht" ist.
...
- [ ] **Rechtstexte: was vor dem Verkauf gebraucht wird.** Wartet auf die fünf Punkte aus `marketing/kanzleianfragen/FRAGEN-2026-09-02.md` (Entscheidung Robert 02.09.2026: „nur das was wir wirklich brauchen"): CRA-Meldeweg bis 11.09. (Absatz in `SECURITY.md`), `AGB.md:111` § 356 Abs. 5 oder 6 zusammen mit `tests/test_legal.py:188`, Merchant of Record (Vertragspartner, Widerruf, § 356a, Umsatzsteuer), Markenrecherche Klassen 9 und 42, Impressum-Kontaktweg nach EuGH C-298/07. Alles Weitere aus dem Review (Speicherdauer der Aktivierungsdaten, Datenschutzerklärung in den Zielsprachen, Schweiz, LGPL-Quelltextangebot, TripoSG-/SDXL-Lizenzkette, Anthropic-Rolle, § 327h, § 309, Exportkontrolle, Produkthaftungs-Richtlinie, PayPal-Steuer) ist Prüfliste für den Juristen, keine Aufgabe für uns, solange er es nicht anspricht.
...
- [ ] **PHP-Endpunkte: kleine Härtungen.** Wartet auf `activation.php:59`/`deactivation.php:92` die sodium-Prüfung vor die Ratenbegrenzung; `stats.php:414` bei fehlendem Zählerordner 503 statt „Zu viele Versuche"; `activation_common.php:508/813` Test-Schlüssel nur unter `cli-server`; `support.php` Docblock um `appdata/` 0700; ein eigener Rate-Key statt `activation_seed()` als Pseudonymwurzel (`activation_common.php:203`).
- [ ] **Kern: Restpunkte aus dem Review.** Wartet auf `project.py:77/583` Summengrenze über der Einzelgrenze (zwei 300-MB-Modelle sind sonst unspeicherbar) oder ein Hinweis in der Meldung; `_read_linked_source` (falscher OSError-Text, `require_checksum` bei leerem Hash) vor dem Verknüpfungsweg; „Deckel"/„Prüfstück" mit Zähler; `_fell_apart` einmal statt zweimal; `catalog.detail()` zeigt Autor und Lizenz.
- [ ] **Regelwerk-Nachträge.** Wartet auf Roberts Ansage für `AGENTS.md`/`CLAUDE.md`: Regel 5 und 8 als Haltungssätze ohne Testzusage, „nach jedem Schritt die vollständige Suite" zweistufig (betroffene Dateien je Schritt, Tor vor dem Commit), „Verzweigungen im Op-Stack" mit „in dieser Ausbaustufe (§41)", der Commit-Satz `CLAUDE.md:281` als „auf Ansage"; ohne Ansage: Wächter für Regel 2/6/10/19 in `tests/test_hard_rules.py` mit Gegenproben, die Suite-Fahrweise nur in `/pruefen` und `tests/CLAUDE.md`, die Fallteile von `tests.md`/`oberflaeche.md`/`ansicht.md` nach `.claude/memory/`, eine `.claude/rules/auslieferung.md` für `tools/` und `packaging/`, ein Regelnummern-Test wie `test_plan_references.py`, eine Frontmatter-`paths`-Prüfung.
...
- [ ] **Handbuch, SEO und Stempel vor dem Upload neu erzeugen.** Wartet auf `make_manual.py` → `make_seo.py` → `stamp_assets.py` in dieser Reihenfolge — die fünf übersetzten `manual.html` und `llms.txt` stammen von einem älteren Katalogstand, und `manual.py` hat heute drei Absätze geändert.
