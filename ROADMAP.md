...
| Zwei Fenstertests kippen mit der Reihenfolge, einer an einer hängengebliebenen Sprache | Drei Läufe am 29./30.08.2026: `test_parameter_rows_fit_the_left_card…` und `test_the_shortcut_list_knows_the_drawing_keys` sind im Torlauf rot und einzeln grün — auf `e8dc8651`, dem Stand vor dem Kachel-Umbau, identisch rot, sie hängen also an keiner der neuen Zeilen. Beim ersten ist die Ursache gemessen: 270 Bildpunkte sind exakt der französische Wert (de 258 · en 246 · fr 270 · es 234 · it 222 · pt 234) gegen eine Grenze von 260 — ein Test lässt die Anzeigesprache hängen | eine `set_language`-Rücksetz-Fixture in `tests/conftest.py` nach dem Muster der Anzeigeeinheit, deren Begründung wörtlich auch für die Sprache gilt; Vorsicht: die Datei trägt 33/19 unzugeordnete Baum-Zeilen. Für das `['Home']` des zweiten fehlt die Ursache noch — die Sprache erklärt ihn nicht. Wer nachmisst: nur unter dem Schloss (ohne riss der Lauf zweimal mit Zugriffsverletzung ab), und die zwei reproduzieren **nur** mit dem vollständigen `test_ui.py` davor — einzeln, zu viert und in kleiner Kombination sind sie grün. Vergleichsbaum: `git worktree add --detach <pfad> e8dc8651`, dann mit `cd` hinein statt `--rootdir`, sonst importiert der Lauf `app` aus dem Hauptbaum |
