...
| Der ComfyUI-Startknopf meldet Fehlschlag über einen laufenden Start | Weg-3-Prüfung Ende zu Ende (30.08.2026): `tools.START_TIMEOUT_SECONDS` steht auf 20 s, ComfyUI Desktop braucht auf dieser Maschine über zwei Minuten bis zur Antwort — der Prozess läuft die ganze Zeit, nur der Port ist noch zu. Genau der Fall aus `kern.md`: Ein Zeitlimit gilt dem Hängen, nicht der Langsamkeit | keine größere Zahl (zwei stumme Minuten verstießen gegen §2.8), sondern eine Auskunft, die den laufenden Start benennt — „ComfyUI startet, das dauert ein bis zwei Minuten" mit weiterlaufendem Zustand; das ist ein Bedienentwurf und liegt bei Robert |
