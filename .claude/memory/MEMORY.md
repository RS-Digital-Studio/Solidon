...
- [Changelog vor dem Versionssprung](changelog-vor-dem-versionssprung.md) · [Wächter erst nach dem Sprung](changelog-waechter-nach-dem-versionssprung.md) · [Kein zweites Tor vor dem Tag](kein-zweites-tor-vor-dem-tag.md) — Abschnitt zuerst, auch für unfertige Arbeit; die Version hebt erst der Bau; nach Bump und Erzeugen direkt taggen, die CI misst.
- [Serverstand sofort selbst prüfen](serverstand-sofort-selbst-pruefen.md) — eine Frage nach dem Stand draußen ist der Auftrag, ihn zu messen und nachzuziehen; nicht erst fragen.
...
- [Kalenderdatum](kalenderdatum-folgt-appsprache.md) · [QDateEdit-Sonderwert](qdateedit-sonderwert-schluckt-die-eingabe.md) · [Sprachwechsel](sprachwechsel-zwei-schritte.md) · [Katalogschlüssel](katalog-schluessel-sind-woerter.md) · [Marke im span](marke-im-span-zerteilt.md) — Sprache und Kataloge; am Sonderwert erst ein echtes Datum (1. Januar), dann tippen.
...
- [Begrenzt am falschen Maß](begrenzt-am-falschen-mass.md) · [Schranke aus einem Messwert](schranke-aus-einem-messwert-ist-geraten.md) · [Obergrenze ≠ Zusicherung](obergrenze-ist-keine-zusicherung.md) · [Zwei Schwellen](zwei-schwellen-eine-frage.md) · [Schwelle, falsche Achse](schwelle-misst-die-falsche-achse.md) — Grenzen; die Nachbarschwelle zuerst übernehmen, den gefürchteten Fall messen.
...
- [Eigene Toleranz gilt nicht für fremde Netze](eigene-toleranz-gilt-nicht-fuer-fremde-netze.md) — Schweißtoleranz 0,2 µm wies jede importierte runde Wand ab; Siebhalter+X1C.3mf ist das Bajonett-Referenzstück.
...
- [Ungenutzter Import reißt den Prozess](ungenutzter-import-reisst-den-prozess.md) — zweimal auf einen Qt-Import bisektiert, Ursache war PySides Lazy-Init; `PYSIDE6_OPTION_LAZY=0` in app/ui, Importe und die Einstellung dahinter verdächtigen.
- [Absturz-Frame](absturz-frame-ist-die-naechste-allokation.md) · [Speicherriss ohne Zeile](speicherriss-hat-keine-ausloesende-zeile.md) · [rtree-Abstürze](rtree-abstuerze-im-langen-lauf.md) · [Native Bibliotheken](native-bibliotheken-speicher.md) · [ast.walk reißt im Torlauf](ast-walk-reisst-im-torlauf.md) — Abrisse: eine Familie; TypeError in iter_fields ist Umgebung, standalone dreimal fahren.
...
- [Sonde über alle Operationen](sonde-ueber-alle-operationen.md) — durchs echte Fenster, ein Prozess je Kategorie, Modal-Wächter, Zeilen sofort; 28 Dialoge mit lügendem Band.
...
- [Regel gilt je Bedienort](regel-gilt-je-bedienort.md) — die Bausteinregel galt rechts und nicht am Griff; jeden Ort derselben Sache einmal fahren.
- [Zwei Wünsche, eine Taste](zwei-wuensche-eine-taste-die-lage-entscheidet.md) — Entf: morgens „nimmt den Körper“, abends „nimmt ihn nicht mehr“; die Regel je Lage schreiben, nie die Richtung drehen.
- [Warnung ist keine Wirkung](warnung-ist-keine-wirkung.md) — die Geste verspricht die Wirkung; ein Befund über das andere Ergebnis ist keine Lösung.
...
- [Teardown-Riss bei gewählter Fläche](teardown-riss-bei-gewaehlter-flaeche.md) — Exit 127 nach grünen Zusicherungen, am HEAD genauso, echter Schließweg sauber; Auswahl vor dem Testende leeren; py-spy kennt 3.14 nicht, faulthandler.dump_traceback_later.
...
- [Patchübernahme in den geteilten Baum](patchuebernahme-in-den-geteilten-baum.md) — `git apply -3` bricht an jeder schmutzigen Datei ganz ab; Kataloge und Changelog per Schlüssel- und Ankerskript; fremde Werkzeuge schreiben ganze Dateien als CRLF, binär zählen.
- [Skript im Worktree lädt app aus dem Hauptbaum](skript-im-worktree-laedt-app-aus-dem-hauptbaum.md) — sys.path[0] ist der Skriptordner; Sonden setzen den Baum selbst vorn und prüfen app.__file__.
...
- [Datei vor dem Patch modifiziert: nur als Blob](datei-die-vor-dem-patch-modifiziert-war-geht-nur-als-blob.md) — `git status` vor dem Patch in die Commit-Liste übernehmen; 280dbe51 nahm vier fremde Hunks mit; den geteilten Index nur für eigene Pfade zurücksetzen.
...
- [Geteilter Index nach fremdem Commit veraltet](geteilter-index-nach-fremdem-commit-veraltet.md) — `git diff --cached --stat` vor jedem Commit; Phantom-Löschungen heilt `git reset -q`; Index nur gegen einen HEAD bauen, numstat-Gegenprobe; und zwischen `add` und `commit` staget die Parallelsitzung dazwischen — mit Pathspec committen.
- [Fremder Commit nimmt unfertige Hunks mit](fremder-commit-nimmt-unfertige-hunks-mit.md) — 4f1c49b5 committete zwei Kopien meiner Tests ohne den Kern; HEAD vor jedem Blob-Bau auf eigene Marker greppen, Kopien und Paare im selben Lauf erzeugen.
