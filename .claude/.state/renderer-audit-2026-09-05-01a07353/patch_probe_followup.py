...
OUT = HERE / "patch-probe-v6-followup"
...
try:
    ...
    def continuous(raw, chosen, lift, offset, coordinates):
        indices = np.asarray(raw.faces)[chosen].reshape(-1)
        points = np.asarray(raw.vertices)[indices]
        keys = points if coordinates else indices
        groups, mapping = np.unique(keys, axis=0, return_inverse=True)
        weighted = np.repeat(np.asarray(raw.face_normals)[chosen]*np.asarray(raw.area_faces)[chosen,None],3,axis=0)
        means = np.zeros((len(groups),3))
        np.add.at(means,mapping,weighted)
        lengths = np.linalg.norm(means,axis=1)
        means /= np.maximum(lengths,1e-15)[:,None]
        return view._clip_feature_corners(view._lift_within_section(points,means[mapping]*lift))+offset

    def shared_ids(self, raw, chosen, lift, offset):
        return continuous(raw, chosen, lift, offset, False)

    def shared_coordinates(self, raw, chosen, lift, offset):
        return continuous(raw, chosen, lift, offset, True)
    ...
    def control(name, method, depth):
        ...
        for obj in patch.objects:
            ...
            if not depth:
                obj.material.render_queue = 4000
        view._draw()
        pump(.3)
        image = shot(name)
        delta = np.max(np.abs(image.astype(int)-reference.astype(int)),axis=2)
        result["checks"].append({"name": name, "depth_test": depth, "render_queues": [obj.material.render_queue for obj in patch.objects], "pixels_changed_vs_original": int(np.count_nonzero(delta>3))})
        print(name, flush=True)
        (OUT/"result.json").write_text(json.dumps(result,indent=2),encoding="utf-8")

    control("01-original-overlay-queue", None, False)
    control("02-shared-ids", shared_ids, True)
    control("03-shared-coordinates", shared_coordinates, True)
    control("04-shared-coordinates-overlay-queue", shared_coordinates, False)
