...
MANIFEST = ROOT / "core_crash_probes_v7_manifest.json"
...
def counter(manifest, out):
    ...
    for entry in sorted(index["objects"], key=lambda value: value["id"]):
        ...
        assert set(entry["features"]) == set(wanted["features"])
        metadata = {
            ...
            "features": {
                fid: {
                    ...
                    "face_hash": hashlib.sha256(
                        np.asarray(entry["features"][fid]["face_indices"], dtype=np.int64).tobytes()
                    ).hexdigest(),
                }
...
def main():
    ...
    return 0 if len(processes) == 2 and all(p.get("exit") == 0 for p in processes) else 1
