...
| Blockieren die gemeinsamen Projekt-Hooks verwerfende Codex-Befehle mit dem unterstützten Protokoll, und gibt das Sitzungsende sein Gebiet frei? | `test_solidon3d_hooks.py` |
| Stehen die Codex-Agentenprofile auf dem Stand ihrer Claude-Quelle — und ist jedes lesbares TOML? | `test_agent_mirror.py`; erzeugt werden sie von `tools/sync_agents.py`, und Codex läuft nicht auf jeder Maschine hier, also prüft der Test die Dateien selbst mit `tomllib` |
