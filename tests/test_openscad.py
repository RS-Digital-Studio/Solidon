"""OpenSCAD as a checked fallback (Bauplan §32, §36, §40 for P6).

The acceptance criterion is blunt: refused source is **provably not executed**.
So the tests do not check that a run fails — they check that no run happens.
"""
...
def test_a_reference_out_of_the_folder_is_refused(source: str) -> None:
    """§32: include, use, import and surface only relative and below."""
...
def test_a_mixture_is_refused_as_a_whole() -> None:
    """One bad reference is enough — the rest is not run "as far as possible"."""
...
def test_the_error_carries_a_way_out() -> None:
    """§33.1: every exception names at least one thing to do."""
...
def test_without_an_installation_it_says_so(monkeypatch: pytest.MonkeyPatch) -> None:
    """§36: OpenSCAD is called, never bundled — so it can be missing."""
...
def test_a_run_gets_its_own_folder_and_no_network(monkeypatch: pytest.MonkeyPatch) -> None:
    """§32: fixed working directory per run, no way out."""
...
def test_a_render_says_where_the_body_came_from(monkeypatch: pytest.MonkeyPatch) -> None:
    """§26.4 in spirit: a body out of OpenSCAD is not a body out of the library."""
...
def test_the_operation_checks_before_it_runs(monkeypatch: pytest.MonkeyPatch) -> None:
    """``create_from_scad`` is the fallback level — and it checks first (§32)."""
...
def test_no_core_path_needs_openscad() -> None:
    """§24.1: parts build against manifold3d, so the library works without it."""
