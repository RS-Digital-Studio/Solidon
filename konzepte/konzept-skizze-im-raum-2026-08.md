...
| P1b | `Viewport.view_on_plane(frame)` mit `camera_for_plane` als freier Funktion; Raster als Anzeige-Actor | S | **grün:** `camera_for_plane` gegen die Ebenennormale, ohne Plotter kehrt `view_on_plane` folgenlos zurück. **Bild:** Kamera steht senkrecht, Raster liegt in der Ebene | **fertig** `7bbe3887`, berichtigt `be84c697` (XZ ist linkshändig, siehe §1.8) und `8a1f172a` (Untergrenze für die Kameradistanz) |
...
| P2b | Kurven und Raster als Actor in der Szene (Entscheidung B); Rasterweite aus dem Kameramaßstab | L | **grün:** die Rasterweite als freie Funktion gegen die 1-2-5-Folge; ohne Plotter kehrt das Zeichnen folgenlos zurück. **Bild:** Skizze liegt auf der Fläche, Raster darunter | **fertig** `677571ef`, `a2295a31` (das Raster hing an `solved` und fehlte bei leerer Skizze), `8a1f172a` (die Weite kam aus der unsichtbaren Zeichenfläche) |
...
| P3 | Fläche anklicken → Skizze dort (E). Zeigemodus `_sketching` in `_on_picked`, Kontextmenüeintrag | M | **grün:** ein eingespeister Flächentreffer setzt `sketch.plane` auf `feature:<id>`, das Klappfeld zeigt dieselbe Wahl. **Bild:** Klick auf die Deckfläche beginnt dort | **fertig** `824f3eb3` |
| P4 | **Der Schnitt.** `start_sketch` schwenkt statt zu tauschen; Modell abgeblendet; Ansichtsaktionen und Ziffern wieder frei (A, D) | L | **grün:** `middle_stack` wechselt die Seite nicht mehr; Ansichtsaktionen sind aktiv **und wirken** — geprüft an der Kameravorgabe, nicht am `enabled`-Zustand; Escape kommt heraus. **Bild:** der Schwenk | **fertig** `84528bed`, `c3155cb9` (Klick trifft die Ebene), `ffa5ebfd` (die Ziffern hingen an `WidgetWithChildrenShortcut` und hätten nach dem Schnitt nie mehr gefeuert), `d6b7acd7` (die Bedienleiste verdeckte die untere Bildhälfte) |
| P5 | Abbau: `SketchPanel` als Vollbildseite entfällt; `SketchField`/`SketchEditorDialog` bleiben (zweiter Weg über den Op-Dialog) | M | **grün:** Referenzzahl der Suite unverändert, kein toter Import | **fertig** `8a1f172a` — `set_zone_margins`, `in_viewport()` und `_in_viewport` sind weg; `OverlayHost.card_width` trägt den Absatz, warum die Ausweichrechnung trotzdem bleibt |
| P6 | Doku: `konzept-bedienung.md` Teil 4 um die Raumachse ergänzen, `ROADMAP.md` fortschreiben, Handbuch und Abbildung nachziehen | S | `make_manual.py` läuft; Abbildung neu | **läuft** |
