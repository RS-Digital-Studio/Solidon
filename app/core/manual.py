...
INTRODUCTION: Final[tuple[Page, ...]] = (
    ...
    Page(
        ...
        body=_(
            ...
            "![](figure:start-screen)\n\n"
            "Unter den vier Einstiegen stehen zwei kleinere Karten. *Feedback "
            "geben* öffnet den Rückmeldungsdialog und zeigt vor dem Senden eine "
            "Vorschau. *Solidon freiwillig unterstützen* öffnet zuerst einen "
            "Hinweis; PayPal folgt erst im nächsten Schritt. Beide Karten "
            "ändern weder das Projekt noch den Verlauf.\n\n"
    ...
    Page(
        ...
        body=_(
            ...
            "**Er gehört Ihnen und wird auf diesem Rechner angelegt.** Beim "
            "Speichern wird nichts hochgeladen. Im Katalog steht er mit einer "
            "Markierung neben den mitgelieferten. Über eine `.solidon-part`-Datei "
            "können Sie ihn später selbst weitergeben. Wenn Sie ein Projekt "
    ...
    Page(
        key="exchange",
        summary=_(
            "Einen eigenen Baustein exportieren und einen fremden importieren — "
            "lokal per Datei, ohne Konto oder Server."
        ),
        title=_("Bausteine als Datei importieren und exportieren"),
        body=_(
            "Bausteine wechseln in Solidon über eine Datei den Rechner. Es gibt "
            "dafür keine gehostete Tauschbörse: Solidon lädt nichts hoch und "
            "braucht weder Konto noch Mailadresse. Sie entscheiden selbst, wie "
            "die Datei zum Empfänger gelangt.\n\n"
            "**Exportieren beginnt im Bausteinkatalog** (*Datei → "
            "Bausteinkatalog …*, Strg+K). Wählen Sie einen eigenen Baustein und "
            "drücken Sie *Als Datei exportieren …*. Der Speicherdialog heißt "
            "*Baustein als Datei exportieren*; der Dateityp darin heißt "
            "*Baustein weitergeben*. Solidon ergänzt die Endung "
            "`.solidon-part` selbst.\n\n"
            "**Nur eigene Bausteine lassen sich exportieren.** Bei "
            "mitgelieferten oder aus einer Datei übernommenen Bausteinen bleibt "
            "der Knopf gesperrt und nennt den Grund direkt daneben.\n\n"
            "**Die Datei enthält ein Rezept und keinen ausführbaren Code.** Sie "
            "trägt die registrierten Schritte, ihre Werte, benannte Stellen und "
            "gegebenenfalls die eingebetteten Quelldaten, aus denen der Baustein "
            "entsteht. Der Empfänger rechnet ihn in seiner Installation neu.\n\n"
            "**Importieren geht im selben Katalog.** *Aus Datei importieren …* "
            "öffnet den Dialog *Baustein aus Datei importieren*; der Dateityp "
            "heißt dort *Baustein hinzufügen*. Eine `.solidon-part`-Datei lässt "
            "sich außerdem auf das Fenster ziehen oder über ihre Dateizuordnung "
            "öffnen. Nach erfolgreicher Prüfung steht der Baustein im Katalog.\n\n"
            "**Solidon prüft beide Richtungen lokal.** Vor dem Speichern und vor "
            "dem Import gelten dieselben Grenzen für Dateigröße, Felder, Werte "
            "und bekannte Operationen. Wird eine Datei abgelehnt, nennt die "
            "Meldung den Grund und bietet den nächsten Schritt an.\n\n"
            "**Ein importierter Baustein bleibt bei diesem Projekt.** Der "
            "Katalog kennzeichnet seine Herkunft; er wird nicht in den Ordner "
            "der eigenen Bausteine geschrieben. Gibt es bereits einen eigenen "
            "Baustein mit demselben Namen, bleibt dieser unverändert, und der "
            "importierte steht unter einem abgeleiteten Namen daneben."
        ),
    ),
    Page(
        key="print",
    ...
    Page(
        ...
        body=_(
            ...
            "Einrichtung in ComfyUI geholt. Andere Modelle, darunter "
            "Hunyuan3D, haben eigene Bedingungen, die für die jeweils "
            "eingesetzte Fassung vor dem Einsatz geprüft werden müssen.\n\n"
    ...
    Page(
        ...
        body=_(
            ...
            "gemessene Auftrag von Solidon ist 19 641 Token lang; beim Rückfall "
            "auf den Prozessor dauert es dort etwa **zweiundvierzig Minuten**, "
            "bis eine Antwort überhaupt beginnt. Die Messwerte für eine "
            "unterstützte Grafikkarte stehen davon getrennt. "
